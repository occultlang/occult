...
namespace occultlang { // for expressions, operator precedence doesn't matter, we're transpiling, a C++ compiler will take care of it :) 
	enum ast_type { /* there is one type for integers and floats :) */
		root,
		num_declaration,
		bool_declaration,
	...
	namespace occ_ast {
		...
		*/
		...
		struct break_declaration : public ast { // ast is just one break statement lol
