...
int main(int argc, char *argv[]) {
  ...
  if (parser.get_state() == occult::parser::state::failed) {
    std::cout << RED << "[OCCULTC] Parsing failed with "
              << parser.get_error_count() << " error(s)" << RESET << std::endl;
    return 1;
  }
  if (showtime) {
  ...
  for (const auto &err : linter.get_errors()) {
    const char *prefix = (err.level == occult::lint_error::severity::error)
                             ? RED "[LINT ERROR] " RESET
                             : YELLOW "[LINT WARN]  " RESET;
    std::cout << prefix << err.message << "\n";
  }
  if (!lint_ok) {
    std::cout << RED << "[OCCULTC] Linting failed \u2014 "
              << linter.get_errors().size() << " error(s)" << RESET << "\n";
    return 1;
  }
  ...
  try {
    ...
    std::cerr << RED << "[OCCULTC] Code generation failed: " << RESET
              << e.what() << std::endl;
    return 1;
  }
  end = std::chrono::high_resolution_clock::now();
  ...
  if (jit) {
    if (auto it = jit_runtime.function_map.find("main");
      ...
      struct sigaction sa{}, old_sigsegv{}, old_sigabrt{}, old_sigfpe{},
          old_sigbus{};
      ...
      if (setjmp(jit_jmp_buf) == 0) {
        ...
        std::cerr << RED << "[OCCULTC] JIT execution crashed (signal "
                  << jit_signal_caught << ")" << RESET << std::endl;
        return 1;
      }
  ...
  if (jit) {
    ...
    occult::linker::link_and_create_binary(
        ...
        jit_runtime.function_raw_code_map, jit_runtime.string_literals, debug,
        showtime);
