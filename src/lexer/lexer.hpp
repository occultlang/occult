...
typedef struct token_t {
    ...
    explicit token_t(const std::uintptr_t& line = 0, const std::uintptr_t& column = 0,
                     std::string lexeme = "", const token_type& tt = token_type::unkown_tt)
        : line(line), column(column), lexeme(std::move(lexeme)), tt(tt) {}
...
class lexer {
    ...
    void increment(const std::uintptr_t& _line = 0, const std::uintptr_t& _pos = 0,
                   const std::uintptr_t& _column = 0);
...
public:
    explicit lexer(const std::string& source, const bool use_whitespace = false)
        : source(source), pos(0), line(1), column(1), use_whitespace(use_whitespace) {}
