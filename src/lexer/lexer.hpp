...
typedef struct token_t {
  ...
  explicit token_t(const std::uintptr_t &line = 0,
                   const std::uintptr_t &column = 0, std::string lexeme = "",
...
class lexer {
  ...
  void increment(const std::uintptr_t &_line = 0,
                 const std::uintptr_t &_pos = 0,
...
public:
  ...
  void visualize(
      const std::optional<std::vector<token_t>> &o_s = std::nullopt) const;
};
} // namespace occult
