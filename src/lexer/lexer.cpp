...
token_t lexer::handle_operator(const bool &is_double) {
  ...
  if (is_double) {
    if (pos + 1 < source.size()) {
      increment(0, 2, 2);
      return token_t(line, column,
                     std::string() + source[pos - 2] + source[pos - 1],
                     operator_map_double[std::string() + source[pos - 2] +
                                         source[pos - 1]]);
    }
  } else {
    increment(0, 1, 1);
...
std::vector<token_t> lexer::analyze() {
  ...
  auto is_unary_context = [](token_type prev_type) {
    return prev_type == left_paren_tt || prev_type == comma_tt ||
           prev_type == left_curly_bracket_tt || prev_type == semicolon_tt ||
