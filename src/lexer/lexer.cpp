...
std::vector<token_t> lexer::analyze() {
    ...
    auto is_unary_context = [](token_type prev_type) {
        return prev_type == left_paren_tt || prev_type == comma_tt || prev_type == left_curly_bracket_tt || prev_type == semicolon_tt || prev_type == end_of_file_tt || prev_type == return_keyword_tt || prev_type == not_equals_operator_tt ||
               prev_type == equals_operator_tt || prev_type == greater_than_operator_tt || prev_type == less_than_operator_tt || prev_type == greater_than_or_equal_operator_tt || prev_type == less_than_or_equal_operator_tt ||
