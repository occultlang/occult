    ...
    if (const auto &lexeme = std::string() + source[pos] + source[pos + 1];
      ...
      if (comment_map[lexeme] == comment_tt) {
        ...
        while (pos < source.size()) {
          if (pos + 1 < source.size()) {
            if (const auto &end_lexeme =
                    std::string() + source[pos] + source[pos + 1];
...
token_t lexer::handle_numeric() {
  if (source[pos] == '0') {
    ...
    if (source[pos] == '.') {
      ...
      return token_t(line, column, std::to_string(octal_value),
                     number_literal_tt);
    }
  }
  ...
  while (numeric_set.contains(source[pos]) || source[pos] == '.' ||
         source[pos] == 'e' || source[pos] == 'E' || source[pos] == '-' ||
         source[pos] == '+') {
  ...
  try {
    normal_number =
        to_parsable_type<std::uintptr_t>(normal_number, DECIMAL_BASE);
  } catch (const std::exception &) {
    return token_t(line, column, "0", number_literal_tt);
  }
...
token_t lexer::handle_operator(const operator_type &op_type) {
  ...
  if (op_type == operator_type::three) {
    if (pos + 2 < source.size()) {
      increment(0, 3, 3);
      auto lexeme =
          std::string() + source[pos - 3] + source[pos - 2] + source[pos - 1];
...
token_t lexer::get_next_token() {
  ...
  if (pos + 2 < source.length() &&
      operator_map_triple.contains(std::string() + source[pos] +
                                   source[pos + 1] + source[pos + 2])) {
    return handle_operator(operator_type::three);
  }

  if (pos + 1 < source.length() &&
      operator_map_double.contains(std::string() + source[pos] +
                                   source[pos + 1])) {
  ...
  while (token.tt != end_of_file_tt) {
    if (token.tt != unkown_tt) { // comments and whitespaces are just unknown
      ...
      if (token.tt == asm_keyword_tt) {
        ...
        token_stream.push_back(token_t(
            line, column, "}",
            right_curly_bracket_tt)); // already consumed by asm block func
