...
void lexer::increment(const std::uintptr_t& _line, const std::uintptr_t& _pos,
                      const std::uintptr_t& _column) {
    this->line += _line;
    this->pos += _pos;
    this->column += _column;
}
...
void lexer::handle_comment() {
    if (pos + 1 < source.size()) {
        if (const auto& lexeme = std::string() + source[pos] + source[pos + 1];
            comment_map.contains(lexeme)) {
...
token_t lexer::handle_numeric() {
    ...
    while (numeric_set.contains(source[pos]) || source[pos] == '.' || source[pos] == 'e' ||
           source[pos] == 'E' || source[pos] == '-' || source[pos] == '+') {
...
token_t lexer::handle_operator(const bool& is_double) {
    ...
    if (is_double) {
        ...
        increment(0, 1, 1);
        return token_t(line, column, std::string(1, source[pos - 1]),
                       operator_map_single[source[pos - 1]]);
    }
...
token_t lexer::get_next_token() {
    ...
    if (pos + 1 < source.length() &&
        operator_map_double.contains(std::string() + source[pos] + source[pos + 1])) {
...
std::vector<token_t> lexer::analyze() {
    ...
    auto is_unary_context = [](token_type prev_type) {
        return prev_type == assignment_tt || prev_type == left_paren_tt || prev_type == comma_tt ||
               prev_type == semicolon_tt || prev_type == end_of_file_tt ||
               prev_type == return_keyword_tt || prev_type == not_equals_operator_tt ||
               prev_type == equals_operator_tt || prev_type == greater_than_operator_tt ||
        ...
        if (token.tt != unkown_tt) { // comments and whitespaces are just unknown
            if (token.tt == add_operator_tt || token.tt == subtract_operator_tt) {
                if (is_unary_context(previous_token_type)) {
                    token.tt = (token.tt == add_operator_tt) ? unary_plus_operator_tt
