...
namespace occult {
    std::string token_t::get_typename(const token_type &tt) {
        if (token_typename_map.contains(tt)) { return token_typename_map[tt]; }
        ...
        if (pos + 1 < source.size()) {
            if (const auto &lexeme = std::string() + source[pos] + source[pos + 1]; comment_map.contains(lexeme)) {
                ...
                if (comment_map[lexeme] == comment_tt) {
                    while (pos < source.size() && source[pos] != '\n' && source[pos] != '\r') { increment(0, 1, 1); }
                }
                else if (comment_map[lexeme] == multiline_comment_start_tt) {
                    while (pos < source.size()) {
                        if (pos + 1 < source.size()) {
                            if (const auto &end_lexeme = std::string() + source[pos] + source[pos + 1];
                                comment_map[end_lexeme] == multiline_comment_end_tt) {
                                increment(0, 2, 2); // skip the closing */
                                break;
                            }
                        }
                        if (source[pos] == '\n' || source[pos] == '\r') { increment(1, 0, 0); }
                        increment(0, 1, 1);
                    }
                }
            }
    ...
    token_t lexer::handle_string() {
        ...
        if (pos < source.length() && source[pos] == '\"') { increment(0, 1, 1); }
    ...
    token_t lexer::handle_numeric() {
        ...
        while (numeric_set.contains(source[pos]) || source[pos] == '.' || source[pos] == 'e' || source[pos] == 'E' ||
               source[pos] == '-' || source[pos] == '+') {
            normal_number += source[pos];
            increment(0, 1, 1);
        }

        if (normal_number.contains('.')) { has_decimal_point = true; }

        if (normal_number.contains('e') || normal_number.contains('E')) { has_exponent = true; }
    ...
    token_t lexer::handle_operator(const bool &is_double) {
        ...
        if (is_double) {
            if (pos + 1 < source.size()) {
                increment(0, 2, 2);
                return token_t(line, column, std::string() + source[pos - 2] + source[pos - 1],
                               operator_map_double[std::string() + source[pos - 2] + source[pos - 1]]);
            }
        }
        else {
            increment(0, 1, 1);
    ...
    token_t lexer::handle_symbol() {
        ...
        while (pos < source.length() && alnumeric_set.contains(source[pos])) { increment(0, 1, 1); }

        auto lexeme = source.substr(start_pos, pos - start_pos);

        if (lexeme == "else" && pos < source.length() && source[pos] == ' ') {
            auto next_pos = pos + 1;
            while (next_pos < source.length() && source[next_pos] == ' ') { ++next_pos; }
        ...
        if (keyword_map.contains(lexeme)) { return token_t(line, start_column, lexeme, keyword_map[lexeme]); }

        return token_t(line, start_column, lexeme, identifier_tt);
    }

    token_t lexer::get_next_token() {
        if (pos >= source.length()) { return token_t(line, column, "end of file", end_of_file_tt); }

        if (use_whitespace && whitespace_map.contains(source[pos])) { return handle_whitespace_with_token(); }

        handle_whitespace(); // else

        if (pos < source.length() && source[pos] == '\"') { return handle_string(); }

        if (pos < source.length() && source[pos] == '\'') { return handle_char(); }

        if (pos < source.length() && numeric_set.contains(source[pos])) { return handle_numeric(); }

        if (pos < source.length() && delimiter_map.contains(source[pos])) { return handle_delimiter(); }

        if (pos + 1 < source.length() && operator_map_double.contains(std::string() + source[pos] + source[pos + 1])) {
            return handle_operator(true);
        }

        if (pos < source.length() && operator_map_single.contains(source[pos])) { return handle_operator(false); }

        if (pos < source.length() && alnumeric_set.contains(source[pos])) { return handle_symbol(); }
        ...
        while (token.tt != end_of_file_tt) {
            if (token.tt != unkown_tt) { // comments and whitespaces are just unknown
                if (token.tt == add_operator_tt || token.tt == subtract_operator_tt) {
                    ...
                    if (token.tt == unary_plus_operator_tt) { token.lexeme = "unary_plus"; }
                    else if (
                        token.tt == unary_minus_operator_tt) { token.lexeme = "unary_minus"; }
                }
