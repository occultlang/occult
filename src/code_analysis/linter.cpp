...
bool linter::declare_var(const std::string &name, const std::string &type) {
  ...
  top[name] = type;
  if (debug) {
    std::cout << CYAN << "[LINTER] Declared '" << name << "' : " << type
              << RESET << "\n";
  }
  return true;
}
...
bool linter::types_compatible(const std::string &a,
  ...
  static const std::unordered_set<std::string> all_numeric = {
      "int8",   "int16",  "int32",   "int64",   "uint8", "uint16",
      "uint32", "uint64", "float32", "float64", "bool"};
  static const std::unordered_set<std::string> float_types = {"float32",
                                                              "float64"};
  static const std::unordered_set<std::string> int_types = {
  ...
  auto ends_with_ptr = [](const std::string &s) {
    ...
    case cst_type::bitwise_rshift: {
      if (stk.size() >= 2) {
        ...
        if (rhs_zero && (node->get_type() == cst_type::division_operator ||
          ...
          errors.emplace_back("Division by zero",
                              lint_error::severity::warning);
        }
    ...
    case cst_type::less_than_or_equal_operator: {
      if (stk.size() >= 2) {
        ...
        if (!types_compatible(lhs, rhs)) {
          errors.emplace_back("Type mismatch in comparison: cannot compare '" +
                              lhs + "' with '" + rhs + "'");
        }
      }
      stk.push_back("bool");
...
void linter::collect_functions() {
  ...
  static const char *builtins[] = {
      "print_string", "print_integer", "print_newline", "print_char",
      "alloc",        "del",           "__bitcast_f64", "__bitcast_i64",
  };
  ...
  for (const auto &c : root->get_children()) {
    ...
    if (c->get_type() == cst_type::moduledecl ||
        c->get_type() == cst_type::importdecl) {
      continue;
    }
    if (c->get_type() == cst_type::function) {
      ...
      for (const auto &child : c->get_children()) {
        ...
        if (is_type_node(child->get_type()) ||
            child->get_type() == cst_type::structure) {
...
void linter::lint_expr(cst *node) {
  ...
  case cst_type::functioncall: {
    if (!node->get_children().empty()) {
      const auto &first = node->get_children().front();
      if (first->get_type() == cst_type::identifier) {
        const bool is_method_style =
            !first->content.empty() && first->content.front() == '.';
        if (!is_method_style && !known_functions.count(first->content)) {
          ...
          for (std::size_t i = 1; i < node->get_children().size(); ++i) {
            if (node->get_children().at(i)->get_type() ==
                cst_type::functionargument) {
              actual_args++;
            }
          }
          ...
          if (pc_it != function_param_counts.end()) {
            ...
            if (expected == -1) {
              ...
              if (min_it != function_min_param_counts.end() &&
                  actual_args < min_it->second) {
                errors.emplace_back(
                    "Function '" + first->content + "' expects at least " +
                    std::to_string(min_it->second) + " arguments, but " +
                    std::to_string(actual_args) + " were provided");
              }
            } else if (actual_args != expected) {
              errors.emplace_back(
                  "Function '" + first->content + "' expects " +
    ...
    for (std::size_t i = 1; i < node->get_children().size(); ++i) {
      ...
      if (child->get_type() == cst_type::unary_minus_operator) {
        if (i + 1 < node->get_children().size() &&
            node->get_children().at(i + 1)->get_type() ==
                cst_type::number_literal) {
          errors.emplace_back("Negative array index is not allowed",
                              lint_error::severity::warning);
        }
      }
      lint_expr(child.get());
...
void linter::check_assignment_type(const std::string &decl_type,
                                   const std::string &rhs_type) {
  if (!types_compatible(decl_type, rhs_type)) {
    errors.emplace_back("Type mismatch: cannot assign '" + rhs_type +
                        "' to variable of type '" + decl_type + "'");
  }
}
...
void linter::lint_block(cst_block *block_node) {
  ...
  for (const auto &c : block_node->get_children()) {
      ...
      if (!c->get_children().empty()) {
        ...
        if (id_node->get_type() == cst_type::identifier) {
          ...
          if (c->is_const && c->get_children().size() < 2) {
            errors.emplace_back("const variable '" + id_node->content +
        ...
        if (c->get_children().size() >= 2) {
          ...
          if (assign->get_type() == cst_type::assignment &&
              !assign->get_children().empty()) {
            check_assignment_type(
                tname, infer_type_rpn(to_raw(assign->get_children())));
          }
        }
      }
      break;
    ...
    case cst_type::ifstmt: {
      ...
      for (const auto &child : c->get_children()) {
        if (child->get_type() != cst_type::block &&
            child->get_type() != cst_type::elsestmt &&
            child->get_type() != cst_type::elseifstmt) {
      ...
      // check for bare condition without comparison operator
      {
        ...
        for (auto *n : cond) {
          auto t = n->get_type();
          if (t == cst_type::equals_operator ||
              t == cst_type::not_equals_operator ||
              t == cst_type::greater_than_operator ||
              t == cst_type::less_than_operator ||
              t == cst_type::greater_than_or_equal_operator ||
        ...
        if (!has_comparison && !cond.empty()) {
          errors.emplace_back("Conditions require an explicit binary "
                              "comparison (==, !=, <, >, <=, >=)",
                              lint_error::severity::warning);
        }
      }
      ...
      if (!c->get_children().empty()) {
        ...
        if (is_type_node(init_node->get_type())) {
          ...
          if (init_node->get_type() == cst_type::float32_datatype ||
            ...
            errors.emplace_back(
                "Float types are not allowed as for-loop counter variables",
                lint_error::severity::error);
          }
          ...
          if (!init_node->get_children().empty()) {
            ...
            if (id_node->get_type() == cst_type::identifier) {
              ...
              if (init_node->get_children().size() >= 2) {
                ...
                if (init_assign->get_type() == cst_type::assignment &&
                    !init_assign->get_children().empty()) {
                  check_assignment_type(
                      itype,
                      infer_type_rpn(to_raw(init_assign->get_children())));
                }
              }
            }
