...
struct lint_error {
  ...
  std::string message;
  lint_error(std::string msg, severity s = severity::error)
      : level(s), message(std::move(msg)) {}
};
...
class linter {
  ...
  std::unordered_map<std::string, int>
      function_param_counts; // -1 = variadic minimum count stored separately
  std::unordered_map<std::string, int>
      function_min_param_counts; // minimum required args for variadic functions
  ...
  void check_assignment_type(const std::string &decl_type,
                             const std::string &rhs_type);

public:
  explicit linter(cst_root *root, bool debug = false)
