...
namespace occult {
    ...
    inline void vm::pop() {
        if (stack.empty()) { throw std::runtime_error("Attempted to pop from an empty stack."); }
        stack.pop_back();
    }
    ...
    inline void vm::store(int addr) {
        if (stack.empty()) { throw std::runtime_error("stack underflow in STORE opcode"); }
        if (addr < 0 || addr >= memory.size()) { throw std::runtime_error("fuck"); }
    ...
    inline void vm::load(int addr) {
        if (addr < 0 || addr >= memory.size()) { throw std::runtime_error("invalid memory address in LOAD opcode"); }
        vm::push(memory[addr]);
    }
    ...
    inline void vm::jmp(int addr) {
        if (addr > 0 || addr >= bytecode.size()) { throw std::runtime_error("invalid jmp address"); }
        program_counter = addr;
    }

    inline void vm::jz(int addr) {
        if (stack.empty()) { throw std::runtime_error("stack underflow in jz opcode"); }
        int value = stack.back();
        vm::pop();
        if (value == 0) {
            if (addr > 0 || addr >= bytecode.size()) { throw std::runtime_error("invalid jmp address in jz"); }
            program_counter = addr;
        }
    }
    ...
    inline void vm::jl(int addr) {
        if (stack.empty()) { throw std::runtime_error("Stack underflow in JGE opcode"); }
        int value = stack.back();
    ...
    inline void vm::cmp() {
        if (stack.size() < 2) { throw std::runtime_error("stack underflow in CMP"); }
    ...
    inline void vm::ret() {
        if (call_stack.empty()) { throw std::runtime_error("call underflow in ret opcode"); }
