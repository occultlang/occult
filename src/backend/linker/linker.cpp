...
void linker::link_and_create_binary(
  ...
  if (debug) {
    std::cout << BLUE << "[LINKER INFO] Base address: " << RED << "0x"
              << std::hex << base_addr << std::dec << RESET << std::endl;
    std::cout << BLUE << "[LINKER INFO] Entry address: " << RED << "0x"
              << std::hex << entry_addr << std::dec << RESET << std::endl;
  }
  ...
  for (std::size_t i = 0; i < final_code.size() - 13; i++) {
    ...
    if (current_byte == 0x48 && (next_byte == 0xb8 || next_byte == 0xbb) &&
      ...
      for (std::size_t j = 2; j <= 9; j++) {
        address |= static_cast<std::uint64_t>(final_code.at(i + j))
                   << ((j - 2) * 8);
      }

      if (debug) {
        std::cout << BLUE << "[LINKER INFO] Address to reloc: " << RED << "0x"
                  << std::hex << address << std::dec << RESET << "\n";
      ...
      std::uint64_t new_address =
          base_addr + locations[func_addr_map[address]] + sizeof(elf_header) +
          sizeof(elf_program_header);

      if (debug)
        std::cout << BLUE << "[LINKER INFO] New address: " << RED << "0x"
                  << std::hex << new_address << std::dec << RESET << "\n";
