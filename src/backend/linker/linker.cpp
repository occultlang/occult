...
void linker::link_and_create_binary(
    ...
    for (const auto& m : function_raw_code_map) {
        ...
        if (debug)
            std::cout << BLUE << "[LINKER INFO] Added function: " << YELLOW << "\"" << m.first
                      << "\"" << RESET << std::endl;
    ...
    std::uint64_t entry_addr =
        base_addr + locations["main"] + sizeof(elf_header) + sizeof(elf_program_header);

    if (debug) {
        std::cout << BLUE << "[LINKER INFO] Base address: " << RED << "0x" << std::hex << base_addr
                  << std::dec << RESET << std::endl;
        std::cout << BLUE << "[LINKER INFO] Entry address: " << RED << "0x" << std::hex
                  << entry_addr << std::dec << RESET << std::endl;
    }
    ...
    for (std::size_t i = 0; i < final_code.size() - 13; i++) {
        ...
        if (current_byte == 0x48 && (next_byte == 0xb8 || next_byte == 0xbb) &&
            second_last_byte == 0xff && (last_byte == 0x10 || last_byte == 0x13)) {
            if (debug) {
                ...
                for (std::size_t j = 0; j <= 12; j++) {
                    std::cout << std::setw(2) << std::setfill('0') << std::uppercase << std::hex
                              << static_cast<int>(final_code.at(i + j)) << " ";
                }
            ...
            if (debug) {
                std::cout << BLUE << "[LINKER INFO] Address to reloc: " << RED << "0x" << std::hex
                          << address << std::dec << RESET << "\n";
                std::cout << BLUE << "[LINKER INFO] Resolving: " << YELLOW << "\""
                          << func_addr_map[address] << "\"\n"
                          << RESET;
            }

            // Calculate the new resolved address based on the symbol location
            std::uint64_t new_address = base_addr + locations[func_addr_map[address]] +
                                        sizeof(elf_header) + sizeof(elf_program_header);

            if (debug)
                std::cout << BLUE << "[LINKER INFO] New address: " << RED << "0x" << std::hex
                          << new_address << std::dec << RESET << "\n";

            // Replace the address bytes with the new relative offset
            for (std::size_t j = 0; j < sizeof(new_address); ++j) {
                final_code.at(i + 2 + j) =
                    static_cast<std::uint8_t>((new_address >> (j * 8)) & 0xFF);
            }

            if (last_byte ==
                0x10) { // fix calls to not call to memory, but rather an absolute address
    ...
    if (showtime) {
        std::cout << GREEN << "[OCCULTC] Completed linking functions \033[0m" << duration.count()
                  << "ms\n";
    }
