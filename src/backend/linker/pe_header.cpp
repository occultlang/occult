...
void generate_pe_header() {
  ...
  nt_headers.OptionalHeader.SizeOfCode =
      0x200; // 512 byte block of memory | TODO: change later on obviously
  ...
  nt_headers.OptionalHeader.BaseOfData =
      0x2000; // not typically used, next alignment

  nt_headers.OptionalHeader.ImageBase =
      0x00400000; // typical default image base
  ...
  nt_headers.OptionalHeader.Subsystem =
      IMAGE_SUBSYSTEM_WINDOWS_CUI; // could change it to gui
  ...
  section_header.VirtualAddress =
      0x1000; // in the loaded process .text is at ImageBase + 0x1000
  section_header.SizeOfRawData =
      0x200; // must be multiple of FileAlignment (0x200), we'll just use 0x200
  ...
  // Section Header
  file.write(reinterpret_cast<const char *>(&section_header),
