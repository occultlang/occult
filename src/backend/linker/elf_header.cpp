...
elf_program_header elf::generate_program_header(std::uint64_t program_size, std::uint64_t vaddr,
    ...
    program_header.p_filesz =
        program_size;                // File size, TODO: add argument for file size in the future
...
void elf::generate_binary(const std::string& binary_name, const std::vector<std::uint8_t>& code,
                          std::uint64_t program_size, std::uint64_t entry_addr, std::uint64_t vaddr,
                          std::uint64_t paddr, std::uint64_t memsz) {
