...
void elf::generate_binary(const std::string &binary_name,
                          ...
                          std::uint64_t program_size, std::uint64_t entry_addr,
  ...
  elf_program_header program_header =
      generate_program_header(program_size, vaddr, paddr, memsz);

  file.write(reinterpret_cast<const char *>(&elf_header), sizeof(elf_header));
  file.write(reinterpret_cast<const char *>(&program_header),
