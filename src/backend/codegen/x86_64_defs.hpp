...
static std::uint64_t double_to_bits(double d) {
    return std::bit_cast<std::uint64_t>(d);
}

// rebases register to the correct size
static grp rebase_register(const grp& reg) {
    return static_cast<grp>(static_cast<std::uint8_t>(reg) & 0b111);
}
...
struct modrm {
    ...
    explicit modrm(const mod_field& mod, const simd128& reg, const simd128& rm) : mod(mod), reg(static_cast<grp>(reg)), rm(static_cast<rm_field>(rm)) {
    } // hate simd
