...
namespace occult::x86_64 {
    enum rex_bits : std::uint8_t { none = 0, b = 1 << 0, x = 1 << 1, r = 1 << 2, w = 1 << 3 };

    /*
        to clarify:

        w is the 64-bit operand size (extended register size for 64 bit) rAX to rSP
        r is extending the reg field in the ModR/M byte (+8 to register number i.e
       r8-15) e.g add r9, rax (rax is base) x is extending the index field in the
       SIB byte (+8 to index register) b is extending the rm or base field in ModR/M
       or SIB byte (+8 to the base register)
    */
    enum rex : std::uint8_t {
        ...
        rex_rxb = rex | r | x | b,     // access to extended modrm reg, extended SIB index
                                       // and extended base register
        ...
        rex_wrxb = rex | w | r | x | b // 64-bit operand size, extended modrm reg, extended
                                       // SIB index and extended base register
    };
    ...
    constexpr std::uint8_t kREP_Prefix = 0xF3;
    constexpr std::uint8_t kREPNE_Prefix = 0xF2;

    // rm field encodes the low 3 bits of the register/memory operand
    enum class rm_field : std::uint8_t { // low 3 bits of reg (ModRM.rm)
        rax = 0b000,
        rcx = 0b001,
        rdx = 0b010,
        rbx = 0b011,
        rsp = 0b100, // also triggers SIB encoding
        rbp = 0b101, // rip-relative when mod == 00
        rsi = 0b110,
        rdi = 0b111,
        indexed = rsp,
        rip_relative = rbp
    ...
    // simd registers (already rebased)
    enum simd128 : std::uint8_t { xmm0, xmm1, xmm2, xmm3, xmm4, xmm5, xmm6, xmm7, xmm8, xmm9, xmm10, xmm11, xmm12, xmm13, xmm14, xmm15 };
    ...
    static grp as_8(const grp& reg) {
        ...
        throw std::runtime_error("Can only convert 64-bit register to 8-bit.");
    }

    static grp as_16(const grp& reg) {
        if ((reg >= rax && reg <= rdi) || (reg >= r8 && reg <= r15)) {
            switch (reg) {
            case rax:
                return ax;
        ...
        throw std::runtime_error("Can only convert 64-bit register to 16-bit.");
    }

    static std::uint32_t float_to_bits(float f) { return std::bit_cast<std::uint32_t>(f); }

    static std::uint64_t double_to_bits(double d) { return std::bit_cast<std::uint64_t>(d); }

    // rebases register to the correct size
    static grp rebase_register(const grp& reg) { return static_cast<grp>(static_cast<std::uint8_t>(reg) & 0b111); }

    static constexpr std::string reg_to_string(grp r) {
        switch (r) {
        case grp::rax:
            return "rax";
        case grp::rcx:
    ...
    static constexpr std::string simd_to_string(simd128 r) {
        switch (r) {
        case simd128::xmm0:
            return "xmm0";
        case simd128::xmm1:
    ...
    struct modrm {
        ...
        explicit modrm(const mod_field& mod, const grp& rm, const grp& reg) : mod(mod), reg(reg), rm(static_cast<rm_field>(rm)) {}

        explicit modrm(const mod_field& mod, const grp& rm, const rm_field& reg) : mod(mod), reg(static_cast<grp>(reg)), rm(static_cast<rm_field>(rm)) {}

        explicit modrm(const mod_field& mod, const rm_field& rm, const rm_field& reg) : mod(mod), reg(static_cast<grp>(reg)), rm(static_cast<rm_field>(rm)) {}

        explicit modrm(const mod_field& mod, const simd128& reg, const simd128& rm) : mod(mod), reg(static_cast<grp>(reg)), rm(static_cast<rm_field>(rm)) {} // hate simd

        explicit modrm(const mod_field& mod, const simd128& reg, const grp& rm) : mod(mod), reg(static_cast<grp>(reg)), rm(static_cast<rm_field>(rm)) {}

        explicit modrm(const mod_field& mod, const grp& reg, const simd128& rm) : mod(mod), reg(reg), rm(static_cast<rm_field>(rm)) {}

        explicit modrm(const mod_field& mod, const rm_field& rm_mode, const simd128& reg) : mod(mod), reg(static_cast<grp>(reg)), rm(rm_mode) {}

        operator std::uint8_t() const {
            const std::uint8_t mod_bits = static_cast<std::uint8_t>(mod) & 0x3;
            const std::uint8_t reg_bits = static_cast<std::uint8_t>(reg) & 0x7;
            const std::uint8_t rm_bits = static_cast<std::uint8_t>(rm) & 0x7;
    ...
    struct sib {
        ...
        explicit sib(const std::uint8_t& scale, const grp& index, const grp& base) : scale(scale), index(index), base(base) {}

        explicit sib(const std::uint8_t& scale, const std::uint8_t& index, const grp& base) : scale(scale), index(static_cast<grp>(index)), base(base) {}

        explicit sib(const std::uint8_t& scale, const std::uint8_t& index, const std::uint8_t& base) : scale(scale), index(static_cast<grp>(index)), base(static_cast<grp>(base)) {}

        operator std::uint8_t() const {
            // Mask base/index down to the low 3 bits; high bits are carried by
            // REX.X/REX.B
            const std::uint8_t idx = static_cast<std::uint8_t>(rebase_register(index));
            const std::uint8_t bas = static_cast<std::uint8_t>(rebase_register(base));
            return static_cast<std::uint8_t>((scale << 6) | (idx << 3) | bas);
        }
    };
    ...
    */
    ...
    enum opcode : std::uint8_t {
        ...
        DECLARE_OPCODE(IMUL_r16_to_64_rm16_to_64_imm16_to_32,
                       0x69)            // IMUL r16_to_64, r16_to_64, imm16_to_32
        DECLARE_OPCODE(PUSH_imm8, 0x6A) // PUSH imm8
        DECLARE_OPCODE(IMUL_r16_to_64_rm16_to_64_imm8,
                       0x6B) // IMUL r16_to_64, r16_to_64, imm8
        ...
        DECLARE_OPCODE(ADD_rm8_to_64_imm16_or_32,
                       0x81)                           // ADD r/m16/32/64, imm16_or_32
        DECLARE_OPCODE(OR_rm8_to_64_imm16_or_32, 0x81) // OR r/m16/32/64, imm16_or_32
        DECLARE_OPCODE(ADC_rm8_to_64_imm16_or_32,
                       0x81) // ADC r/m16/32/64, imm16_or_32
        DECLARE_OPCODE(SBB_rm8_to_64_imm16_or_32,
                       0x81) // SBB r/m16/32/64, imm16_or_32
        DECLARE_OPCODE(AND_rm8_to_64_imm16_or_32,
                       0x81) // AND r/m16/32/64, imm16_or_32
        DECLARE_OPCODE(SUB_rm8_to_64_imm16_or_32,
                       0x81) // SUB r/m16/32/64, imm16_or_32
        DECLARE_OPCODE(XOR_rm8_to_64_imm16_or_32,
                       0x81) // XOR r/m16/32/64, imm16_or_32
        DECLARE_OPCODE(CMP_rm8_to_64_imm16_or_32,
                       0x81) // CMP r/m16/32/64, imm16_or_32
        ...
        DECLARE_OPCODE(MOV_r16_to_64_imm16_to_64,
                       0xB8) // MOV r/m16_to_64, imm16_to_64
        ...
        DECLARE_OPCODE(MOV_rm16_to_64_imm16_or_32,
                       0xC7) // MOV r/m16_to_64, imm16_or_32
        ...
        DECLARE_OPCODE(TEST_rm16_to_64_imm16_or_32,
                       0xF7)                          // TEST r/m16_to_64, imm16_or_32
        ...
        DECLARE_OPCODE(CALL_rel32, 0xE8) DECLARE_OPCODE(JMP_rel8, 0xEB) DECLARE_OPCODE(JMP_rel32, 0xE9)
    };
    ...
    // - packed single FP values don't need any prefix (only the k2ByteOpcodePrefix)
    // - packed double FP values need 0x66 (operand size override) prior to 0x0F
    // - scalar packed single FP values need 0xF3 (kREP_Prefix / REP prefix)
    // - scalar packed double FP values need 0xF2 (kREPNE_Prefix / REPNE prefix)
    enum float_opcode_2b : std::uint8_t {
        DECLARE_OPCODE(ANDPS_xmm_to_xmm_or_m128,
                       0x54) // Bitwise Logical AND of Packed Single-FP Values, xmm xmm/m128
        DECLARE_OPCODE(ANDPD_xmm_to_xmm_or_m128,
                       0x54) // Bitwise Logical AND of Packed Single-FP Values, xmm xmm/m128
        DECLARE_OPCODE(ANDNPS_xmm_to_xmm_or_m128,
                       0x55) // Bitwise Logical AND NOT of Packed Single-FP Values, xmm xmm/m128
        DECLARE_OPCODE(ANDNPD_xmm_to_xmm_or_m128,
                       0x55)                                                                         // Bitwise Logical AND NOT of Packed Single-FP Values, xmm xmm/m128
        DECLARE_OPCODE(ORPS_xmm_to_xmm_or_m128, 0x56)                                                // bitwise or fp values
        DECLARE_OPCODE(ORPD_xmm_to_xmm_or_m128, 0x56) DECLARE_OPCODE(XORPS_xmm_to_xmm_or_m128, 0x57) // bitwise xor fp values
        DECLARE_OPCODE(XORPD_xmm_to_xmm_or_m128, 0x57)

            DECLARE_OPCODE(ADDPS_xmm_to_xmm_or_m128, 0x58) // add fp values
        DECLARE_OPCODE(ADDSS_xmm_to_xmm_or_m32, 0x58)      // requires kREP_Prefix
        DECLARE_OPCODE(ADDPD_xmm_to_xmm_or_m128,
                       0x58)                          // requires operand size override
        DECLARE_OPCODE(ADDSD_xmm_to_xmm_or_m64, 0x58) // requires kREPNE_Prefix

        DECLARE_OPCODE(MULPS_xmm_to_xmm_or_m128, 0x59) // mul fp values
        DECLARE_OPCODE(MULSS_xmm_to_xmm_or_m32, 0x59)  // requires kREP_Prefix
        DECLARE_OPCODE(MULPD_xmm_to_xmm_or_m128,
                       0x59)                          // requires operand size override
        DECLARE_OPCODE(MULSD_xmm_to_xmm_or_m64, 0x59) // requires kREPNE_Prefix

        DECLARE_OPCODE(SUBPS_xmm_to_xmm_or_m128, 0x5C) // sub fp values
        DECLARE_OPCODE(SUBSS_xmm_to_xmm_or_m32, 0x5C)  // requires kREP_Prefix
        DECLARE_OPCODE(SUBPD_xmm_to_xmm_or_m128,
                       0x5C)                          // requires operand size override
        DECLARE_OPCODE(SUBSD_xmm_to_xmm_or_m64, 0x5C) // requires kREPNE_Prefix

        DECLARE_OPCODE(DIVPS_xmm_to_xmm_or_m128, 0x5E) // div fp values
        DECLARE_OPCODE(DIVSS_xmm_to_xmm_or_m32, 0x5E)  // requires kREP_Prefix
        DECLARE_OPCODE(DIVPD_xmm_to_xmm_or_m128,
                       0x5E)                          // requires operand size override
        DECLARE_OPCODE(DIVSD_xmm_to_xmm_or_m64, 0x5E) // requires kREPNE_Prefix

        DECLARE_OPCODE(MOVD_xmm_to_rm32_to_64, 0x6E) // requires operand size override
        DECLARE_OPCODE(MOVQ_xmm_to_rm32_to_64,
                       0x6E) // requires operand size override (for 64 bits also
                             // requires REX byte)
        // ^^ btw this is better than the mmx one, this one is SSE2

        DECLARE_OPCODE(MOVQ_rm64_from_xmm,
                       0x7E) // 66 REX.W 0F 7E /r: move xmm bits to GP register
        ...
        DECLARE_OPCODE(MOVSD_xmm_or_m64_to_xmm, 0x10) // REPNE

        DECLARE_OPCODE(CVTTSD2SI_xmm_or_m64_to_r32_to_64, 0x2C) DECLARE_OPCODE(CVTSI2SD_rm32_to_64_to_xmm, 0x2A)

            DECLARE_OPCODE(CVTSS2SD_xmm_or_m32_to_xmm,
                           0x5A) // REP: scalar float32 -> float64
        DECLARE_OPCODE(CVTSD2SS_xmm_or_m64_to_xmm,
                       0x5A) // REPNE: scalar float64 -> float32
