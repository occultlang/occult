...
/*
 * This IR generation is really crappy and disorganized, I will make it organized later on, and I'll
 * probably move to SSA or TAC later on as well, but this is what I can do for now, as I want to
 * make actual progress on the language, but we can still do some stack optimisations we can use
 * this paper (http://www.rigwit.co.uk/thesis/chap-5.pdf) as an example, and other theories too.
 */
...
void ir_gen::generate_function_args(ir_function& function, cst_functionargs* func_args_node) {
    for (const auto& arg : func_args_node->get_children()) {
        ...
        for (const auto& c : arg->get_children()) {
            ...
            if (debug) {
                std::cout << CYAN << "[IR GEN] Arg (type, name): " << RESET << "(" << type << ", "
...
void ir_gen::generate_common_generic(ir_function& function, cst* node,
                                     std::optional<std::string> type, type_of_push type_push) {
    bool needs_value = false;
    for (const auto& c : node->get_children()) {
        if (c->get_type() == cst_type::equals_operator ||
            c->get_type() == cst_type::not_equals_operator ||
            c->get_type() == cst_type::and_operator || c->get_type() == cst_type::or_operator ||
    ...
    for (const auto& c : node->get_children()) {
        ...
        case cst_type::dereference:
            ...
            break;
        default:
            generate_arith_and_bitwise_operators(function, c.get(), type.value());
            break;
        }
    }
}
...
void ir_gen::generate_function_call(ir_function& function, cst* c) {
    ...
    const auto identifier =
        cst::cast_raw<cst_identifier>(node->get_children().front().get()); // name of call

    const auto func_it = func_map.find(identifier->content);
    if (func_it == func_map.end()) {
        if (debug) {
            std::cout << CYAN << "[IR GEN] External/built-in function call: " << identifier->content
                      << RESET << std::endl;
        }
        ...
        while (node->get_children().at(arg_location).get()->content != "end_call") {
            const auto arg_node =
                cst::cast_raw<cst_functionarg>(node->get_children().at(arg_location).get());
    ...
    if (debug) {
        std::cout << CYAN << "[IR GEN] Args for call " << identifier->content << ": " << RESET
                  << func_it->second.args.size() << std::endl;
    }
    ...
    while (node->get_children().at(arg_location).get()->content != "end_call") {
        ...
        if (debug) {
            std::cout << CYAN << "[IR GEN] Generating argument for call (type, loc): " << RESET
                      << "(" << argument_type << ", " << arg_location - 1 << ")" << std::endl;
        }
...
void ir_gen::generate_condition(ir_function& function, cst* node, const std::string& false_label,
    ...
    for (std::size_t i = 0; i < node->get_children().size(); i++) {
        ...
        case cst_type::expr_start: {
            ...
            if (i > 0 &&
                node->get_children().at(i - 1)->get_type() == cst_type::unary_not_operator) {
                std::size_t not_pos = i;
                std::size_t not_count = 0;

                while (not_pos > 0 && node->get_children().at(not_pos - 1)->get_type() ==
    ...
    if (debug && !logic_stack.empty()) {
        ...
        std::cout << BLUE << "[IR GEN] Logic stack size: " << RESET << logic_stack.size()
                  << std::endl;
    }
    ...
    if (debug && !eval_stack.empty()) {
        ...
        std::cout << BLUE << "[IR GEN] Eval stack size: " << RESET << eval_stack.size()
                  << std::endl;
    }
    ...
    if (debug && !eval_stack_m.empty()) {
        ...
        std::cout << BLUE << "[IR GEN] Eval stack (m) size: " << RESET << eval_stack.size()
                  << std::endl;
    }
...
void ir_gen::generate_if(ir_function& function, cst_ifstmt* if_node,
    ...
    for (const auto& c : if_node->get_children()) {
        if (c->get_type() == cst_type::block) {
            generate_block(function, cst::cast_raw<cst_block>(c.get()), current_break_label,
                           current_loop_start);
            break;
        }
    }
        ...
        if (elseif_node->get_type() == cst_type::elseifstmt) {
            ...
            for (const auto& b : elseif_node->get_children()) {
                if (b->get_type() == cst_type::block) {
                    generate_block(function, cst::cast_raw<cst_block>(b.get()), current_break_label,
                                   current_loop_start);
                    break;
                }
            }
...
void ir_gen::generate_for(ir_function& function, cst_forstmt* for_node) {
    ...
    case cst_type::int64_datatype: {
        const auto identifier =
            cst::cast_raw<cst_identifier>(first_node->get_children().front().get()); // name
...
void ir_gen::generate_array_decl(ir_function& function, cst_array* array_node) {
    ...
    if (debug) {
        std::cout << CYAN << "[IR GEN] Array type: " << RESET
                  << type->to_string().substr(4, type->to_string().size()) << std::endl;
    ...
    function.code.emplace_back(op_decl_array_type,
                               type->to_string().substr(4, type->to_string().size()));

    auto dimensions_count_node =
        cst::cast_raw<cst_dimensions_count>(array_node->get_children().at(1).get());

    if (debug) {
        std::cout << CYAN << "[IR GEN] Dimensions: " << RESET << dimensions_count_node->content
                  << std::endl;
    }
    ...
    if (dimensions.size() == 1) {
        ...
        for (std::size_t i = 0; i < dimensions.front(); ++i) {
            ...
            auto element =
                cst::cast_raw<cst_arrayelement>(first_array_node->get_children().at(i).get());
...
void ir_gen::generate_array_access(ir_function& function, cst_arrayaccess* array_access_node) {
    if (array_access_node->get_children().front()->get_type() == cst_type::identifier) {
        ...
        auto identifier =
            cst::cast_raw<cst_identifier>(array_access_node->get_children().front().get());
        auto has_assigment =
            array_access_node->get_children().back()->get_type() == cst_type::assignment;

        for (std::size_t i = 1;
             i < array_access_node->get_children().size() - (has_assigment) ? 1 : 0; i++) {
        ...
        if (array_access_node->get_children().back()->get_type() == cst_type::assignment) {
            auto assignment =
                cst::cast_raw<cst_assignment>(array_access_node->get_children().back().get());
...
void ir_gen::generate_member_access(ir_function& function, cst_memberaccess* member_access_node) {
    ...
    if (debug) {
        std::cout << CYAN << "[IR GEN] Member " << (is_assignment ? "store: " : "access: ") << RESET
                  << base_var;
...
void ir_gen::generate_block(ir_function& function, cst_block* block_node,
                            std::string current_break_label, std::string current_loop_start) {
    for (const auto& c : block_node->get_children()) {
        ...
        case cst_type::string_datatype: {
            ...
            const auto identifier =
                cst::cast_raw<cst_identifier>(node->get_children().front().get()); // name
            const auto assignment =
                cst::cast_raw<cst_assignment>(node->get_children().back().get());

            generate_common(function, assignment, type_with_ptrs); // expression stuff
        ...
        case cst_type::identifier: {
            if (debug) {
                std::cout << CYAN
                          << "[IR GEN] Loading: " << local_variable_map[function][c->content]
                          << std::endl;
            }
        ...
        case cst_type::ifstmt: {
            generate_if(function, cst::cast_raw<cst_ifstmt>(c.get()), current_break_label,
                        current_loop_start);
...
void ir_gen::visualize_stack_ir(const std::vector<ir_function>& funcs) {
    ...
    for (const auto& [code, args, name, type, uses_shellcode] : funcs) {
        std::cout << "Type: " << type << "\n";
                        ...
                        if (ir_s.datatype != c->content) {
                            ...
                            if (c->num_pointers > 0) {
                                ...
                                throw std::runtime_error(
                                    "[IR ERR] Struct can't have nested type without ptr (sz = " +
