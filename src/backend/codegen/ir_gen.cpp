...
namespace occult {
    ...
    void ir_gen::generate_condition(ir_function &function, cst *node, const std::string &false_label,
                ...
                case cst_type::expr_start: {
                    ...
                    if (i > 0 && node->get_children().at(i - 1)->get_type() == cst_type::unary_not_operator) {
                        ...
                        for (int j = static_cast<int>(curr_pos); j >= 0; j--) {
                            ...
                            if (is_logical(curr_node.get())) { curr_node->do_not = apply_not; }
                        }
                    }
        ...
        if (!logic_stack.empty()) { // if condition with logical operations (2+)
                ...
                switch (c->get_type()) {
                    case cst_type::or_operator: {
                        if (eval_index < eval_stack_m.size()) {
                            ...
                            if (expr.back()->do_not) { generate_and_jump(function, expr.back(), true_label); }
                            else { generate_or_jump(function, expr.back(), true_label); }
                    ...
                    case cst_type::and_operator: {
                        if (eval_index < eval_stack_m.size()) {
                            ...
                            if (expr.back()->do_not) { generate_or_jump(function, expr.back(), false_label); }
                            else { generate_and_jump(function, expr.back(), false_label); }
            ...
            while (eval_index < eval_stack_m.size()) {
                ...
                if (expr.back()->do_not) { generate_inverted_jump(function, expr.back(), false_label); }
                else { generate_normal_jump(function, expr.back(), false_label); }
    ...
    void ir_gen::generate_array_access(ir_function &function, cst_arrayaccess *array_access_node) {
        if (array_access_node->get_children().front()->get_type() == cst_type::identifier) {
            ...
            else { function.code.emplace_back(op_array_access_element, identifier->content); }
        }
            ...
            else { function.code.emplace_back(op_array_access_element, array_name->content); }
        }
    }
    ...
    void ir_gen::generate_block(ir_function &function, cst_block *block_node, std::string current_break_label,
        ...
        for (const auto &c: block_node->get_children()) {
            switch (c->get_type()) {
                ...
                case cst_type::float64_datatype: {
                    ...
                    function.code.emplace_back(op_storef, identifier->content,
                                               c->to_string().substr(4, c->to_string().size()));
            ...
            for (std::size_t i = 0; i < func.code.size(); i++) {
                switch (func.code.at(i).op) {
                    ...
                    case ir_opcode::op_negate: {
                        if (func.code.at(i + 1).op == ir_opcode::op_load || func.code.at(i + 1).op ==
                            ir_opcode::op_push) { std::iter_swap(func.code.begin() + i, func.code.begin() + i + 1); }
    ...
    std::vector<ir_struct> ir_gen::lower_structs() {
        ...
        for (auto &[name, child]: custom_type_map) { structs.emplace_back(handle_struct(name, child)); }
