    ...
    case cst_type::asm_code: {
      for (auto &str_literal : c->get_children()) {
        if (str_literal->get_type() == cst_type::stringliteral) {
          ...
          std::cout << RED
                    << "[IR ERR] Expected string literal in assembly block.\n"
                    << RESET;
...
void ir_gen::generate_function_args(ir_function &function,
  ...
  for (const auto &arg : func_args_node->get_children()) {
    if (arg->get_type() == cst_type::variadic) {
      ...
      if (debug) {
        std::cout << CYAN << "[IR GEN] Function is variadic" << RESET
                  << std::endl;
      }
      continue;
    }
...
void ir_gen::generate_arith_and_bitwise_operators(
  ...
  case cst_type::multiply_operator: {
    if (type.has_value() && type.value() == "float32") {
      ...
      ir_opcode to_push =
          (type.has_value() && is_signed[type.value()]) ? op_imul : op_mul;
  ...
  case cst_type::division_operator: {
    if (type.has_value() && type.value() == "float32") {
      ...
      ir_opcode to_push =
          (type.has_value() && is_signed[type.value()]) ? op_idiv : op_div;
      function.code.emplace_back(to_push);
    }
  ...
  case cst_type::modulo_operator: {
    if (type.has_value() && type.value() == "float32") {
      ...
      ir_opcode to_push =
          (type.has_value() && is_signed[type.value()]) ? op_imod : op_mod;
      function.code.emplace_back(to_push);
    }
  ...
  case cst_type::bitwise_rshift: {
    ir_opcode to_push = (type.has_value() && is_signed.count(type.value()) &&
...
void ir_gen::generate_boolean_value(ir_function &function, cst *node,
  ...
  for (const auto &c : node->get_children()) {
    ...
    if (c->get_type() == cst_type::and_operator ||
        c->get_type() == cst_type::or_operator) {
      has_logical = true;

      if (debug) {
        std::cout << CYAN
                  << "[IR GEN] Generating boolean value for logical operator.\n"
                  << RESET;
      }
...
void ir_gen::generate_common_generic(ir_function &function, cst *node,
  ...
  for (const auto &c : node->get_children()) {
    ...
    case cst_type::float_literal:
      ...
      break;
    case cst_type::charliteral: {
      std::int64_t char_val =
          c->content.empty() ? 0
                             : static_cast<std::int64_t>(
                                   static_cast<unsigned char>(c->content[0]));
...
void ir_gen::generate_function_call(ir_function &function, cst *c) {
  ...
  if (!resolved_call_name.empty() && resolved_call_name.front() == '.' &&
      node->get_children().size() > 1) {
    const auto self_arg =
        cst::cast_raw<cst_functionarg>(node->get_children().at(1).get());
    if (self_arg && !self_arg->get_children().empty() &&
        self_arg->get_children().front()->get_type() == cst_type::identifier) {
      ...
      if (self_it != local_variable_map[function].end()) {
        ...
        if (func_map.contains(direct_candidate)) {
          ...
          for (const auto &[fname, _] : func_map) {
            if (fname.size() >= direct_candidate.size() &&
                fname.compare(fname.size() - direct_candidate.size(),
                              direct_candidate.size(), direct_candidate) == 0) {
  ...
  if (resolved_call_name == "__bitcast_f64") {
    ...
    function.code.emplace_back(op_bitcast, std::string("float64"),
                               std::string("int64"));
    return;
  }
  if (resolved_call_name == "__bitcast_i64") {
    ...
    function.code.emplace_back(op_bitcast, std::string("int64"),
                               std::string("float64"));
    return;
  }
  ...
  if (func_it == func_map.end()) {
    if (debug) {
      std::cout << CYAN << "[IR GEN] External/built-in function call: "
                << resolved_call_name << RESET << std::endl;
    }
  ...
  while (node->get_children().at(arg_location).get()->content != "end_call") {
    ...
    if (static_cast<std::size_t>(arg_location - 1) <
        func_it->second.args.size()) {
    ...
    if (func_it->second.is_variadic) {
      ...
      if (is_va_slot) {
              ...
              for (const auto &[ct_name, ct_node] : custom_type_map) {
                for (const auto &member : ct_node->get_children()) {
                  for (const auto &mid : member->get_children()) {
                    if (mid->get_type() == cst_type::identifier &&
                        mid->content == last_member) {
                      if (member->get_type() == cst_type::float32_datatype ||
                          member->get_type() == cst_type::float64_datatype) {
                        has_float_member_access = true;
                        float_member_type =
                            (member->get_type() == cst_type::float32_datatype)
                                ? "float32"
                                : "float64";
                      }
                    }
        ...
        if (has_float_literal) {
          ...
          // to int64
          std::string var_float_type =
              local_variable_map[function][float_var_name];
          generate_common(function, arg_node, var_float_type);
          if (var_float_type == "float32") {
            function.code.emplace_back(op_cast, std::string("f64"),
                                       std::string("float32"));
          }
          function.code.emplace_back(op_bitcast, std::string("int64"),
                                     std::string("float64"));
...
void ir_gen::generate_condition(ir_function &function, cst *node,
  ...
  auto handle_internal_push = [&, this](cst *expr) -> void {
    ...
    case cst_type::dereference: {
      function.code.emplace_back(
          op_dereference, from_numerical_string<std::int64_t>(expr->content));
      break;
    }
    default: {
  ...
  const bool is_float_cmp =
      expr_type.has_value() &&
      (expr_type.value() == "float32" || expr_type.value() == "float64");
...
void ir_gen::generate_for(ir_function &function, cst_forstmt *for_node) {
  ...
  case cst_type::int64_datatype: {
    ...
    // expression stuff
    if (assignment->get_children().front()->get_type() ==
        cst_type::cast_to_datatype) {
      const auto casting_datatype = cst::cast_raw<cst_cast_to_datatype>(
          assignment->get_children().front().get());
...
void ir_gen::generate_switch(ir_function &function, cst_switchstmt *switch_node,
                             const std::string &current_break_label,
                             const std::string &current_loop_start) {
  const std::string end_label = create_label();
  ...
  for (auto *expr_node : switch_expr_nodes) {
    ...
    case cst_type::number_literal:
      function.code.emplace_back(
          op_push, from_numerical_string<std::int64_t>(expr_node->content));
      break;
    case cst_type::charliteral: {
      std::int64_t char_val =
          expr_node->content.empty()
              ? 0
              : static_cast<std::int64_t>(
                    static_cast<unsigned char>(expr_node->content[0]));
      function.code.emplace_back(op_push, char_val);
      break;
    }
    case cst_type::functioncall:
    ...
    case cst_type::memberaccess:
      generate_member_access(function,
                             cst::cast_raw<cst_memberaccess>(expr_node));
      break;
    case cst_type::arrayaccess:
      generate_array_access(function,
                            cst::cast_raw<cst_arrayaccess>(expr_node));
      break;
    default:
  ...
  for (std::size_t i = 0; i < case_infos.size(); i++) {
    ...
    for (auto *cv : ci.val_nodes) {
      ...
      case cst_type::number_literal:
        function.code.emplace_back(
            op_push, from_numerical_string<std::int64_t>(cv->content));
        break;
      case cst_type::charliteral: {
        std::int64_t char_val =
            cv->content.empty()
                ? 0
                : static_cast<std::int64_t>(
                      static_cast<unsigned char>(cv->content[0]));
        function.code.emplace_back(op_push, char_val);
        break;
      }
      case cst_type::functioncall:
    ...
    if (ci.block) {
      ...
      generate_block(function, ci.block, current_break_label,
                     current_loop_start);
      function.code.emplace_back(op_jmp, end_label);
    } else {
  ...
  if (default_node) {
    ...
    for (const auto &dc : default_node->get_children()) {
      if (dc->get_type() == cst_type::block) {
        generate_block(function, cst::cast_raw<cst_block>(dc.get()),
                       current_break_label, current_loop_start);
      }
    }
  }
  ...
  if (auto it = local_variable_map.find(function);
      it != local_variable_map.end()) {
...
void ir_gen::generate_struct_decl(ir_function &function,
  ...
  if (custom_type_map.contains(struct_node->content) &&
      custom_type_map[struct_node->content]->get_type() ==
          cst_type::enumeration) {
    ...
    if (struct_node->get_children().size() > 1) {
      ...
      function.code.emplace_back(op_push, static_cast<std::int64_t>(0),
                                 std::string("int64"));
      function.code.emplace_back(op_store, id->content, std::string("int64"));
    }
    return;
...
void ir_gen::generate_member_access(ir_function &function,
  ...
  bool is_deref =
      member_access_node->num_pointers > 0 || children[0]->num_pointers > 0;
  ...
  if (var_it_for_access != local_variable_map[function].end()) {
    ...
    if (base_struct_type.ends_with("_ptr")) {
      base_struct_type =
          base_struct_type.substr(0, base_struct_type.size() - 4);
    }
    if (base_struct_type.ends_with("_reference")) {
      base_struct_type =
          base_struct_type.substr(0, base_struct_type.size() - 10);
    }
  }
  ...
  if (debug) {
    std::cout << CYAN << "[IR GEN] Member "
              << (is_assignment ? "store: " : "access: ")
              << (is_deref ? "(deref) " : "") << RESET << base_var;
      ...
      if (map_it != custom_type_map.end()) {
        ...
        for (size_t ci = 0; ci < member_chain.size(); ci++) {
          for (const auto &mc : current_struct->get_children()) {
            if (!mc->get_children().empty() &&
                mc->get_children().front()->content == member_chain[ci]) {
...
void ir_gen::generate_block(ir_function &function, cst_block *block_node,
  ...
  for (const auto &c : block_node->get_children()) {
    ...
    case cst_type::dereference: {
      ...
      if (assignment) {
        for (const auto &ac : assignment->get_children()) {
          ...
          case cst_type::arrayaccess:
            generate_array_access(function,
                                  cst::cast_raw<cst_arrayaccess>(ac.get()));
            break;
          case cst_type::memberaccess:
            generate_member_access(function,
                                   cst::cast_raw<cst_memberaccess>(ac.get()));
            break;
          case cst_type::dereference:
            function.code.emplace_back(
                op_dereference,
                from_numerical_string<std::int64_t>(ac->content));
            break;
          default:
    ...
    case cst_type::switchstmt: {
      generate_switch(function, cst::cast_raw<cst_switchstmt>(c.get()),
                      current_break_label, current_loop_start);
...
void ir_gen::visualize_stack_ir(const std::vector<ir_function> &funcs) {
  ...
  for (const auto &[code, args, name, type, uses_shellcode, is_external,
                    is_variadic, uses_assembly] : funcs) {
    std::cout << "Type: " << type << "\n";
  ...
  try {
    ...
    for (const auto &c : root->get_children()) {
      ...
      if (type == cst_type::int8_datatype || type == cst_type::int16_datatype ||
          type == cst_type::int32_datatype ||
          type == cst_type::int64_datatype ||
          type == cst_type::uint8_datatype ||
          type == cst_type::uint16_datatype ||
          type == cst_type::uint32_datatype ||
          type == cst_type::uint64_datatype ||
          type == cst_type::float32_datatype ||
          type == cst_type::float64_datatype ||
          type == cst_type::string_datatype ||
        ...
        if (!c->get_children().empty()) {
          const auto id =
              cst::cast_raw<cst_identifier>(c->get_children().front().get());
          if (id) {
    ...
    for (const auto &c : root->get_children()) {
      if (const auto type = c->get_type(); type == cst_type::function) {
        ...
        if (!global_vars.empty() && !func.uses_shellcode &&
            !func.uses_assembly) {
          ...
          for (auto *gv : global_vars) {
            std::string gv_type =
                gv->to_string().substr(4, gv->to_string().size());
            if (gv->num_pointers > 0) {
              gv_type += "_ptr";
            }

            if (!gv->get_children().empty()) {
              if (gv->get_children().size() > 1 &&
                  gv->get_children().back()->get_type() ==
                      cst_type::assignment) {
                const auto identifier = cst::cast_raw<cst_identifier>(
                    gv->get_children().front().get());
                const auto assignment = cst::cast_raw<cst_assignment>(
                    gv->get_children().back().get());
                generate_common(temp_func, assignment, gv_type);
                temp_func.code.emplace_back(op_store, identifier->content,
                                            gv_type);
              }
            }
          }
          ...
          merged.insert(merged.end(), temp_func.code.begin(),
                        temp_func.code.end());
...
std::vector<ir_struct> ir_gen::lower_structs() {
  try {
    ...
    auto handle_struct = [this](const std::string &name,
                                cst *child) -> ir_struct {
      ir_struct ir_s;
      ...
      for (const auto &c : child->get_children()) {
        ...
        default: {
          if (c->get_children().size() == 1 &&
              custom_type_map.contains(c->content) &&
              custom_type_map[c->content]->get_type() ==
                  cst_type::enumeration) {
            ir_s.members.emplace_back("int64",
                                      c->get_children().front()->content);
            break;
