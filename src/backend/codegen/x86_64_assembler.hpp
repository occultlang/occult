...
class assembler {
  ...
  assembler_token handle_numeric() {
    if (source[pos] == '0') {
      ...
      if (source[pos] == 'x' || source[pos] == 'X') {
        ...
        return assembler_token(hex_number,
                               x86_64_instr_token::number_literal_tt);
      }
      ...
      if (source[pos] == 'b' || source[pos] == 'B') {
        ...
        try {
          binary_number =
              to_parsable_type<std::uintptr_t>(binary_number, BINARY_BASE);
        } catch (const std::exception &) {
          return assembler_token("0", x86_64_instr_token::number_literal_tt);
        }
        return assembler_token(binary_number,
                               x86_64_instr_token::number_literal_tt);
      }
      ...
      if (source[pos] != '.') {
        ...
        return assembler_token(std::to_string(octal_value),
                               x86_64_instr_token::number_literal_tt);
      }
    }
    ...
    while (numeric_set.contains(source[pos]) || source[pos] == '-' ||
           source[pos] == '+') {
      normal_number += source[pos];
      pos += 1;
    }

    return assembler_token(normal_number,
                           x86_64_instr_token::number_literal_tt);
  }
  ...
  assembler_token get_next_token() {
    if (pos >= source.length()) {
      return assembler_token("end of assembly",
                             x86_64_instr_token::end_of_assembly_tt);
    }
  ...
  // [reg], [reg+disp], [reg-disp], [reg+idx], [reg+idx*scale],
  // [reg+idx*scale+disp], [reg+idx*scale-disp]
  mem parse_mem() {
    if (!is_gpr(peek().tt)) {
    ...
    if (peek().tt == x86_64_instr_token::minus_tt) {
      ...
      if (peek().tt != x86_64_instr_token::number_literal_tt) {
        throw std::runtime_error(
            "expected displacement after '-' in memory operand");
      }
      ...
      if (peek().tt != x86_64_instr_token::bracket_right_tt) {
        throw std::runtime_error(
            "expected ']' after displacement in memory operand");
      }
      consume();
    ...
    if (peek().tt == x86_64_instr_token::addition_tt) {
      ...
      if (peek().tt == x86_64_instr_token::number_literal_tt) {
        ...
        consume();
        std::int32_t disp =
            from_numerical_string<std::int32_t>(disp_tok.lexeme);
      ...
      if (is_gpr(peek().tt)) {
        ...
        if (peek().tt == x86_64_instr_token::multiplication_tt) {
          ...
          if (peek().tt != x86_64_instr_token::number_literal_tt) {
            throw std::runtime_error(
                "expected scale after '*' in memory operand");
          }

          auto scale_tok = peek();
          consume();
          std::size_t scale = encode_scale(
              from_numerical_string<std::size_t>(scale_tok.lexeme));
          ...
          if (peek().tt == x86_64_instr_token::addition_tt ||
            ...
            if (peek().tt != x86_64_instr_token::number_literal_tt) {
              throw std::runtime_error(
                  "expected displacement after scale in memory operand");
            }
...
#define EMIT_ARITH(X)                                                          \
  ...
  if (peek().tt == x86_64_instr_token::bracket_left_tt) {                      \
    ...
    if (peek().tt == x86_64_instr_token::number_literal_tt ||                  \
      ...
      throw std::runtime_error("unsupported source operand for " #X            \
                               " [mem], X");                                   \
    }                                                                          \
  } else if (is_gpr(peek().tt)) {                                              \
    auto reg = peek();                                                         \
    ...
    if (peek().tt == x86_64_instr_token::number_literal_tt ||                  \
      ...
      w.emit_##X(assembler_token_to_gpr(reg.tt),                               \
                 assembler_token_to_gpr(reg2.tt));                             \
    } else {                                                                   \
      throw std::runtime_error("unsupported source operand for " #X            \
                               " reg, X");                                     \
    }                                                                          \
...
#define EMIT_SIMD_ARITH(X)                                                     \
  ...
  {                                                                            \
    ...
    auto src_tok = peek();                                                     \
    consume();                                                                 \
    w.emit_##X(assembler_token_to_simd[dest_tok.tt],                           \
               assembler_token_to_simd[src_tok.tt]);                           \
  }
...
public:
    ...
    while (pos_stream < assembler_token_vec.size()) {
      ...
      case x86_64_instr_token::imul_tt: {
        ...
        if (peek().tt == x86_64_instr_token::bracket_left_tt) {
          ...
          std::int64_t imm = parse_signed_imm();
          w.emit_imul(assembler_token_to_gpr(reg.tt),
                      assembler_token_to_gpr(reg2.tt), imm);
        } else {
      ...
      // call reg or [mem] only (rel32 needs label resolution, not yet
      // supported)
      case x86_64_instr_token::call_tt: {
        ...
        if (peek().tt == x86_64_instr_token::bracket_left_tt) {
          ...
          throw std::runtime_error(
              "unsupported operand for call (label targets not yet supported)");
        }
      ...
      case x86_64_instr_token::movzx_tt: {
        ...
        if (!is_gpr(peek().tt)) {
          throw std::runtime_error(
              "expected destination register for movsx/movzx");
        }
        ...
        if (peek().tt == x86_64_instr_token::bracket_left_tt) {
          consume();
          mem src_mem = parse_mem();
          // without a size hint in the source memory operand we default to
          // 8-bit; callers should use movzx/movsx with a typed register source
          // when possible
          if (is_sx)
          ...
          if (is_sx)
            w.emit_movsx(assembler_token_to_gpr(dest_reg.tt),
                         assembler_token_to_gpr(src_reg.tt), is_8);
          else
            w.emit_movzx(assembler_token_to_gpr(dest_reg.tt),
                         assembler_token_to_gpr(src_reg.tt), is_8);
        } else {
          throw std::runtime_error(
              "unsupported source operand for movsx/movzx");
        }
      ...
      case x86_64_instr_token::movsxd_tt: {
        ...
        if (peek().tt == x86_64_instr_token::bracket_left_tt) {
          ...
          auto src_reg = peek();
          consume();
          w.emit_movsxd(assembler_token_to_gpr(dest_reg.tt),
                        assembler_token_to_gpr(src_reg.tt));
        } else {
      ...
      case x86_64_instr_token::movsd_tt: {
        ...
        if (peek().tt == x86_64_instr_token::bracket_left_tt) {
          ...
          if (!is_xmm(peek().tt)) {
            throw std::runtime_error(
                "expected xmm source in movss/movsd [mem], xmm");
          }
          auto src_tok = peek();
          consume();
          ...
          if (peek().tt == x86_64_instr_token::bracket_left_tt) {
            ...
            consume();
            if (is_ss)
              w.emit_movss(assembler_token_to_simd[dest_tok.tt],
                           assembler_token_to_simd[src_tok.tt]);
            else
              w.emit_movsd(assembler_token_to_simd[dest_tok.tt],
                           assembler_token_to_simd[src_tok.tt]);
          } else {
            throw std::runtime_error(
                "unsupported source operand for movss/movsd");
          }
        } else {
          throw std::runtime_error(
              "unsupported destination operand for movss/movsd");
        }
      ...
      case x86_64_instr_token::movq_tt: {
        ...
        if (is_xmm(peek().tt)) {
          ...
          auto src_tok = peek();
          consume();
          w.emit_movq(assembler_token_to_simd[dest_tok.tt],
                      assembler_token_to_gpr(src_tok.tt));
        } else if (is_gpr(peek().tt)) {
          // movq reg, xmm
          auto dest_tok = peek();
          consume();
          ...
          auto src_tok = peek();
          consume();
          w.emit_movq(assembler_token_to_gpr(dest_tok.tt),
                      assembler_token_to_simd[src_tok.tt]);
        } else {
      ...
      case x86_64_instr_token::mov_tt: {
        ...
        if (peek().tt ==
            x86_64_instr_token::bracket_left_tt) { // destination is memory
          consume();
          ...
          if (peek().tt == x86_64_instr_token::number_literal_tt ||
            ...
            throw std::runtime_error(
                "unsupported source operand for mov [mem], X");
          }
          ...
          if (peek().tt == x86_64_instr_token::number_literal_tt ||
            ...
            w.emit_mov(assembler_token_to_gpr(reg.tt), imm);
                     ...
                     x86_64_instr_token::bracket_left_tt) { // mov reg, [mem]
            consume();
            mem src_mem = parse_mem();
            ...
            auto reg2 = peek();
            consume();
            w.emit_mov(assembler_token_to_gpr(reg.tt),
                       assembler_token_to_gpr(reg2.tt));
          } else {
            throw std::runtime_error(
                "unsupported source operand for mov reg, X");
          }
        } else {
  ...
  assembler(const std::string &assembly, const bool debug = false)
      : source(assembly), debug(debug) {}
};
} // namespace occult::x86_64
