...
static const std::unordered_map<std::string, std::string>
    cast_keyword_to_typename = {
        {"i8", "int8"},     {"i16", "int16"},  {"i32", "int32"},
        {"i64", "int64"},   {"u8", "uint8"},   {"u16", "uint16"},
        {"u32", "uint32"},  {"u64", "uint64"}, {"f32", "float32"},
        {"f64", "float64"},
};
...
class codegen {
  ...
  template <typename RegT, std::size_t RegSize> class register_pool {
    struct stack_value {
  ...
  public:
    explicit register_pool(x86_64_writer *w,
                           std::initializer_list<RegT> initial_regs)
        : initial_free_regs(initial_regs), writer(w), r_debug(w->debug) {
    ...
    void push(RegT r) {
      reg_stack.push_back({.is_spilled = false, .reg = r, .offset = 0});
    }
  ...
  void generate_code(ir_function &func, x86_64_writer *w, bool is_main,
                     bool use_jit) {
    register_pool_gp pool(w, {r10, r11, r12, r13, r14, r15, rbx});
    // Pool allocates from back of list: xmm1 first, then xmm2-7, then xmm0
    // (reserved as last-resort so it stays free for function returns/args),
    // then xmm8-15.
    register_pool_simd simd_pool(w, {xmm15, xmm14, xmm13, xmm12, xmm11, xmm10,
                                     xmm9, xmm8, xmm0, xmm7, xmm6, xmm5, xmm4,
                                     xmm3, xmm2, xmm1});
    ...
    std::string pending_struct_type; // set by op_struct_decl, consumed by
                                     // op_struct_store
    ...
    constexpr simd128 sysv_regs_simd[] = {xmm0, xmm1, xmm2, xmm3,
                                          xmm4, xmm5, xmm6, xmm7};

    if (!func.uses_shellcode && !func.uses_assembly) {
      std::size_t gp_arg_idx = 0;
      std::size_t fp_arg_idx = 0;
      std::size_t stack_arg_offset =
          16; // start after return address and saved rbp
        ...
        if (is_systemv) {
          if (is_fp) {
            ...
            if (gp_arg_idx < 6) {
              ...
              auto r = pool.alloc();
              w->emit_mov(
                  r, mem{rbp, static_cast<std::int32_t>(stack_arg_offset)});
              w->emit_mov(mem{rbp, -static_cast<std::int32_t>(totalsizes)}, r);
              pool.free(r);
              stack_arg_offset += 8;
            }
          ...
          if (is_fp) {
            ...
            if (i < 4) {
              ...
              auto r = pool.alloc();
              w->emit_mov(r, mem{rbp, static_cast<std::int32_t>(
                                          32 + (i - 4) * 8 + 16)});
              w->emit_mov(mem{rbp, -static_cast<std::int32_t>(totalsizes)}, r);
              pool.free(r);
            }
      ...
      if (!va_arg_names.empty()) {
        ...
        for (std::size_t j = 0; j < va_count; j++) {
          ...
          std::int32_t element_offset =
              base_offset + static_cast<std::int32_t>(j * 8);
    ...
    for (std::size_t i = 0; i < func.code.size(); i++) {
        ...
        if (std::holds_alternative<std::int64_t>(code.operand)) {
          ...
          *reinterpret_cast<std::uint64_t *>(buf.get()) = str.size();
          ...
          std::uint64_t host_addr =
              reinterpret_cast<std::uint64_t>(literal_ptr);
      ...
      case ir_opcode::op_push_single: {
        if (std::holds_alternative<std::int64_t>(code.operand)) {
          std::int64_t val = std::get<std::int64_t>(code.operand);
          grp r =
              (push_single) ? pool.top() : pool.alloc(); // have to check this
      ...
      case ir_opcode::op_store: {
        ...
        if (!is_float_store && it != local_variable_map.end()) {
          ...
          if (type_it != local_variable_map_types.end()) {
            is_float_store =
                (type_it->second == "float32" || type_it->second == "float64");
            if (is_float_store) {
        ...
        if (it == local_variable_map.end()) {
          ...
          if (is_reference_next) {
            std::cout << RED
                      << "[CODEGEN ERROR] Cannot have reference in store."
                      << RESET << std::endl;
          }
          ...
          if (debug) {
            std::cout << BLUE
                      << "[CODEGEN INFO] Re-emitting variable: " << RESET
                      << var_name << std::endl;
          }
      ...
      case ir_opcode::op_load: {
        ...
        if (it == local_variable_map.end()) {
          std::cerr << RED
                    << "[CODEGEN ERROR] Attempted to load undeclared variable: "
                    << RESET << var_name << std::endl;
          return;
        }
        ...
        if (is_reference_next) {
          ...
          // forward the underlying address value, not produce
          // pointer-to-pointer. This is required for reference forwarding
          // across function calls.
          const bool is_reference_typed_var =
              var_type.ends_with("_reference") &&
              !struct_layouts.contains(var_type);
      ...
      case ir_opcode::op_dereference_assign: {
        ...
        if (debug) {
          std::cout << BLUE << "[CODEGEN INFO] Dereference (assignment) count: "
                    << RESET << operand << std::endl;
        }
      ...
      case ir_opcode::op_cast: {
        ...
        const std::string &target_type =
            (norm_it != cast_keyword_to_typename.end()) ? norm_it->second
                                                        : raw_target;
        ...
        // Determine source float type from IR type annotation (may be empty for
        // unknown)
        auto src_norm_it = cast_keyword_to_typename.find(code.type);
        const std::string &src_type =
            (src_norm_it != cast_keyword_to_typename.end())
                ? src_norm_it->second
                : code.type;
        const bool source_is_f64 = (src_type == "float64");
        const bool source_is_f32 = (src_type == "float32");
        // Use the IR type annotation when available; fall back to pool
        // heuristic
        const bool source_is_float = (src_type.empty())
                                         ? (pool.empty() && !simd_pool.empty())
                                         : (source_is_f32 || source_is_f64);
      ...
      case ir_opcode::op_bitcast: {
        ...
        const bool target_is_float =
            (target_type == "float32" || target_type == "float64");
        const bool source_is_float =
            (source_type == "float32" || source_type == "float64");
      ...
      case ir_opcode::op_ret: {
        bool is_float_return =
            (func.type == "float32" || func.type == "float64");
      ...
      case ir_opcode::op_jae: {
        ...
        w->push_bytes({opcode::NOP, opcode::NOP, opcode::NOP, opcode::NOP,
      ...
      case ir_opcode::op_call: {
        ...
        if (debug) {
          std::cout << BLUE << "[CODEGEN INFO] Calling: " << YELLOW << "\""
                    << func_name << "\"\n"
                    << RESET;
        }

        auto it = std::ranges::find_if(ir_funcs, [&](const ir_function &f) {
        ...
        if (debug) {
          std::cout << BLUE << "[CODEGEN INFO] Argument count: " << RESET
                    << arg_count << std::endl;
        }

        // collect arguments with their types
        std::vector<std::pair<std::variant<grp, simd128>, bool>>
            args; // pair<reg, is_fp>
        struct spilled_arg {
          std::int32_t offset;
          bool is_fp;
        ...
        if (is_systemv) {
          ...
          constexpr simd128 win_caller_saved[] = {xmm0, xmm1, xmm2,
                                                  xmm3, xmm4, xmm5};
          auto active_simd = simd_pool.get_active_registers();
        ...
        const std::size_t caller_saved_bytes =
            caller_saved_gp.size() * 8 + caller_saved_simd.size() * 16;

        const std::size_t current_mod =
            (totalsizes + caller_saved_bytes + stack_args_bytes) % 16;
        ...
        if (target_fn == nullptr) {
          std::cout << RED << "[CODEGEN ERROR] Function \"" << func_name
                    << "\" unresolved (null target)." << RESET << std::endl;
          return;
        }
        ...
        for (auto it_simd = caller_saved_simd.rbegin();
             it_simd != caller_saved_simd.rend(); ++it_simd) {
        ...
        if (is_systemv) {
          for (auto it_gp = caller_saved_gp.rbegin();
               it_gp != caller_saved_gp.rend(); ++it_gp) {
            w->emit_pop(*it_gp);
          }
        }
          ...
          case ir_opcode::op_load: {
            ...
            for (std::size_t la = i + 1; la < func.code.size(); ++la) {
              ...
              if (la_op == ir_opcode::op_push ||
                ...
                if (!call_type.empty() &&
                    std::all_of(call_type.begin(), call_type.end(),
                                ::isdigit)) {
                  ...
                  auto call_it =
                      std::ranges::find_if(ir_funcs, [&](const ir_function &f) {
                        return f.name == call_name;
                      });
                ...
                depth += 1;
              } else if (la_op == ir_opcode::op_cmp ||
                ...
                break;
                         ...
                         la_op == ir_opcode::op_negatef32 ||
                         la_op == ir_opcode::op_negatef64 ||
                ...
                // net 0 change
                         ...
                         la_op == ir_opcode::op_jb ||
                         la_op == ir_opcode::op_jbe ||
                         la_op == ir_opcode::op_ja ||
                         la_op == ir_opcode::op_jae ||
                         la_op == ir_opcode::op_struct_decl) {
                break;
              }
            }
            break;
      ...
      case ir_opcode::op_array_decl: {
        ...
        if (debug) {
          std::cout << BLUE << "[CODEGEN INFO] Got name of array: " << RESET
                    << name << std::endl;
        }
        ...
        if (debug) {
          std::cout << BLUE << "[CODEGEN INFO] Dimensions count: " << RESET
                    << dimensions << std::endl;
        }

        std::vector<std::uint64_t> dimensions_vec;
        for (std::size_t j = 0; j < dimensions; j++) {
          dimensions_vec.emplace_back(
              std::get<std::uint64_t>(func.code.at(++i).operand));
        }
        ...
        local_variable_map[name] =
            base_offset; // store base address as negative offset from rbp

        if (debug) {
          std::cout << BLUE << "[CODEGEN INFO] Allocated array: " << RESET
                    << name << "\n"
                    << BLUE << "               totalsizes before: " << RESET
                    << (totalsizes - total_mem) << "\n"
                    << BLUE << "               total_mem: " << RESET
                    << total_mem << "\n"
                    << BLUE << "               base_offset: " << RESET
                    << base_offset << "\n"
                    << BLUE << "               totalsizes after: " << RESET
                    << totalsizes << std::endl;
        }
      ...
      case ir_opcode::op_declare_where_to_store: {
        ...
        if (debug) {
          std::cout << BLUE << "[CODEGEN INFO] Declaring store index: " << RESET
                    << index << std::endl;
        }
      ...
      case ir_opcode::op_array_store_element: {
        ...
        if (it == local_variable_map.end()) {
          std::cerr << RED << "[CODEGEN ERROR] Array " << array_name
                    << " not found" << RESET << std::endl;
        ...
        std::int32_t array_base_offset =
            it->second; // negative offset from rbp where array starts

        auto &store_meta = local_array_metadata[array_name];
        bool is_float_array =
            (store_meta.type == "float32" || store_meta.type == "float64");
        ...
        for (std::size_t j = 1; j <= 50 && i >= j;
             j++) { // 50 instr lookback for larger right hand side expr
        ...
        if (!has_constant_index && i >= 2 &&
            func.code.at(i - 1).op == ir_opcode::op_mark_for_array_access &&
            func.code.at(i - 2).op == ir_opcode::op_push) {
          const auto &opnd = func.code.at(i - 2).operand;
          if (std::holds_alternative<std::int64_t>(opnd)) {
            inline_const_index =
                static_cast<std::uint64_t>(std::get<std::int64_t>(opnd));
            ...
            inline_const_index =
                static_cast<std::uint64_t>(std::get<std::int32_t>(opnd));
            ...
            inline_const_index =
                static_cast<std::uint64_t>(std::get<std::int16_t>(opnd));
            ...
            inline_const_index =
                static_cast<std::uint64_t>(std::get<std::int8_t>(opnd));
        ...
        if (has_constant_index) {
          ...
          if (debug) {
            std::cout << BLUE << "[CODEGEN INFO] Array store (constant) to "
                      << array_name << "[" << index << "]" << RESET
                      << std::endl;
          ...
          if (inline_const_index.has_value()) {
            std::int32_t element_offset =
                array_base_offset +
                static_cast<std::int32_t>(8 * inline_const_index.value());

            if (debug) {
              std::cout << BLUE
                        << "[CODEGEN INFO] Array store (inline constant) to "
                        << array_name << "[" << inline_const_index.value()
                        << "]" << RESET << std::endl;
              std::cout << "\tBase offset: " << array_base_offset << std::endl;
              std::cout << "\tElement offset: " << element_offset << std::endl;
            }
          ...
          if (index_to_push != (std::numeric_limits<std::uint64_t>::max)()) {
            ...
            if (debug) {
              std::cout << BLUE
                        << "[CODEGEN INFO] Array store (pending constant) to "
                        << array_name << "[" << index << "]" << RESET
                        << std::endl;
              std::cout << "\tBase offset: " << array_base_offset << std::endl;
              std::cout << "\tElement offset: " << element_offset << std::endl;
          ...
          if (is_float_array) {
            ...
            if (debug) {
              std::cout << BLUE << "[CODEGEN INFO] Array store (dynamic) to "
                        << array_name << "[index_reg]" << RESET << std::endl;
              std::cout << "\tBase offset: " << array_base_offset << std::endl;
            }
      ...
      case ir_opcode::op_array_access_element: {
        ...
        bool is_float_array = (local_metadata.type == "float32" ||
                               local_metadata.type == "float64");

        if (debug) {
          std::cout << BLUE << "[CODEGEN INFO] Array access: " << RESET
                    << array_name << std::endl;
          std::cout << BLUE
                    << "[CODEGEN INFO] Array access total dims: " << RESET
        ...
        if (i >= 2 &&
          ...
          if (inline_idx.has_value()) {
            ...
            std::int32_t element_offset =
                array_base_offset +
                static_cast<std::int32_t>(8 * inline_idx.value());
        ...
        for (std::size_t dims = 0; dims < local_metadata.dimensions.size();
             dims++) {
        ...
        for (std::size_t dim = 0; dim < local_metadata.dimensions.size();
             dim++) {
      ...
      case ir_opcode::op_struct_decl: {
        ...
        if (debug) {
          std::cout << BLUE
                    << "[CODEGEN INFO] Struct declaration of type: " << RESET
                    << pending_struct_type << std::endl;
        }
      ...
      case ir_opcode::op_struct_store: {
        ...
        if (it == local_variable_map.end()) {
          if (is_struct_ptr) {
            // if pointer to struct type, just allocate 8 bytes like a regular
            // var
            totalsizes += 8;
        ...
        if (debug) {
          std::cout << BLUE << "[CODEGEN INFO] Struct variable " << RESET
                    << var_name << " at offset -"
                    << local_variable_map[var_name] << std::endl;
        }
      ...
      case ir_opcode::op_member_access: {
        ...
        if (member_is_nested_struct) {
          ...
          pool.push(base);
        } else if (member_type_name == "float32" ||
                   member_type_name == "float64") {
      ...
      case ir_opcode::op_member_store: {
        ...
        if (!simd_pool.empty() &&
            (member_type == "float32" || member_type == "float64")) {
    ...
    if (debug) {
      std::cout << BLUE
                << "[CODEGEN INFO] Total jump instructions to backpatch: "
                << RESET << jump_instructions.size() << std::endl;
    }
    ...
    for (auto &[fst, snd] : jump_instructions) {
      ...
      if (debug) {
        ...
        if (label_map.contains(label_name)) {
          std::cout << "\tLabel location: " << label_map[label_name]
                    << std::endl;
      ...
      if (label_map.contains(label_name)) {
        ...
        std::cout << RED << "[CODEGEN ERROR] Label \"" << label_name
                  << "\" not found." << RESET << std::endl;
        return;
      }
    }
    ...
    if ((!func.uses_shellcode && !func.uses_assembly) &&
        stack_alloc_placeholder_location > 0) {
      auto &code = w->get_code();
      ...
      for (int i = 0; i < 4; ++i) {
        code[stack_alloc_placeholder_location + 3 + i] =
            static_cast<std::uint8_t>((alloc_size >> (i * 8)) & 0xFF);
      }

      if (debug) {
        std::cout << BLUE
                  << "[CODEGEN INFO] Patched stack allocation: sub rsp, "
                  << totalsizes << RESET << std::endl;
      }
    }
    ...
    if ((!func.uses_shellcode || !func.uses_assembly) &&
      ...
      if (totalsizes % 16 != 0) {
        ...
        if (debug && alignment_padding > 0) {
          std::cout << BLUE << "[CODEGEN INFO] Added " << alignment_padding
                    << " bytes stack alignment padding (totalsizes was "
                    << (totalsizes - alignment_padding) << ")" << RESET
                    << std::endl;
        }
      }
      ...
      if (debug) {
        std::cout << BLUE << "[CODEGEN INFO] Patched stack alloc to "
                  << alloc_size << " bytes" << RESET << std::endl;
      }
    }
  }
#ifndef _WIN64
  void patch_and_register_cached(
      ir_function &func, std::vector<std::uint8_t> &cached_code,
      const std::unordered_map<std::string, jit_function> &cached_function_map,
      const std::unordered_map<std::uint64_t, std::string>
          &cached_string_literals,
      bool use_jit) {
    function_map.insert(
        {func.name,
         nullptr}); // prevent recursive cache patching from re-entering itself.
    ...
    for (const auto &[old_addr, content] : cached_string_literals) {
      ...
      *reinterpret_cast<std::uint64_t *>(buf.get()) = content.size();
      ...
      const std::uint64_t new_addr =
          reinterpret_cast<std::uint64_t>(buf.get() + header_size);
      ...
      if (debug) {
        std::cout << BLUE << "[CACHE] String relocated: \""
                  << content.substr(0, 40) << "\"  old=0x" << std::hex
                  << old_addr << " -> new=0x" << new_addr << std::dec << RESET
                  << "\n";
      }
    }
    ...
    for (std::size_t i = 0; i < code_size;) {
      ...
      for (int j = 0; j < 8; ++j) {
        embedded_addr |= static_cast<std::uint64_t>(cached_code[i + 2 + j])
                         << (j * 8);
      }
      ...
      if (b0 == 0x48 && b1 == 0xB8) {
        ...
        const bool call_3b =
            (i + 13 <= code_size) && cached_code[i + 10] == 0x48 &&
            cached_code[i + 11] == 0xFF && cached_code[i + 12] == 0xD0;
        ...
        if (call_2b || call_3b) {
          ...
          if (it != old_addr_to_name.end()) {
            ...
            const bool callee_ready = (callee_it != function_map.end() &&
                                       callee_it->second != nullptr);

            if (!callee_ready) {
              auto fn_it = std::ranges::find_if(
                  ir_funcs, [&](const auto &f) { return f.name == callee; });
            ...
            if (target_it != function_map.end() &&
                target_it->second != nullptr) {
              const std::uint64_t new_addr =
                  reinterpret_cast<std::uint64_t>(target_it->second);

              for (int j = 0; j < 8; ++j) {
                cached_code[i + 2 + j] =
                    static_cast<std::uint8_t>((new_addr >> (j * 8)) & 0xFF);
              }

              if (debug) {
                std::cout << BLUE << "[CACHE] Patched call -> " << callee
                          << " = 0x" << std::hex << new_addr << std::dec
                          << RESET << "\n";
              }
            } else {
      ...
      // any movabs r64 carrying a string literal pointer
      {
        ...
        if (it != string_old_to_new.end()) {
          ...
          if (debug) {
            std::cout << BLUE << "[CACHE] Patched string ptr 0x" << std::hex
                      << embedded_addr << " -> 0x" << new_addr << std::dec
                      << RESET << "\n";
          }
        }
    ...
    for (const auto &pending : pending_calls) {
      ...
      if (target_it == function_map.end() || target_it->second == nullptr) {
        throw std::runtime_error("cache patch failed: unresolved function \"" +
                                 pending.callee + "\"");
      }
      ...
      for (int j = 0; j < 8; ++j) {
        cached_code[pending.imm_offset + j] =
            static_cast<std::uint8_t>((new_addr >> (j * 8)) & 0xFF);
      }
    }
    ...
    if (debug) {
      std::cout << GREEN << "[CACHE] Registered cached function: " << func.name
                << RESET << "\n";
    }
  }
#endif
  ...
  void compile_function(ir_function &func, const bool use_jit) {
    ...
    if (function_map.contains(func.name)) {
      if (debug) {
        std::cout << BLUE << "[CODEGEN SKIP] Already registered: " << YELLOW
                  << func.name << RESET << "\n";
      }
    ...
    if (!cached_bytes.empty() &&
        cached_bytes.size() >= sizeof(bytecode_header)) {
      const auto *header =
          reinterpret_cast<const bytecode_header *>(cached_bytes.data());
      IRHash current_hash = hash_ir_function(func);

      if (header->magic == OCCULT_MAGIC &&
          header->ir_hash_low == current_hash.low64 &&
          header->ir_hash_high == current_hash.high64) {
        if (debug) {
          std::cout << GREEN << "[CODEGEN CACHE HIT] " << func.name << RESET
                    << "\n";
        }

        std::vector<std::uint8_t> cached_code(
            cached_bytes.begin() + header->code_offset,
            cached_bytes.begin() + header->code_offset + header->code_size);
        ...
        while (i + 16 <= cached_bytes.size()) {
          ...
          std::string lit(
              reinterpret_cast<const char *>(cached_bytes.data() + i), len);
        ...
        patch_and_register_cached(func, cached_code, cached_function_map,
                                  cached_string_literals, use_jit);
    ...
    // fresh compilation
    if (debug) {
      std::cout << BLUE << "[CODEGEN] Compiling: " << YELLOW << func.name
                << RESET << "\n";
    }
    ...
    write_cached_code(func, function_raw_code_map, function_map,
                      string_literals);
#endif
    w->setup_function();
    writers.push_back(std::move(w));
