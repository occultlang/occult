...
namespace occult::x86_64 {
    ...
    class codegen {
        ...
        void generate_code(ir_function &func, x86_64_writer *w, bool is_main, bool use_jit) {
            ...
            for (std::size_t i = 0; i < func.args.size(); ++i) {
                ...
                if (is_systemv) {
                    if (i < sizeof(sysv_regs) / sizeof(grp)) { w->emit_mov(r, sysv_regs[i]); }
                    else {
                ...
                switch (code.op) {
                        ...
                        if (std::holds_alternative<std::int64_t>(code.operand)) {
                            ...
                            for (std::size_t j = 0; j < num_chunks; ++j) {
                                ...
                                for (std::size_t k = 0; k < 8 && (j * 8 + k) < str.size(); ++k) {
                                    chunk |= static_cast<std::uint64_t>(static_cast<std::uint8_t>(str[j * 8 + k]))
                                            << (k * 8);
                                }
                    ...
                    case ir_opcode::op_store: {
                        ...
                        if (it == local_variable_map.end()) { // variable doesn't exist yet
                            if (debug) {
                                std::cout << BLUE << "[CODEGEN INFO] Emitting variable: " << RESET << var_name <<
                                        std::endl;
                            ...
                            if (is_reference_next) {
                                std::cout << RED << "[CODEGEN ERROR] Cannot have reference in store." << RESET <<
                                        std::endl;
                            }
                            ...
                            if (debug) {
                                std::cout << BLUE << "[CODEGEN INFO] Re-emitting variable: " << RESET << var_name <<
                                        std::endl;
                            }
                    ...
                    case ir_opcode::op_load: {
                        ...
                        if (it == local_variable_map.end()) {
                            std::cerr << RED << "[CODEGEN ERROR] Attempted to load undeclared variable: " << RESET
                                    << var_name << std::endl;
                        ...
                        if (var_type == "int8") { w->emit_movsx(r, mem{rbp, offset}); }
                        else if (var_type == "uint8") { w->emit_movzx(r, mem{rbp, offset}); }
                        else if (var_type == "int16") { w->emit_movsx(r, mem{rbp, offset}, false); }
                        else if (var_type == "uint16") { w->emit_movzx(r, mem{rbp, offset}, false); }
                        else if (var_type == "int32") { w->emit_movsxd(r, mem{rbp, offset}); }
                        else if (var_type == "uint32") { w->emit_mov(as_32(r), mem{rbp, offset}); }
                        else
                            if (var_type == "int64" || var_type == "uint64") { w->emit_mov(r, mem{rbp, offset}); }
                    ...
                    case ir_opcode::op_call: {
                        ...
                        if (!function_map.contains(func_name)) {
                            std::cout << RED << "[CODEGEN ERROR] Function \"" << func_name << "\" not found." <<
                                    RESET << std::endl;
                        ...
                        if (debug) {
                            std::cout << BLUE << "[CODEGEN INFO] Popping arguments for function call: " << YELLOW <<
                                    "\"" << func_name << "\"\n" << RESET;
                        }
                        ...
                        if (is_systemv) {
                            ...
                            for (auto reg: caller_saved) {
                                if (std::find(reg_stack_copy.begin(), reg_stack_copy.end(), reg) != reg_stack_copy.
                                    end()) {
                                    w->emit_push(reg);
                                    used.push_back(reg);
                                }
                    ...
                    case ir_opcode::op_array_decl: {
                        ...
                        if (debug) { std::cout << BLUE << "[CODEGEN INFO] Array type: " << RESET << type << std::endl; }
                        ...
                        if (debug) {
                            ...
                            for (const auto &e: dimensions_vec) {
                                std::cout << BLUE << "[CODEGEN INFO] Size in dimension (" << ++j << "): " << RESET
                                        << e << std::endl;
                            }
                        }
                        ...
                        for (const auto &dim: dimensions_vec) { element_count *= dim; }
                    ...
                    case ir_opcode::op_array_store_element: {
                        ...
                        if (it == local_variable_map.end()) {
                            std::cerr << RED << "[CODEGEN ERROR] Array " << array_name << " not found" << RESET <<
                                    std::endl;
                            return;
                        }
        ...
        void compile_function(ir_function &func, bool use_jit) {
            ...
            if (debug && function_map.contains(func.name)) {
                std::cout << BLUE << "[CODEGEN INFO] Symbol " << YELLOW << "\"" << func.name << "\"" << BLUE <<
                        " already exists, probably already compiled or is compiling." << RESET << std::endl;
                return;
            }

            if (compiling_functions.contains(func.name)) { return; }
    ...
    public:
        ...
        void compile(const bool use_jit = true) { for (auto &func: ir_funcs) { compile_function(func, use_jit); } }
    };
}
