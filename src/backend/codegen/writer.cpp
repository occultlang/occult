...
namespace occult {
    ...
    void writer::push_bytes(const std::initializer_list<std::uint8_t> &bytes) { code.insert(code.end(), bytes); }
    ...
    std::vector<std::uint8_t> writer::string_to_bytes(const std::string &str) {
        ...
        for (const auto &byte: str) { vectorized_data.emplace_back(static_cast<std::uint8_t>(byte)); }

        return vectorized_data;
    }

    std::size_t writer::push_string(const std::string &str) {
        if (!string_locations.contains(str)) { string_locations.insert({str, code.size()}); }
    ...
    jit_function writer::setup_function() {
        ...
        if (required_size > allocated_size) {
            ...
            if (new_memory == MAP_FAILED) { throw std::runtime_error("Failed to reallocate memory"); }
    ...
    jit_function writer::setup_function() {
        if (code.size() > allocated_size) {
            ...
            DWORD old_protect = 0;
            PVOID new_memory = VirtualAlloc(nullptr, new_size, PAGE_EXECUTE_READWRITE | MEM_COMMIT | MEM_RESERVE,
                                            old_protect);

            if (new_memory == nullptr) { throw std::runtime_error("Failed to allocate additional memory"); }
    ...
    void writer::print_bytes() const {
        for (auto &c: code) {
            std::cout << std::setw(2) << std::setfill('0') << std::uppercase << std::hex << static_cast<int>(c) << " "
                    << std::dec;
        }
