...
namespace occult {
    ...
    void writer::push_bytes(const std::vector<std::uint8_t>& bytes) { code.insert(code.end(), bytes.begin(), bytes.end()); }
    ...
    jit_function writer::setup_function() {
        ...
        if (required_size > allocated_size) {
            ...
            PVOID new_memory = VirtualAlloc(nullptr, new_size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
