...
using ir_operand = std::variant<std::monostate, std::int64_t, std::uint64_t, std::int32_t,
...
static std::unordered_map<std::string, bool> is_signed = {
    {"int64", true},   {"int32", true},   {"int16", true},  {"int8", true}, {"uint64", false},
    {"uint32", false}, {"uint16", false}, {"uint8", false}, {"bool", true}, {"char", true},
};
...
public:
    ir_gen(cst_root* root, std::unordered_map<std::string, cst*> custom_type_map,
        ...
        : root(root), label_count(0), debug(debug), custom_type_map(std::move(custom_type_map)) {}
