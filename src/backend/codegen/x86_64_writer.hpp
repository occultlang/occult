...
struct mem {
    ...
    explicit mem(const grp& reg, const grp& index, const std::size_t& scale) : reg(reg), scale(scale), index(index), disp(0) {
        mode = mem_mode::scaled_index;
    }
...
class x86_64_writer : public writer {
    ...
    template <typename IntType = std::int8_t> void emit_imm(IntType imm, const int size) {
...
#define CHECK_2BYTE_OPCODE_PREFIX_AND_PUSH                                                                                                                                                                                                     \
    ...
    // "different modes?"
    template <std::integral T> void emit_reg_imm(const opcode& op8, const opcode& op, const grp& dest, const T& imm, const rm_field& rm, const bool is_signed = true, const imm_mode im_m = imm_mode::normal) {
    ...
    template <std::integral T> void emit_reg_imm_basic(const opcode& op8, const opcode& op, const grp& dest, const T& imm, bool is_signed = true) {
        if (REG_RANGE(dest, r8b, r15b) || REG_RANGE(dest, al, dil)) {
            if (REG_RANGE(dest, r8b, r15b)) {
                push_byte(rex_b);
            }
    ...
    // future i guess lmao
    template <std::integral T> void emit_mem_imm(const opcode& op8, const opcode& op, const mem& dest, const T& imm, const rm_field& rm, const bool is_signed = true, const imm_mode im_m = imm_mode::normal) {
        if (dest.reg == rip) { // rip relative
  ...
  public:
    ...
    */
    ...
    void emit_movq(REG_TO_SIMD128_ARG) {
        emit_reg_to_simd128(float_opcode_2b::MOVQ_xmm_to_rm32_to_64, base, dest, {operand_size_override});
    }

    // move value into sse registers from GRP (single or double precision)
    void emit_movd(REG_TO_SIMD128_ARG) {
        emit_reg_to_simd128(float_opcode_2b::MOVD_xmm_to_rm32_to_64, base, dest, {operand_size_override});
    }

    // add scalar packed double precision
    void emit_addsd(SIMD128_TO_SIMD128_ARG) {
        push_bytes({kREPNE_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(ADDSD_xmm_to_xmm_or_m64), modrm(mod_field::register_direct, dest, base)});
    }

    // add scalar packed single precision
    void emit_addss(SIMD128_TO_SIMD128_ARG) {
        push_bytes({kREP_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(ADDSS_xmm_to_xmm_or_m32), modrm(mod_field::register_direct, dest, base)});
    }

    void emit_subsd(SIMD128_TO_SIMD128_ARG) {
        push_bytes({kREPNE_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(SUBSD_xmm_to_xmm_or_m64), modrm(mod_field::register_direct, dest, base)});
    }

    void emit_subss(SIMD128_TO_SIMD128_ARG) {
        push_bytes({kREP_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(SUBSS_xmm_to_xmm_or_m32), modrm(mod_field::register_direct, dest, base)});
    }

    void emit_mulsd(SIMD128_TO_SIMD128_ARG) {
        push_bytes({kREPNE_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(MULSD_xmm_to_xmm_or_m64), modrm(mod_field::register_direct, dest, base)});
    }

    void emit_mulss(SIMD128_TO_SIMD128_ARG) {
        push_bytes({kREP_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(MULSS_xmm_to_xmm_or_m32), modrm(mod_field::register_direct, dest, base)});
    }

    void emit_divsd(SIMD128_TO_SIMD128_ARG) {
        push_bytes({kREPNE_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(DIVSD_xmm_to_xmm_or_m64), modrm(mod_field::register_direct, dest, base)});
    }

    void emit_divss(SIMD128_TO_SIMD128_ARG) {
        push_bytes({kREP_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(DIVSS_xmm_to_xmm_or_m32), modrm(mod_field::register_direct, dest, base)});
    }

    void emit_cvttsd2si(REG_TO_SIMD128_ARG) {
        emit_reg_to_simd128(CVTTSD2SI_xmm_or_m64_to_r32_to_64, static_cast<grp>(base), static_cast<simd128>(dest), {kREPNE_Prefix});
    }

    void emit_cvtsi2sd(SIMD128_TO_REG_ARG) {
        emit_reg_to_simd128(CVTSI2SD_rm32_to_64_to_xmm, static_cast<grp>(base), static_cast<simd128>(dest), {kREPNE_Prefix});
    }

    void emit_cvttss2si(REG_TO_SIMD128_ARG) {
        emit_reg_to_simd128(CVTTSD2SI_xmm_or_m64_to_r32_to_64, static_cast<grp>(dest), static_cast<simd128>(base), {kREP_Prefix});
    }

    void emit_cvtsi2ss(SIMD128_TO_REG_ARG) {
        emit_reg_to_simd128(CVTSI2SD_rm32_to_64_to_xmm, static_cast<grp>(dest), static_cast<simd128>(base), {kREP_Prefix});
    }
    ...
    void emit_movss(SIMD128_TO_MEM_ARG) {
        emit_simd128_to_mem(static_cast<opcode>(MOVSS_xmm_to_xmm_or_m32), base, dest, {kREP_Prefix});
    }

    void emit_movss(MEM_TO_SIMD128_ARG) {
        emit_simd128_to_mem(static_cast<opcode>(MOVSS_xmm_or_m32_to_xmm), dest, base, {kREP_Prefix});
    }

    void emit_movsd(SIMD128_TO_MEM_ARG) {
        emit_simd128_to_mem(static_cast<opcode>(MOVSD_xmm_to_xmm_or_m64), base, dest, {kREPNE_Prefix});
    }

    void emit_movsd(MEM_TO_SIMD128_ARG) {
        emit_simd128_to_mem(static_cast<opcode>(MOVSD_xmm_or_m64_to_xmm), dest, base, {kREPNE_Prefix});
    }
    ...
    void emit_comisd(SIMD128_TO_SIMD128_ARG) {
        push_bytes({operand_size_override, k2ByteOpcodePrefix, static_cast<uint8_t>(0x2F), modrm(mod_field::register_direct, dest, base)});
    }
};
} // namespace occult::x86_64
