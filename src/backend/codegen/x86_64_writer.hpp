...
// disabled address_size_override 0x67, only calculate using 64-bit addressing for now. (has bug in
// codegen)

#define REG_RANGE(to_cmp, start, end) (to_cmp >= start && to_cmp <= end)

#define NOT_STACK_BASE_PTR(dest) (dest != rbp && dest != ebp && dest != bp && dest != bpl)

#define NOT_STACK_PTR(dest) (dest != rsp && dest != esp && dest != sp && dest != spl)

#define IS_STACK_BASE_PTR(dest) (dest == rbp || dest == ebp || dest == bp || dest == bpl)

#define IS_STACK_PTR(dest) (dest == rsp || dest == esp || dest == sp || dest == spl)
...
#define CHECK_2BYTE_OPCODE_PREFIX_AND_PUSH                                                         \
    ...
    void emit_reg_to_reg(const opcode& op8, const opcode& op, const grp& dest, const grp& base,
                         const bool is_2bOPC = false) {
        if (REG_RANGE(dest, r8b, r15b) || REG_RANGE(dest, al, dil) || REG_RANGE(base, r8b, r15b) ||
            REG_RANGE(base, al, dil)) {
            ...
            push_byte(op8);
            push_byte(
                modrm(mod_field::register_direct, rebase_register(dest), rebase_register(base)));
    ...
    void emit_reg_to_simd128(const float_opcode_2b& op, const grp& dest, const simd128& base,
    ...
    void print_modrm_debug(uint8_t modrm_byte, uint8_t reg_field, uint8_t rm_field, uint8_t mod) {
        ...
        std::cout << std::bitset<3>(rm_field) << " = 0x" << std::hex << std::uppercase
                  << +modrm_byte << std::dec << "\n";
    }

    void emit_simd128_to_mem(const opcode& op, const simd128& base, const mem& dest,
        ...
        if (dest.reg == rip) { // rip relative
            ...
            // mem_mode::reg;\n";
            check_reg_r2m_simd(op, op, dest, static_cast<grp>(base), is_2bOPC, prefix);
            // std::cout << "Rebased (Source): " <<  reg_to_string(rebase_register(base)) <<
            // std::endl; std::cout << "Rebased (Dest): " <<
    ...
    void check_reg_r2m_simd(const opcode& op8, const opcode& op, const mem& dest, const grp& base,
                            const bool is_2bOPC = false,
                            const std::vector<std::uint8_t>& prefix = {}) {
        auto emit_2b = [&]() {
            push_bytes(prefix);
        ...
        if (dest.reg == sp || dest.reg == esp || dest.reg == rsp || dest.reg == spl || base == sp ||
            base == esp || base == rsp || base == spl) {
    ...
    void emit_reg_to_mem(const opcode& op8, const opcode& op, const mem& dest, const grp& base,
        ...
        if (dest.reg == rip) { // rip relative
            ...
            if (debug)
                std::cout << "Rebased (Dest): " << reg_to_string(rebase_register(dest.reg))
                          << std::endl;
    ...
    // rm field indicates which operation is used for things such as (ADD, OR, etc. ) (opcodes are
    // 0x80 for 8bit and 0x81 for higher than 8bit) this can also be used for 0xC7 and 0xC6 this is
    // for the opcodes which require "different modes?"
    template <std::integral T>
    void emit_reg_imm(const opcode& op8, const opcode& op, const grp& dest, const T& imm,
                      const rm_field& rm, const bool is_signed = true,
    ...
    template <std::integral T>
    void emit_mem_imm(const opcode& op8, const opcode& op, const mem& dest, const T& imm,
                      const rm_field& rm, const bool is_signed = true,
                      const imm_mode im_m = imm_mode::normal) {
        if (dest.reg == rip) { // rip relative
    ...
    void emit_setX_reg8(const opcode_2b& op, REG_ARG) { // remove the t ;)
        emit_reg_to_reg(static_cast<opcode>(op), static_cast<opcode>(op), _reg, static_cast<grp>(0),
                        true);
    }

    void emit_setX_mem8(const opcode_2b& op, MEM_ARG) {
        emit_reg_to_mem(static_cast<opcode>(op), static_cast<opcode>(op), m, static_cast<grp>(0),
                        true);
    }
...
public:
    ...
    void emit_or(SIGNED_IMM_TO_REG_ARG) {
        assert_imm_size<std::int64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
    ...
    void emit_or(SIGNED_IMM_TO_MEM_ARG) {
        assert_imm_size<std::int64_t>(imm);
        emit_mem_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(1), true, imm_mode::no_64);
    }
    ...
    void emit_adc(SIGNED_IMM_TO_REG_ARG) {
        // not gonna lie, the opcode numbers are the same, i cba to change them right now, will do
        // them later
        assert_imm_size<std::int64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(2), true, imm_mode::no_64);
    }

    void emit_adc(UNSIGNED_IMM_TO_REG_ARG) {
        assert_imm_size<std::uint64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(2), false, imm_mode::no_64);
    }
    ...
    void emit_sbb(SIGNED_IMM_TO_REG_ARG) {
        assert_imm_size<std::int64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(3), true, imm_mode::no_64);
    }

    void emit_sbb(UNSIGNED_IMM_TO_REG_ARG) {
        assert_imm_size<std::uint64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(3), false, imm_mode::no_64);
    }
    ...
    void emit_and(SIGNED_IMM_TO_REG_ARG) {
        assert_imm_size<std::int64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(4), true, imm_mode::no_64);
    }

    void emit_and(UNSIGNED_IMM_TO_REG_ARG) {
        assert_imm_size<std::uint64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(4), false, imm_mode::no_64);
    }
    ...
    void emit_sub(SIGNED_IMM_TO_REG_ARG) {
        assert_imm_size<std::int64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(5), true, imm_mode::no_64);
    }

    void emit_sub(UNSIGNED_IMM_TO_REG_ARG) {
        assert_imm_size<std::uint64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(5), false, imm_mode::no_64);
    }
    ...
    void emit_xor(SIGNED_IMM_TO_REG_ARG) {
        assert_imm_size<std::int64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(6), true, imm_mode::no_64);
    }

    void emit_xor(UNSIGNED_IMM_TO_REG_ARG) {
        assert_imm_size<std::uint64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(6), false, imm_mode::no_64);
    }
    ...
    void emit_cmp(SIGNED_IMM_TO_REG_ARG) {
        assert_imm_size<std::int64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(7), true, imm_mode::no_64);
    }

    void emit_cmp(UNSIGNED_IMM_TO_REG_ARG) {
        assert_imm_size<std::uint64_t>(imm);
        emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest, imm,
                     static_cast<rm_field>(7), false, imm_mode::no_64);
    }
    ...
    /*
        quick note, this library does not support direct memory addresses, you have to load it into
       a register first then use `operation operand, [register]` for example
        ...
        from what i know, this is specific with LEA and perhaps CALL, but its fine i guess, we can't
       really call with 64 bit addresses anyways?
    */
    ...
    void emit_rol(SIGNED_IMM_TO_REG_ARG) { // no check for size here, just goes to imm8 static cast
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::int8_t>(imm), static_cast<rm_field>(0), true, imm_mode::do_8);
    }

    void emit_rol(UNSIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::uint8_t>(imm), static_cast<rm_field>(0), false,
                     imm_mode::do_8);
    }

    void emit_ror(SIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::int8_t>(imm), static_cast<rm_field>(1), true, imm_mode::do_8);
    }

    void emit_ror(UNSIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::uint8_t>(imm), static_cast<rm_field>(1), false,
                     imm_mode::do_8);
    }

    void emit_rcl(SIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::int8_t>(imm), static_cast<rm_field>(2), true, imm_mode::do_8);
    }

    void emit_rcl(UNSIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::uint8_t>(imm), static_cast<rm_field>(2), false,
                     imm_mode::do_8);
    }

    void emit_rcr(SIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::int8_t>(imm), static_cast<rm_field>(3), true, imm_mode::do_8);
    }

    void emit_rcr(UNSIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::uint8_t>(imm), static_cast<rm_field>(3), false,
                     imm_mode::do_8);
    }
    ...
    void emit_shl(SIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::int8_t>(imm), static_cast<rm_field>(4), true, imm_mode::do_8);
    }

    /* do note that SAL and SHL are the SAME logical operation */
    void emit_shl(UNSIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::uint8_t>(imm), static_cast<rm_field>(4), false,
                     imm_mode::do_8);
    }

    void emit_shr(SIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::int8_t>(imm), static_cast<rm_field>(5), true, imm_mode::do_8);
    }

    void emit_shr(UNSIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::uint8_t>(imm), static_cast<rm_field>(5), false,
                     imm_mode::do_8);
    }
    ...
    void emit_sal(SIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::int8_t>(imm), static_cast<rm_field>(6), true, imm_mode::do_8);
    }

    /* do note that SAL and SHL are the SAME logical operation */
    void emit_sal(UNSIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::uint8_t>(imm), static_cast<rm_field>(6), false,
                     imm_mode::do_8);
    }

    void emit_sar(SIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::int8_t>(imm), static_cast<rm_field>(7), true, imm_mode::do_8);
    }

    void emit_sar(UNSIGNED_IMM_TO_REG_ARG) {
        emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                     static_cast<std::uint8_t>(imm), static_cast<rm_field>(7), false,
                     imm_mode::do_8);
    }
    ...
    void emit_shl(REG_ARG) {
        emit_reg_to_reg(opcode::SHR_rm8_CL, opcode::SHR_rm16_to_64_CL, _reg, static_cast<grp>(4),
                        false);
    }

    void emit_shr(REG_ARG) {
        emit_reg_to_reg(opcode::SHR_rm8_CL, opcode::SHR_rm16_to_64_CL, _reg, static_cast<grp>(5),
                        false);
    }
    ...
    void emit_addss(SIMD128_TO_SIMD128_ARG) {
        push_bytes({kREP_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(ADDSS_xmm_to_xmm_or_m32),
                    modrm(mod_field::register_direct, dest, base)});
    }
    ...
    void emit_subss(SIMD128_TO_SIMD128_ARG) {
        push_bytes({kREP_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(SUBSS_xmm_to_xmm_or_m32),
                    modrm(mod_field::register_direct, dest, base)});
    }
    ...
    void emit_mulss(SIMD128_TO_SIMD128_ARG) {
        push_bytes({kREP_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(MULSS_xmm_to_xmm_or_m32),
                    modrm(mod_field::register_direct, dest, base)});
    }
    ...
    void emit_divss(SIMD128_TO_SIMD128_ARG) {
        push_bytes({kREP_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(DIVSS_xmm_to_xmm_or_m32),
                    modrm(mod_field::register_direct, dest, base)});
    }

    void emit_cvttsd2si(REG_TO_SIMD128_ARG) {
        emit_reg_to_simd128(CVTTSD2SI_xmm_or_m64_to_r32_to_64, static_cast<grp>(base),
    ...
    void emit_cvttss2si(REG_TO_SIMD128_ARG) {
        emit_reg_to_simd128(CVTTSD2SI_xmm_or_m64_to_r32_to_64, static_cast<grp>(dest),
    ...
    void emit_movss(SIMD128_TO_SIMD128_ARG) {
        ...
        push_bytes({kREP_Prefix, k2ByteOpcodePrefix, static_cast<uint8_t>(MOVSD_xmm_or_m64_to_xmm),
    ...
    void emit_movss(SIMD128_TO_MEM_ARG) {
        emit_simd128_to_mem(static_cast<opcode>(MOVSS_xmm_to_xmm_or_m32), base, dest,
                            {kREP_Prefix});
    }

    void emit_movss(MEM_TO_SIMD128_ARG) {
        emit_simd128_to_mem(static_cast<opcode>(MOVSS_xmm_or_m32_to_xmm), dest, base,
                            {kREP_Prefix});
    }

    void emit_movsd(SIMD128_TO_MEM_ARG) {
        emit_simd128_to_mem(static_cast<opcode>(MOVSD_xmm_to_xmm_or_m64), base, dest,
                            {kREPNE_Prefix});
    }

    void emit_movsd(MEM_TO_SIMD128_ARG) {
        emit_simd128_to_mem(static_cast<opcode>(MOVSD_xmm_or_m64_to_xmm), dest, base,
                            {kREPNE_Prefix});
    }

    void emit_fmod_double(simd128 result, simd128 x, simd128 y, simd128 temp1, simd128 temp2,
                          grp temp_gpr) {
        emit_movsd(temp1, x);
        emit_divsd(temp1, y);
    ...
    void emit_fmod_float(simd128 result, simd128 x, simd128 y, simd128 temp1, simd128 temp2,
                         grp temp_gpr) {
        emit_movss(temp1, x);
        emit_divss(temp1, y);
    ...
    void emit_comisd(SIMD128_TO_SIMD128_ARG) {
        push_bytes({operand_size_override, k2ByteOpcodePrefix, static_cast<uint8_t>(0x2F),
                    modrm(mod_field::register_direct, dest, base)});
    }
};
} // namespace occult::x86_64
