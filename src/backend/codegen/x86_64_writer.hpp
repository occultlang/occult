...
#define CHECK_2BYTE_OPCODE_PREFIX_AND_PUSH                                     \
  ...
  void check_simd128_r2m(const float_opcode_2b &op, const mem &dest,
                         const simd128 &src) {
    return;
  }
  ...
  void emit_simd128_to_mem(const opcode &op, const simd128 &base,
    ...
    if (dest.reg == rip) { // rip relative
      check_reg_r2m_simd(op, op, dest, static_cast<grp>(base), is_2bOPC,
                         prefix);
  ...
  void emit_reg_to_mem(const opcode &op8, const opcode &op, const mem &dest,
    ...
    if (dest.reg == rip) { // rip relative
      ...
      emit_imm32(dest.disp);
               ...
               dest.mode == mem_mode::disp) { // [rsp + disp32]
      check_reg_r2m(op8, op, dest, base, is_2bOPC);
      push_byte(modrm(mod_field::disp32, rm_field::indexed,
                      rebase_register(dest.reg)));
      push_byte(sib(0, kSpecialSIBIndex, rebase_register(dest.reg)));
      emit_imm32(dest.disp);
    } else if (dest.mode == mem_mode::scaled_index &&
...
public:
  ...
  */
  ...
  void emit_addsd(SIMD128_TO_SIMD128_ARG) {
    push_bytes({kREPNE_Prefix, k2ByteOpcodePrefix,
  ...
  void emit_addss(SIMD128_TO_SIMD128_ARG) {
    push_bytes({kREP_Prefix, k2ByteOpcodePrefix,
  ...
  void emit_cvttsd2si(REG_TO_SIMD128_ARG) {
    emit_reg_to_simd128(CVTTSD2SI_xmm_or_m64_to_r32_to_64,
                        static_cast<grp>(base), static_cast<simd128>(dest),
                        {kREPNE_Prefix});
  }

  void emit_cvtsi2sd(SIMD128_TO_REG_ARG) {
    emit_reg_to_simd128(CVTSI2SD_rm32_to_64_to_xmm, static_cast<grp>(base),
                        static_cast<simd128>(dest), {kREPNE_Prefix});
  }

  void emit_cvttss2si(REG_TO_SIMD128_ARG) {
    emit_reg_to_simd128(CVTTSD2SI_xmm_or_m64_to_r32_to_64,
                        static_cast<grp>(dest), static_cast<simd128>(base),
                        {kREP_Prefix});
  }

  void emit_cvtsi2ss(SIMD128_TO_REG_ARG) {
    emit_reg_to_simd128(CVTSI2SD_rm32_to_64_to_xmm, static_cast<grp>(dest),
                        static_cast<simd128>(base), {kREP_Prefix});
  }
  ...
  void emit_movss(SIMD128_TO_SIMD128_ARG) {
    ...
    push_bytes({kREP_Prefix, k2ByteOpcodePrefix,
                ...
                modrm(mod_field::register_direct, true_dest, true_base)});
  }
  ...
  void emit_comisd(SIMD128_TO_SIMD128_ARG) {
    push_bytes({operand_size_override, k2ByteOpcodePrefix,
