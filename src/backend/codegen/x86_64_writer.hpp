...
#define IS_STACK_PTR(dest)                                                     \
  (dest == rsp || dest == esp || dest == sp || dest == spl || dest == r12 ||   \
   dest == r12d || dest == r12w || dest == r12b)
#define NOT_STACK_PTR(dest) (!IS_STACK_PTR(dest))

#define IS_STACK_BASE_PTR(dest)                                                \
  (dest == rbp || dest == ebp || dest == bp || dest == bpl || dest == r13 ||   \
   dest == r13d || dest == r13w || dest == r13b)
...
struct mem {
  ...
  explicit mem(const grp &reg) : reg(reg), scale(0), index(), disp(0) {
    mode = mem_mode::reg;
  }

  explicit mem(const grp &reg, const std::int32_t &disp)
      : reg(reg), scale(0), index(), disp(disp) {
    mode = mem_mode::disp;
  }

  explicit mem(const grp &reg, const grp &index)
...
#define SIGNED_IMM_TO_REG_ARG                                                  \
  const IsGrp auto &dest, const IsSignedImm auto &imm
#define UNSIGNED_IMM_TO_REG_ARG                                                \
  const IsGrp auto &dest, const IsUnsignedImm auto &imm
#define SIGNED_IMM_TO_MEM_ARG                                                  \
  const IsMem auto &dest, const IsSignedImm auto &imm
#define UNSIGNED_IMM_TO_MEM_ARG                                                \
  const IsMem auto &dest, const IsUnsignedImm auto &imm
...
#define SIMD128_TO_SIMD128_ARG                                                 \
  const IsSimd128 auto &dest, const IsGrp auto &base
...
class x86_64_writer : public writer {
  ...
  void emit_imm(IntType imm, const int size) {
    for (int i = 0; i < size; ++i) {
      push_byte(static_cast<uint8_t>(
          (static_cast<std::uint64_t>(imm) >> (i * 8)) & 0xFF));
    }
  }
  ...
  template <typename IntType = std::int16_t> void emit_imm16(IntType imm) {
    emit_imm<IntType>(imm, 2);
  }

  template <typename IntType = std::int32_t> void emit_imm32(IntType imm) {
    emit_imm<IntType>(imm, 4);
  }

  template <typename IntType = std::int64_t> void emit_imm64(IntType imm) {
...
#define CHECK_2BYTE_OPCODE_PREFIX_AND_PUSH                                     \
  ...
  void emit_reg_to_reg(const opcode &op8, const opcode &op, const grp &dest,
                       const grp &base, const bool is_2bOPC = false) {
    if (REG_RANGE(dest, r8b, r15b) || REG_RANGE(dest, al, dil) ||
      ...
      push_byte(op8);
      push_byte(modrm(mod_field::register_direct, rebase_register(dest),
                      rebase_register(base)));
  ...
  void emit_reg_to_simd128(const float_opcode_2b &op, const grp &dest,
                           const simd128 &base,
                           std::vector<std::uint8_t> prefix_special_2b,
                           const bool is_2bOPC = true) {
    ...
    if (REG_RANGE(dest, r8, r15)) {
      ...
      push_byte(simd_ext ? static_cast<std::uint8_t>(
                               static_cast<std::uint8_t>(rex_b) | rex_bits::r)
                         : rex_b);
    } else if (simd_ext) {
      push_byte(rex_r);
    }
  ...
  /*void check_simd128_r2m(const float_opcode_2b& op, const mem& dest, const
  simd128& src) { return;
  }*/
  ...
  void print_modrm_debug(uint8_t modrm_byte, uint8_t reg_field,
                         uint8_t rm_field, uint8_t mod) {
    ...
    std::cout << std::bitset<3>(rm_field) << " = 0x" << std::hex
              << std::uppercase << +modrm_byte << std::dec << "\n";
  }

  void emit_simd128_to_mem(const opcode &op, const simd128 &base,
                           const mem &dest,
    ...
    if (dest.reg == rip) { // rip relative
    ...
    } else if (NOT_STACK_PTR(dest.reg) && NOT_STACK_BASE_PTR(dest.reg) &&
               dest.mode == mem_mode::reg) {
      ...
      push_byte(0);
    } else if (IS_STACK_BASE_PTR(dest.reg) && NOT_STACK_PTR(dest.reg) &&
               dest.mode == mem_mode::reg) {
      // [rbp]
      check_reg_r2m_simd(op, op, dest, static_cast<grp>(base), is_2bOPC, prefix,
                         simd_ext);
      push_byte(modrm(mod_field::disp8, rebase_register(dest.reg), base));
      emit_imm8(0);
    } else if (NOT_STACK_BASE_PTR(dest.reg) && IS_STACK_PTR(dest.reg) &&
               dest.mode == mem_mode::reg) {
      // [rsp]
      check_reg_r2m_simd(op, op, dest, static_cast<grp>(base), is_2bOPC, prefix,
                         simd_ext);
      ...
      emit_imm32(dest.disp);
    } else if (IS_STACK_PTR(dest.reg) &&
               dest.mode == mem_mode::disp) { // [rsp + disp32]
      check_reg_r2m_simd(op, op, dest, static_cast<grp>(base), is_2bOPC, prefix,
                         simd_ext);
      push_byte(modrm(mod_field::disp32, rm_field::indexed, base));
      push_byte(sib(0, kSpecialSIBIndex, rebase_register(dest.reg)));
      emit_imm32(dest.disp);
    } else if (dest.mode == mem_mode::scaled_index &&
               dest.second_mode == mem_mode::none) { // [reg + SIB]
      check_reg_r2m_simd(op, op, dest, static_cast<grp>(base), is_2bOPC, prefix,
                         simd_ext);
      push_byte(modrm(mod_field::indirect, rm_field::indexed, base));
      push_byte(sib(dest.scale, dest.index, rebase_register(dest.reg)));
    } else if (dest.mode == mem_mode::scaled_index &&
               dest.second_mode == mem_mode::disp) {
      // [reg + SIB + disp]
      check_reg_r2m_simd(op, op, dest, static_cast<grp>(base), is_2bOPC, prefix,
                         simd_ext);
      push_byte(modrm(mod_field::disp32, rm_field::indexed, base));
  ...
  void check_reg_r2m_simd(const opcode &op8, const opcode &op, const mem &dest,
                          const grp &base, const bool is_2bOPC = false,
                          const std::vector<std::uint8_t> &prefix = {},
                          bool simd_extended = false) {
    if (debug) {
    ...
    const bool index_r8q =
        dest.mode == mem_mode::scaled_index && REG_RANGE(dest.index, r8, r15);
    const bool index_64 =
        dest.mode == mem_mode::scaled_index && REG_RANGE(dest.index, rax, rdi);
    ...
    if (dest_r8b || dest_8 || base_r8b || base_8) {
      ...
      if (index_r8q)
        rex |= rex_bits::x;
      if (dest.reg == spl || dest.reg == bpl || dest.reg == sil ||
          dest.reg == dil || base == spl || base == bpl || base == sil ||
          base == dil) {
        needs_rex = true;
      }
    ...
    if (dest.reg == sp || dest.reg == esp || dest.reg == rsp ||
        dest.reg == spl || base == sp || base == esp || base == rsp ||
        base == spl) {
      if (dest.reg == sp || base == sp) {
        push_byte(other_prefix::operand_size_override);
      } else if (dest.reg == rsp || dest.reg == spl || base == rsp ||
                 base == spl) {
        rex |= rex_bits::w;
        if (simd_extended)
          rex |= rex_bits::r;
    ...
    if (simd_extended) {
      push_byte(static_cast<std::uint8_t>(static_cast<std::uint8_t>(rex::rex) |
                                          rex_bits::r));
    }
    emit_2b();
    push_byte(op);
  ...
  void check_reg_r2m(const opcode &op8, const opcode &op, const mem &dest,
                     const grp &base, const bool is_2bOPC = false) {
    auto emit_2b = [&]() {
      if (is_2bOPC)
        push_byte(k2ByteOpcodePrefix);
  ...
  void emit_reg_to_mem(const opcode &op8, const opcode &op, const mem &dest,
                       const grp &base, const bool is_2bOPC = false) {
    if (dest.reg == rip) { // rip relative
      check_reg_r2m(op8, op, dest, base, is_2bOPC);
      push_byte(modrm(mod_field::indirect, rm_field::rip_relative,
                      rebase_register(base)));
      emit_imm32(dest.disp);
    } else if (NOT_STACK_PTR(dest.reg) && NOT_STACK_BASE_PTR(dest.reg) &&
               dest.mode == mem_mode::reg) {
      ...
      if (debug)
        std::cout << "Emitting register based addressing for memory\n\tDest: "
                  << reg_to_string(base) << "\n\tSource: ["
                  << reg_to_string(dest.reg) << "]\n\tMode = mem_mode::reg;\n";

      check_reg_r2m(op8, op, dest, base, is_2bOPC);

      if (debug)
        std::cout << "Rebased (Source): "
                  << reg_to_string(rebase_register(base)) << std::endl;

      if (debug)
        std::cout << "Rebased (Dest): "
                  << reg_to_string(rebase_register(dest.reg)) << std::endl;

      push_byte(modrm(mod_field::disp8, rebase_register(dest.reg),
                      rebase_register(base)));
      push_byte(0);
    } else if (IS_STACK_BASE_PTR(dest.reg) && NOT_STACK_PTR(dest.reg) &&
               dest.mode == mem_mode::reg) {
      ...
      check_reg_r2m(op8, op, dest, base, is_2bOPC);
      push_byte(
          modrm(mod_field::indirect, rm_field::indexed, rebase_register(base)));
      push_byte(sib(0, kSpecialSIBIndex, rebase_register(dest.reg)));
    } else if (NOT_STACK_PTR(dest.reg) && dest.mode == mem_mode::disp) {
      // [reg + disp32] (we are always going to do disp32)
      check_reg_r2m(op8, op, dest, base, is_2bOPC);
      push_byte(modrm(mod_field::disp32, rebase_register(dest.reg),
                      rebase_register(base)));
      emit_imm32(dest.disp);
    } else if (IS_STACK_PTR(dest.reg) &&
               dest.mode == mem_mode::disp) { // [rsp + disp32]
      check_reg_r2m(op8, op, dest, base, is_2bOPC);
      push_byte(
          modrm(mod_field::disp32, rm_field::indexed, rebase_register(base)));
      push_byte(sib(0, kSpecialSIBIndex, rebase_register(dest.reg)));
      emit_imm32(dest.disp);
    } else if (dest.mode == mem_mode::scaled_index &&
               dest.second_mode == mem_mode::none) { // [reg + SIB]
      ...
      if (IS_STACK_BASE_PTR(dest.reg)) {
        ...
        push_byte(modrm(mod_field::indirect, rm_field::indexed,
                        rebase_register(base)));
        push_byte(sib(dest.scale, dest.index, rebase_register(dest.reg)));
      }
    } else if (dest.mode == mem_mode::scaled_index &&
  ...
  void emit_reg_imm(const opcode &op8, const opcode &op, const grp &dest,
                    const T &imm, const rm_field &rm,
                    const bool is_signed = true,
                    const imm_mode im_m = imm_mode::normal) {
    if (REG_RANGE(dest, r8b, r15b) || REG_RANGE(dest, al, dil)) {
  ...
  template <std::integral T>
  void emit_reg_imm_basic(const opcode &op8, const opcode &op, const grp &dest,
                          const T &imm, bool is_signed = true) {
    if (REG_RANGE(dest, r8b, r15b) || REG_RANGE(dest, al, dil)) {
      if (REG_RANGE(dest, r8b, r15b)) {
        push_byte(rex_b);
      }

      push_byte(static_cast<std::uint8_t>(op8) +
                static_cast<std::uint8_t>(rebase_register(dest)));
      ...
      push_byte(static_cast<std::uint8_t>(op) +
                static_cast<std::uint8_t>(rebase_register(dest)));
  ...
  // future i guess lmao
  template <std::integral T>
  void emit_mem_imm(const opcode &op8, const opcode &op, const mem &dest,
                    const T &imm, const rm_field &rm,
                    const bool is_signed = true,
                    const imm_mode im_m = imm_mode::normal) {
    if (dest.reg == rip) { // rip relative
      throw std::invalid_argument(
          "Can't move immediate to rip-relative address.");
    }
    if (NOT_STACK_PTR(dest.reg) && NOT_STACK_BASE_PTR(dest.reg) &&
        dest.mode == mem_mode::reg) {
      // [reg]
  ...
  void emit_reg(const opcode &op8, const opcode &op, const grp &r) {
    if (REG_RANGE(r, r8b, r15b) || REG_RANGE(r, al, dil)) {
      ...
      push_byte(static_cast<std::uint8_t>(op8) +
                static_cast<std::uint8_t>(rebase_register(r)));
  ...
  void emit_short_jump(const std::uint8_t opcode,
                       const std::uint8_t target_address) {
  ...
  void emit_near_jump(const std::uint8_t opcode,
  ...
  void emit_long_jump(const std::uint8_t opcode,
                      const std::uint32_t target_address) {
    const std::uint32_t current_address = get_code().size();
  ...
  void emit_setX_reg8(const opcode_2b &op, REG_ARG) { // remove the t ;)
    emit_reg_to_reg(static_cast<opcode>(op), static_cast<opcode>(op), _reg,
                    static_cast<grp>(0), true);
  }

  void emit_setX_mem8(const opcode_2b &op, MEM_ARG) {
    emit_reg_to_mem(static_cast<opcode>(op), static_cast<opcode>(op), m,
                    static_cast<grp>(0), true);
  }

  template <typename T, typename T2> static void assert_imm_size(const T2 &) {
    if (sizeof(T2) == sizeof(T)) {
      throw std::invalid_argument("Immediate value can not be of size (" +
                                  std::to_string(sizeof(T)) + " bytes)");
    }
  }

  template <typename T> bool imm_fits(std::int64_t imm) {
    return imm >= (std::numeric_limits<T>::min)() &&
           imm <= (std::numeric_limits<T>::max)();
  }
...
public:
  ...
  void emit_add(REG_TO_REG_ARG) {
    emit_reg_to_reg(opcode::ADD_r8_rm8, opcode::ADD_r16_to_64_rm16_to_64, base,
                    dest);
  }

  void emit_add(REG_TO_MEM_ARG) {
    emit_reg_to_mem(opcode::ADD_rm8_r8, opcode::ADD_rm16_to_64_r16_to_64, dest,
                    base);
  }

  void emit_add(MEM_TO_REG_ARG) {
    emit_reg_to_mem(opcode::ADD_r8_rm8, opcode::ADD_r16_to_64_rm16_to_64, base,
                    dest);
    // we can just use the same function apparently...?
  }

  void emit_or(REG_TO_REG_ARG) {
    emit_reg_to_reg(opcode::OR_r8_rm8, opcode::OR_r16_to_64_rm16_to_64, base,
                    dest);
  }

  void emit_or(REG_TO_MEM_ARG) {
    emit_reg_to_mem(opcode::OR_rm8_r8, opcode::OR_rm16_to_64_r16_to_64, dest,
                    base);
  }

  void emit_or(MEM_TO_REG_ARG) {
    emit_reg_to_mem(opcode::OR_r8_rm8, opcode::OR_r16_to_64_rm16_to_64, base,
                    dest);
  }

  void emit_adc(REG_TO_REG_ARG) {
    emit_reg_to_reg(opcode::ADC_r8_rm8, opcode::ADC_r16_to_64_rm16_to_64, base,
                    dest);
  }

  void emit_adc(REG_TO_MEM_ARG) {
    emit_reg_to_mem(opcode::ADC_rm8_r8, opcode::ADC_rm16_to_64_r16_to_64, dest,
                    base);
  }

  void emit_adc(MEM_TO_REG_ARG) {
    emit_reg_to_mem(opcode::ADC_r8_rm8, opcode::ADC_r16_to_64_rm16_to_64, base,
                    dest);
  }

  void emit_sbb(REG_TO_REG_ARG) {
    emit_reg_to_reg(opcode::SBB_r8_rm8, opcode::SBB_r16_to_64_rm16_to_64, base,
                    dest);
  }

  void emit_sbb(REG_TO_MEM_ARG) {
    emit_reg_to_mem(opcode::SBB_rm8_r8, opcode::SBB_rm16_to_64_r16_to_64, dest,
                    base);
  }

  void emit_sbb(MEM_TO_REG_ARG) {
    emit_reg_to_mem(opcode::SBB_r8_rm8, opcode::SBB_r16_to_64_rm16_to_64, base,
                    dest);
  }

  void emit_and(REG_TO_REG_ARG) {
    emit_reg_to_reg(opcode::AND_r8_rm8, opcode::AND_r16_to_64_rm16_to_64, base,
                    dest);
  }

  void emit_and(REG_TO_MEM_ARG) {
    emit_reg_to_mem(opcode::AND_rm8_r8, opcode::AND_rm16_to_64_r16_to_64, dest,
                    base);
  }

  void emit_and(MEM_TO_REG_ARG) {
    emit_reg_to_mem(opcode::AND_r8_rm8, opcode::AND_r16_to_64_rm16_to_64, base,
                    dest);
    // we can just use the same function apparently...?
  }

  void emit_sub(REG_TO_REG_ARG) {
    emit_reg_to_reg(opcode::SUB_r8_rm8, opcode::SUB_r16_to_64_rm16_to_64, base,
                    dest);
  }

  void emit_sub(REG_TO_MEM_ARG) {
    emit_reg_to_mem(opcode::SUB_rm8_r8, opcode::SUB_rm16_to_64_r16_to_64, dest,
                    base);
  }

  void emit_sub(MEM_TO_REG_ARG) {
    emit_reg_to_mem(opcode::SUB_r8_rm8, opcode::SUB_r16_to_64_rm16_to_64, base,
                    dest);
    // we can just use the same function apparently...?
  }

  void emit_xor(REG_TO_REG_ARG) {
    emit_reg_to_reg(opcode::XOR_r8_rm8, opcode::XOR_r16_to_64_rm16_to_64, base,
                    dest);
  }

  void emit_xor(REG_TO_MEM_ARG) {
    emit_reg_to_mem(opcode::XOR_rm8_r8, opcode::XOR_rm16_to_64_r16_to_64, dest,
                    base);
  }

  void emit_xor(MEM_TO_REG_ARG) {
    emit_reg_to_mem(opcode::XOR_r8_rm8, opcode::XOR_r16_to_64_rm16_to_64, base,
                    dest);
    // we can just use the same function apparently...?
  }

  void emit_cmp(REG_TO_REG_ARG) {
    emit_reg_to_reg(opcode::CMP_r8_rm8, opcode::CMP_r16_to_64_rm16_to_64, base,
                    dest);
  }

  void emit_cmp(REG_TO_MEM_ARG) {
    emit_reg_to_mem(opcode::CMP_rm8_r8, opcode::CMP_rm16_to_64_r16_to_64, dest,
                    base);
  }

  void emit_cmp(MEM_TO_REG_ARG) {
    emit_reg_to_mem(opcode::CMP_r8_rm8, opcode::CMP_r16_to_64_rm16_to_64, base,
                    dest);
    // we can just use the same function apparently...?
  }
  ...
  void emit_imul(const IsGrp auto &dest, const IsGrp auto &base,
                 const IsSignedImm auto &imm) {
    assert_imm_size<std::int64_t>(imm);
    emit_reg_imm(opcode::IMUL_r16_to_64_rm16_to_64_imm8,
                 opcode::IMUL_r16_to_64_rm16_to_64_imm16_to_32, base, imm,
                 static_cast<rm_field>(dest), true, imm_mode::no_64);
  }

  void emit_imul(const IsGrp auto &dest, const IsMem auto &base,
                 const IsSignedImm auto &imm) {
    assert_imm_size<std::int64_t>(imm);
    emit_mem_imm(opcode::IMUL_r16_to_64_rm16_to_64_imm8,
                 opcode::IMUL_r16_to_64_rm16_to_64_imm16_to_32, base, imm,
                 static_cast<rm_field>(dest), true, imm_mode::no_64);
  }

  void emit_imul(const IsGrp auto &dest, const IsGrp auto &base,
                 const IsUnsignedImm auto &imm) {
    assert_imm_size<std::uint64_t>(imm);
    emit_reg_imm(opcode::IMUL_r16_to_64_rm16_to_64_imm8,
                 opcode::IMUL_r16_to_64_rm16_to_64_imm16_to_32, base, imm,
                 static_cast<rm_field>(dest), false, imm_mode::no_64);
  }
  ...
  void emit_add(SIGNED_IMM_TO_REG_ARG) {
    ...
    assert_imm_size<std::int64_t>(imm);
    emit_reg_imm(opcode::ADD_rm8_imm8, opcode::ADD_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(0), true, imm_mode::no_64);
  }

  void emit_add(UNSIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::uint64_t>(imm);
    emit_reg_imm(opcode::ADD_rm8_imm8, opcode::ADD_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(0), false, imm_mode::no_64);
  }

  void emit_add(SIGNED_IMM_TO_MEM_ARG) {
    assert_imm_size<std::int64_t>(imm);
    emit_mem_imm(opcode::ADD_rm8_imm8, opcode::ADD_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(0), true, imm_mode::no_64);
  }
  ...
  void emit_or(SIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::int64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(1), true, imm_mode::no_64);
  }

  void emit_or(UNSIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::uint64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(1), false, imm_mode::no_64);
  }

  void emit_or(SIGNED_IMM_TO_MEM_ARG) {
    assert_imm_size<std::int64_t>(imm);
    emit_mem_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(1), true, imm_mode::no_64);
  }
  ...
  void emit_adc(SIGNED_IMM_TO_REG_ARG) {
    ...
    assert_imm_size<std::int64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(2), true, imm_mode::no_64);
  }

  void emit_adc(UNSIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::uint64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(2), false, imm_mode::no_64);
  }
  ...
  void emit_sbb(SIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::int64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(3), true, imm_mode::no_64);
  }

  void emit_sbb(UNSIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::uint64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(3), false, imm_mode::no_64);
  }
  ...
  void emit_and(SIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::int64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(4), true, imm_mode::no_64);
  }

  void emit_and(UNSIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::uint64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(4), false, imm_mode::no_64);
  }
  ...
  void emit_sub(SIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::int64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(5), true, imm_mode::no_64);
  }

  void emit_sub(UNSIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::uint64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(5), false, imm_mode::no_64);
  }
  ...
  void emit_xor(SIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::int64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(6), true, imm_mode::no_64);
  }

  void emit_xor(UNSIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::uint64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(6), false, imm_mode::no_64);
  }
  ...
  void emit_cmp(SIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::int64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(7), true, imm_mode::no_64);
  }

  void emit_cmp(UNSIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::uint64_t>(imm);
    emit_reg_imm(opcode::OR_rm8_imm8, opcode::OR_rm8_to_64_imm16_or_32, dest,
                 imm, static_cast<rm_field>(7), false, imm_mode::no_64);
  }
  ...
  void emit_test(REG_TO_REG_ARG) {
    emit_reg_to_reg(opcode::TEST_rm8_imm8, opcode::TEST_rm16_to_64_r16_to_64,
                    dest, base);
  }

  void emit_test(REG_TO_MEM_ARG) {
    emit_reg_to_mem(opcode::TEST_rm8_imm8, opcode::TEST_rm16_to_64_r16_to_64,
                    dest, base);
  }

  void emit_xchg(REG_TO_REG_ARG) {
    emit_reg_to_reg(opcode::XCHG_r8_rm8, opcode::XCHG_r16_to_64_rm16_to_64,
                    dest, base);
  }

  void emit_xchg(MEM_TO_REG_ARG) {
    emit_reg_to_mem(opcode::XCHG_r8_rm8, opcode::XCHG_r16_to_64_rm16_to_64,
                    base,
                    dest); // opposite still
  }

  void emit_mov(REG_TO_REG_ARG) {
    emit_reg_to_reg(opcode::MOV_rm8_r8, opcode::MOV_rm16_to_64_r16_to_64, dest,
                    base);
  }

  void emit_mov(REG_TO_MEM_ARG) {
    emit_reg_to_mem(opcode::MOV_rm8_r8, opcode::MOV_rm16_to_64_r16_to_64, dest,
                    base);
  }

  void emit_mov(MEM_TO_REG_ARG) {
    emit_reg_to_mem(opcode::MOV_r8_rm8, opcode::MOV_r16_to_64_rm16_to_64, base,
                    dest);
  }
  ...
  */
  void emit_lea(MEM_TO_REG_ARG) {
    emit_reg_to_mem(opcode::LEA_r16_to_64_mem, opcode::LEA_r16_to_64_mem, base,
                    dest);
  }

  void emit_pop(MEM_ARG) {
    emit_reg_to_mem(opcode::POP_rm16_to_64, opcode::POP_rm16_to_64, m,
                    static_cast<grp>(0));
  }
  ...
  void emit_mov(SIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm_basic(opcode::MOV_r8_imm8, opcode::MOV_r16_to_64_imm16_to_64,
                       dest, imm);
  }
  ...
  void emit_rol(SIGNED_IMM_TO_REG_ARG) { // no check for size here, just goes to
                                         // imm8 static cast
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::int8_t>(imm), static_cast<rm_field>(0), true,
                 imm_mode::do_8);
  }

  void emit_rol(UNSIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::uint8_t>(imm), static_cast<rm_field>(0),
                 false, imm_mode::do_8);
  }

  void emit_ror(SIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::int8_t>(imm), static_cast<rm_field>(1), true,
                 imm_mode::do_8);
  }

  void emit_ror(UNSIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::uint8_t>(imm), static_cast<rm_field>(1),
                 false, imm_mode::do_8);
  }

  void emit_rcl(SIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::int8_t>(imm), static_cast<rm_field>(2), true,
                 imm_mode::do_8);
  }

  void emit_rcl(UNSIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::uint8_t>(imm), static_cast<rm_field>(2),
                 false, imm_mode::do_8);
  }

  void emit_rcr(SIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::int8_t>(imm), static_cast<rm_field>(3), true,
                 imm_mode::do_8);
  }

  void emit_rcr(UNSIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::uint8_t>(imm), static_cast<rm_field>(3),
                 false, imm_mode::do_8);
  }
  ...
  void emit_shl(SIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::int8_t>(imm), static_cast<rm_field>(4), true,
                 imm_mode::do_8);
  }
  ...
  void emit_shl(UNSIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::uint8_t>(imm), static_cast<rm_field>(4),
                 false, imm_mode::do_8);
  }

  void emit_shr(SIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::int8_t>(imm), static_cast<rm_field>(5), true,
                 imm_mode::do_8);
  }

  void emit_shr(UNSIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::uint8_t>(imm), static_cast<rm_field>(5),
                 false, imm_mode::do_8);
  }
  ...
  void emit_sal(SIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::int8_t>(imm), static_cast<rm_field>(6), true,
                 imm_mode::do_8);
  }
  ...
  void emit_sal(UNSIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::uint8_t>(imm), static_cast<rm_field>(6),
                 false, imm_mode::do_8);
  }

  void emit_sar(SIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::int8_t>(imm), static_cast<rm_field>(7), true,
                 imm_mode::do_8);
  }

  void emit_sar(UNSIGNED_IMM_TO_REG_ARG) {
    emit_reg_imm(opcode::ROL_rm8_imm8, opcode::ROL_rm16_to_64_imm8, dest,
                 static_cast<std::uint8_t>(imm), static_cast<rm_field>(7),
                 false, imm_mode::do_8);
  }
  ...
  void emit_mov(SIGNED_IMM_TO_MEM_ARG) {
    ...
    if (REG_RANGE(dest.reg, al, dil) || REG_RANGE(dest.reg, r8b, r15b)) {
      emit_mem_imm(opcode::MOV_rm8_imm8, opcode::MOV_rm16_to_64_imm16_or_32,
                   dest, imm, static_cast<rm_field>(0), true, imm_mode::do_8);
    } else if (REG_RANGE(dest.reg, ax, di) || REG_RANGE(dest.reg, r8w, r15w)) {
      emit_mem_imm(opcode::MOV_rm8_imm8, opcode::MOV_rm16_to_64_imm16_or_32,
                   dest, imm, static_cast<rm_field>(0), true, imm_mode::do_16);
    } else {
      emit_mem_imm(opcode::MOV_rm8_imm8, opcode::MOV_rm16_to_64_imm16_or_32,
                   dest, imm, static_cast<rm_field>(0), true, imm_mode::no_64);
    }
  }
  ...
  void emit_mov(UNSIGNED_IMM_TO_MEM_ARG) {
    ...
    if (REG_RANGE(dest.reg, al, dil) || REG_RANGE(dest.reg, r8b, r15b)) {
      emit_mem_imm(opcode::MOV_rm8_imm8, opcode::MOV_rm16_to_64_imm16_or_32,
                   dest, imm, static_cast<rm_field>(0), false, imm_mode::do_8);
    } else if (REG_RANGE(dest.reg, ax, di) || REG_RANGE(dest.reg, r8w, r15w)) {
      emit_mem_imm(opcode::MOV_rm8_imm8, opcode::MOV_rm16_to_64_imm16_or_32,
                   dest, imm, static_cast<rm_field>(0), false, imm_mode::do_16);
    } else {
      emit_mem_imm(opcode::MOV_rm8_imm8, opcode::MOV_rm16_to_64_imm16_or_32,
                   dest, imm, static_cast<rm_field>(0), false, imm_mode::no_64);
    }
  }

  void emit_test(
      SIGNED_IMM_TO_REG_ARG) { // i think the rm_field could also be 0 for TEST
    assert_imm_size<std::int64_t>(imm);
    emit_reg_imm(opcode::TEST_rm8_imm8, opcode::TEST_rm16_to_64_imm16_or_32,
                 dest, imm, static_cast<rm_field>(1), true, imm_mode::no_64);
  }

  void emit_test(UNSIGNED_IMM_TO_REG_ARG) {
    assert_imm_size<std::uint64_t>(imm);
    emit_reg_imm(opcode::TEST_rm8_imm8, opcode::TEST_rm16_to_64_imm16_or_32,
                 dest, imm, static_cast<rm_field>(1), false, imm_mode::no_64);
  }

  void emit_test(SIGNED_IMM_TO_MEM_ARG) {
    assert_imm_size<std::int64_t>(imm);
    emit_mem_imm(opcode::TEST_rm8_imm8, opcode::TEST_rm16_to_64_imm16_or_32,
                 dest, imm, static_cast<rm_field>(1), true, imm_mode::no_64);
  }
  ...
  void emit_not(REG_ARG) {
    emit_reg_to_reg(opcode::NOT_rm8, opcode::NOT_rm16_to_64, _reg,
                    static_cast<grp>(2));
    // 2 is which operation
  }

  void emit_not(MEM_ARG) {
    emit_reg_to_mem(opcode::NOT_rm8, opcode::NOT_rm16_to_64, m,
                    static_cast<grp>(2));
  }
  ...
  void emit_mul(REG_ARG) {
    emit_reg_to_reg(opcode::MUL_AX_AL_rm8, opcode::MUL_rDX_rAX_rm16_to_64, _reg,
                    static_cast<grp>(4));
  }

  void emit_mul(MEM_ARG) {
    emit_reg_to_mem(opcode::MUL_AX_AL_rm8, opcode::MUL_rDX_rAX_rm16_to_64, m,
                    static_cast<grp>(4));
  }
  ...
  void emit_xorps(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix,
                static_cast<uint8_t>(XORPS_xmm_to_xmm_or_m128),
                modrm(mod_field::register_direct, dest, base)});
  }

  void emit_inc(REG_ARG) {
    emit_reg_to_reg(opcode::INC_rm8, opcode::INC_rm16_to_64, _reg,
                    static_cast<grp>(0));
  }

  void emit_inc(MEM_ARG) {
    emit_reg_to_mem(opcode::INC_rm8, opcode::INC_rm16_to_64, m,
                    static_cast<grp>(0));
  }

  void emit_dec(REG_ARG) {
    emit_reg_to_reg(opcode::DEC_rm8, opcode::INC_rm16_to_64, _reg,
                    static_cast<grp>(1));
  }

  void emit_dec(MEM_ARG) {
    emit_reg_to_mem(opcode::DEC_rm8, opcode::INC_rm16_to_64, m,
                    static_cast<grp>(1));
  }

  void emit_call(REG_ARG) { // we will not support 64
    emit_reg_to_reg(opcode::CALL_rm16_or_32, opcode::CALL_rm16_or_32, _reg,
                    static_cast<grp>(2));
  }

  void emit_call(MEM_ARG) {
    emit_reg_to_mem(opcode::CALL_rm16_or_32, opcode::CALL_rm16_or_32, m,
                    static_cast<grp>(2));
  }

  void emit_jmp(REG_ARG) {
    emit_reg_to_reg(opcode::JMP_rm16_or_32, opcode::JMP_rm16_or_32, _reg,
                    static_cast<grp>(4));
  }

  void emit_jmp(MEM_ARG) {
    emit_reg_to_mem(opcode::JMP_rm16_or_32, opcode::JMP_rm16_or_32, m,
                    static_cast<grp>(4));
  }

  void emit_push(MEM_ARG) {
    emit_reg_to_mem(opcode::PUSH_rm16_or_32, opcode::PUSH_rm16_or_32, m,
                    static_cast<grp>(6));
  }
  ...
  void emit_shl(REG_ARG) {
    emit_reg_to_reg(opcode::SHR_rm8_CL, opcode::SHR_rm16_to_64_CL, _reg,
                    static_cast<grp>(4), false);
  }

  void emit_shr(REG_ARG) {
    emit_reg_to_reg(opcode::SHR_rm8_CL, opcode::SHR_rm16_to_64_CL, _reg,
                    static_cast<grp>(5), false);
  }

  void emit_sar(REG_ARG) {
    emit_reg_to_reg(opcode::SAR_rm8_CL, opcode::SAR_rm16_to_64_CL, _reg,
                    static_cast<grp>(7), false);
  }
  ...
  void emit_movzx(REG_TO_REG_ARG, bool is_8 = true) {
    if (is_8) {
      emit_reg_to_reg(static_cast<opcode>(opcode_2b::MOVZX_r16_to_64_rm8),
                      static_cast<opcode>(opcode_2b::MOVZX_r16_to_64_rm8), base,
                      dest, true);
    } else {
      emit_reg_to_reg(static_cast<opcode>(opcode_2b::MOVZX_r16_to_64_rm16),
                      static_cast<opcode>(opcode_2b::MOVZX_r16_to_64_rm16),
                      base, dest, true);
    }
  }
  ...
  void emit_movsx(REG_TO_REG_ARG, bool is_8 = true) {
    if (is_8) {
      emit_reg_to_reg(static_cast<opcode>(opcode_2b::MOVSX_r16_to_64_rm8),
                      static_cast<opcode>(opcode_2b::MOVSX_r16_to_64_rm8), base,
                      dest, true);
    } else {
      emit_reg_to_reg(static_cast<opcode>(opcode_2b::MOVSX_r16_to_64_rm16),
                      static_cast<opcode>(opcode_2b::MOVSX_r16_to_64_rm16),
                      base, dest, true);
    }
  }
  ...
  void emit_movsxd(REG_TO_REG_ARG) {
    emit_reg_to_reg(opcode::MOVSXD_r64_or_32_rm32,
                    opcode::MOVSXD_r64_or_32_rm32, base, dest);
  }

  /* 32 to 64 */
  void emit_movsxd(MEM_TO_REG_ARG) {
    emit_reg_to_mem(opcode::MOVSXD_r64_or_32_rm32,
                    opcode::MOVSXD_r64_or_32_rm32, base, dest);
  }

  // move value into sse registers from GRP (single or double precision)
  void emit_movq(REG_TO_SIMD128_ARG) {
    emit_reg_to_simd128(float_opcode_2b::MOVQ_xmm_to_rm32_to_64, base, dest,
                        {operand_size_override});
  }

  // move value from sse register to GRP (bit-preserving, 66 REX.W 0F 7E /r)
  void emit_movq(const grp &dest, const simd128 &base) {
    emit_reg_to_simd128(float_opcode_2b::MOVQ_rm64_from_xmm, dest, base,
                        {operand_size_override});
  }

  // move value into sse registers from GRP (single or double precision)
  void emit_movd(REG_TO_SIMD128_ARG) {
    emit_reg_to_simd128(float_opcode_2b::MOVD_xmm_to_rm32_to_64, base, dest,
                        {operand_size_override});
  }
  ...
  void emit_addsd(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix,
                static_cast<uint8_t>(ADDSD_xmm_to_xmm_or_m64),
                modrm(mod_field::register_direct, dest, base)});
  }
  ...
  void emit_addss(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix,
                static_cast<uint8_t>(ADDSS_xmm_to_xmm_or_m32),
                modrm(mod_field::register_direct, dest, base)});
  }
  ...
  void emit_subsd(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix,
                static_cast<uint8_t>(SUBSD_xmm_to_xmm_or_m64),
                modrm(mod_field::register_direct, dest, base)});
  }
  ...
  void emit_subss(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix,
                static_cast<uint8_t>(SUBSS_xmm_to_xmm_or_m32),
                modrm(mod_field::register_direct, dest, base)});
  }
  ...
  void emit_mulsd(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix,
                static_cast<uint8_t>(MULSD_xmm_to_xmm_or_m64),
                modrm(mod_field::register_direct, dest, base)});
  }
  ...
  void emit_mulss(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix,
                static_cast<uint8_t>(MULSS_xmm_to_xmm_or_m32),
                modrm(mod_field::register_direct, dest, base)});
  }
  ...
  void emit_divsd(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix,
                static_cast<uint8_t>(DIVSD_xmm_to_xmm_or_m64),
                modrm(mod_field::register_direct, dest, base)});
  }
  ...
  void emit_divss(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix,
                static_cast<uint8_t>(DIVSS_xmm_to_xmm_or_m32),
                modrm(mod_field::register_direct, dest, base)});
  }
  ...
  void emit_cvttsd2si(const grp &dst, const simd128 &src) {
    ...
    push_bytes(
        {kREPNE_Prefix, rex, k2ByteOpcodePrefix,
         static_cast<std::uint8_t>(CVTTSD2SI_xmm_or_m64_to_r32_to_64),
         static_cast<std::uint8_t>((0b11 << 6) | (dst_val << 3) | src_val)});
  }
  ...
  void emit_cvtsi2sd(const simd128 &dst, const grp &src) {
    ...
    push_bytes(
        {kREPNE_Prefix, rex, k2ByteOpcodePrefix,
         static_cast<std::uint8_t>(CVTSI2SD_rm32_to_64_to_xmm),
         static_cast<std::uint8_t>((0b11 << 6) | (dst_val << 3) | src_val)});
  }
  ...
  void emit_cvttss2si(const grp &dst, const simd128 &src) {
    ...
    if (static_cast<std::uint8_t>(src) >= 8)
      rex |= 0x01; // REX.B extends xmm8-15
    push_bytes(
        {kREP_Prefix, rex, k2ByteOpcodePrefix,
         static_cast<std::uint8_t>(CVTTSD2SI_xmm_or_m64_to_r32_to_64),
         static_cast<std::uint8_t>((0b11 << 6) | (dst_val << 3) | src_val)});
  }
  ...
  void emit_cvtss2sd(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix,
                static_cast<uint8_t>(CVTSS2SD_xmm_or_m32_to_xmm),
                modrm(mod_field::register_direct, dest, base)});
  }
  ...
  void emit_cvtsd2ss(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix,
                static_cast<uint8_t>(CVTSD2SS_xmm_or_m64_to_xmm),
                modrm(mod_field::register_direct, dest, base)});
  }
  ...
  void emit_movss(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix,
                static_cast<uint8_t>(MOVSD_xmm_or_m64_to_xmm),
                modrm(mod_field::register_direct, dest, base)});
  }
  ...
  void emit_movss(SIMD128_TO_MEM_ARG) {
    emit_simd128_to_mem(static_cast<opcode>(MOVSS_xmm_to_xmm_or_m32), base,
                        dest, {kREP_Prefix});
  }

  void emit_movss(MEM_TO_SIMD128_ARG) {
    emit_simd128_to_mem(static_cast<opcode>(MOVSS_xmm_or_m32_to_xmm), dest,
                        base, {kREP_Prefix});
  }

  void emit_movsd(SIMD128_TO_MEM_ARG) {
    emit_simd128_to_mem(static_cast<opcode>(MOVSD_xmm_to_xmm_or_m64), base,
                        dest, {kREPNE_Prefix});
  }

  void emit_movsd(MEM_TO_SIMD128_ARG) {
    emit_simd128_to_mem(static_cast<opcode>(MOVSD_xmm_or_m64_to_xmm), dest,
                        base, {kREPNE_Prefix});
  }

  void emit_fmod_double(simd128 result, simd128 x, simd128 y, simd128 temp1,
                        simd128 temp2, grp temp_gpr) {
    emit_movsd(temp1, x);
    emit_divsd(temp1, y);
  ...
  void emit_fmod_float(simd128 result, simd128 x, simd128 y, simd128 temp1,
                       simd128 temp2, grp temp_gpr) {
    emit_movss(temp1, x);
    emit_divss(temp1, y);
  ...
  void emit_comiss(SIMD128_TO_SIMD128_ARG) {
    ...
    if (rex)
      push_byte(rex);
    push_bytes({k2ByteOpcodePrefix, static_cast<uint8_t>(0x2F),
                modrm(mod_field::register_direct, dest, base)});
  }
