...
namespace occult::x86_64 {
    ...
    struct mem {
        ...
        explicit mem(const grp &reg) :
            reg(reg), scale(0), index(), disp(0) { mode = mem_mode::reg; }

        explicit mem(const grp &reg, const std::int32_t &disp) :
            reg(reg), scale(0), index(), disp(disp) { mode = mem_mode::disp; }

        explicit mem(const grp &reg, const grp &index) :
            reg(reg), scale(0), index(index), disp(0) { mode = mem_mode::scaled_index; }
...
#define CHECK_2BYTE_OPCODE_PREFIX_AND_PUSH if (is_2bOPC) { push_byte(k2ByteOpcodePrefix); }
        ...
        void emit_reg_imm(const opcode &op8, const opcode &op, const grp &dest, const T &imm, const rm_field &rm,
                          const bool is_signed = true, const imm_mode im_m = imm_mode::normal) {
            if (REG_RANGE(dest, r8b, r15b) || REG_RANGE(dest, al, dil)) {
        ...
        void emit_reg(const opcode &op8, const opcode &op, const grp &r) {
            if (REG_RANGE(r, r8b, r15b) || REG_RANGE(r, al, dil)) {
                if (REG_RANGE(r, r8b, r15b)) { push_byte(rex_rb); }
                else
