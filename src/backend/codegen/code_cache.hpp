...
inline std::string cache_dir_path() {
  return std::string(kCacheRootDir) + cache_namespace_id() + "/";
}

inline std::string cache_path_for(const std::string &func_name) {
  return cache_dir_path() + func_name + ".bin";
}
...
bytecode_header generate_bytecode_header(const ir_function &func,
                                         arch target_arch = arch::x86_64) {
  ...
  header.code_offset =
      sizeof(bytecode_header); // code starts right after header
...
void write_cached_code(
    const ir_function &func,
    const std::map<std::string, std::vector<std::uint8_t>>
        &function_raw_code_map,
    const std::unordered_map<std::string, jit_function> &function_map,
  ...
  if (code_it == function_raw_code_map.end()) {
    throw std::runtime_error(
        "write_cached_code: missing machine code for function " + func.name);
  }
  ...
  for (const auto &[name, fn] : function_map) {
    for (int i = 0; i < 8; ++i) {
      serialized_addr_map.push_back(
          (reinterpret_cast<std::uint64_t>(fn) >> (i * 8)) & 0xFF);
    }
  ...
  for (const auto &[address, literal] : string_literals) {
    for (int i = 0; i < 8; ++i) {
      serialized_string_map.push_back(
          (reinterpret_cast<std::uint64_t>(address) >> (i * 8)) & 0xFF);
    }

    for (int i = 0; i < 8; ++i) { // str - 8 = length
      serialized_string_map.push_back(
          (reinterpret_cast<std::uint64_t>(literal.length()) >> (i * 8)) &
          0xFF);
    }
  ...
  /*
      ...
      const_pool
  */
  header.address_map_offset =
      header.code_offset + header.code_size; // beings right after
  header.const_pool_offset =
      header.address_map_offset + serialized_addr_map.size();

  std::vector<std::uint8_t> bytes_to_write;

  const auto *header_bytes = reinterpret_cast<const std::uint8_t *>(&header);
  bytes_to_write.insert(bytes_to_write.end(), header_bytes,
                        header_bytes + sizeof(header));

  const auto &code = code_it->second;
  bytes_to_write.insert(bytes_to_write.end(), code.begin(), code.end());

  bytes_to_write.insert(bytes_to_write.end(), serialized_addr_map.begin(),
                        serialized_addr_map.end());

  bytes_to_write.insert(bytes_to_write.end(), serialized_string_map.begin(),
                        serialized_string_map.end());
  ...
  out.write(reinterpret_cast<const char *>(bytes_to_write.data()),
