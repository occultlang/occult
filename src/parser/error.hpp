...
public:
    explicit parsing_error(std::string expected, token_t tk, const std::size_t& curr_pos,
                           const char* func_name)
        : std::runtime_error("[PARSE ERROR] "), expected(std::move(expected)), tk(std::move(tk)),
          curr_pos(curr_pos), func_name(func_name) {}
