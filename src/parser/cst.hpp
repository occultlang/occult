...
class cst {
  ...
  public:
    ...
    template <typename BaseCst = cst> static std::unique_ptr<BaseCst> new_node(std::string content) {
    ...
    template <typename BaseCst = cst> static std::unique_ptr<BaseCst> cast(cst* node) {
...
NODE(generic_expression, cst_generic_expr)
