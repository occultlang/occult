...
namespace occult {
    ...
    public:
        ...
        static std::unique_ptr<BaseCst> new_node() { return std::make_unique<BaseCst>(); }
        ...
        static std::unique_ptr<BaseCst> cast(cst *node) {
            if (auto casted_node = dynamic_cast<BaseCst *>(node)) { return std::unique_ptr<BaseCst>(casted_node); }
        ...
        static BaseCst *cast_raw(cst *node) {
            if (auto casted_node = dynamic_cast<BaseCst *>(node)) { return casted_node; }
        ...
        void visualize(const int depth = 0) {
            ...
            if (do_not) { output += GREEN " (do_not = true)" RESET; }

            if (num_pointers > 0) { output += YELLOW " (num_pointers = " + std::to_string(num_pointers) + ")" + RESET; }

            std::cout << output << "\n";

            for (const auto &child: children) { child->visualize(depth + 1); }
        }
    };
