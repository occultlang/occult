...
#define CST_MAP_ENTRY(token, node) {token, [](const std::string& lexeme) { return cst::new_node<node>(lexeme); }}
#define CST_MAP_ENTRY_CUSTOM_LEXEME(token, node, value) {token, [](const std::string&) { return cst::new_node<node>(value); }}
#define CST_MAP_ENTRY_NO_LEXEME(token, node) {token, []() { return cst::new_node<node>(); }}

namespace occult {
inline std::unordered_map<token_type, int> precedence_map = {{unary_plus_operator_tt, 2},
...
template <typename Iterator> int find_first_token(Iterator begin, Iterator end, token_type tt) {
    auto it = std::find_if(begin, end, [tt](const token_t& t) { return t.tt == tt; });

    return (it != end) ? std::distance(begin, it) : -1;
}

inline std::unordered_map<token_type, std::function<std::unique_ptr<cst>(std::string)>> cst_map = {
...
#ifdef __linux
    CST_MAP_ENTRY_CUSTOM_LEXEME(is_linux64, cst_numberliteral, "1"),
    CST_MAP_ENTRY_CUSTOM_LEXEME(is_win64, cst_numberliteral, "0"),
#elif _WIN64
    CST_MAP_ENTRY_CUSTOM_LEXEME(is_linux64, cst_numberliteral, "0"),
    CST_MAP_ENTRY_CUSTOM_LEXEME(is_win64, cst_numberliteral, "1"),
#endif
...
inline std::unordered_map<token_type, std::function<std::unique_ptr<cst>()>> datatype_map = {
    CST_MAP_ENTRY_NO_LEXEME(int8_keyword_tt, cst_int8),       CST_MAP_ENTRY_NO_LEXEME(int16_keyword_tt, cst_int16),     CST_MAP_ENTRY_NO_LEXEME(int32_keyword_tt, cst_int32),   CST_MAP_ENTRY_NO_LEXEME(int64_keyword_tt, cst_int64),
    CST_MAP_ENTRY_NO_LEXEME(uint8_keyword_tt, cst_uint8),     CST_MAP_ENTRY_NO_LEXEME(uint16_keyword_tt, cst_uint16),   CST_MAP_ENTRY_NO_LEXEME(uint32_keyword_tt, cst_uint32), CST_MAP_ENTRY_NO_LEXEME(uint64_keyword_tt, cst_uint64),
    CST_MAP_ENTRY_NO_LEXEME(float32_keyword_tt, cst_float32), CST_MAP_ENTRY_NO_LEXEME(float64_keyword_tt, cst_float64), CST_MAP_ENTRY_NO_LEXEME(string_keyword_tt, cst_string), CST_MAP_ENTRY_NO_LEXEME(char_keyword_tt, cst_int8),
