...
#define CST_MAP_ENTRY_NO_LEXEME(token, node)                                   \
  {token, []() { return cst::new_node<node>(); }}
...
int find_first_token(Iterator begin, Iterator end, token_type tt) {
  auto it =
      std::find_if(begin, end, [tt](const token_t &t) { return t.tt == tt; });

  return (it != end) ? std::distance(begin, it) : -1;
}

inline std::unordered_map<token_type,
                          std::function<std::unique_ptr<cst>(std::string)>>
    cst_map = {
        ...
        CST_MAP_ENTRY(greater_than_or_equal_operator_tt,
                      cst_greater_than_or_equal),
...
inline std::unordered_map<token_type, std::function<std::unique_ptr<cst>()>>
    datatype_map = {
