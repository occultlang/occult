...
private:
  ...
  std::unordered_map<std::string, std::unique_ptr<cst_generic_type>>
      cst_generic_type_cache;
  ...
  std::vector<std::string>
      module_prefix_stack; // current nesting of module names
  std::vector<std::string>
      imported_modules; // imported module paths (e.g. "std::math")
  ...
  void parse_struct_member_access_expr(
  ...
  void shunting_yard(std::stack<token_t> &stack_ref,
                     std::vector<std::unique_ptr<cst>> &expr_cst_ref,
                     const token_t &curr_tok_ref) const;

  void shunting_yard_stack_cleanup(
  ...
  std::unique_ptr<cst> parse_compound_assignment_identifier(
  ...
  parse_regular_for(std::unique_ptr<cst_forstmt> existing_for_node);
  ...
  std::vector<std::vector<token_t>> parse_type_argument_tokens();
  bool type_args_have_generic_params(
      const std::vector<std::vector<token_t>> &type_args) const;
  std::string
  mangle_type_token_sequence(const std::vector<token_t> &tokens) const;

  std::string instantiate_generic_struct(
      const std::string &template_name,
      const std::vector<std::vector<token_t>> &concrete_type_args);
  std::string mangle_generic_struct_name(
      const std::string &template_name,
  ...
  const std::unordered_map<std::string, generic_template> &
  get_generic_struct_templates() const {
    return generic_struct_templates;
  }
  const std::unordered_map<std::string, generic_template> &
  get_generic_func_templates() const {
    return generic_func_templates;
  }
  const std::unordered_set<std::string> &get_instantiated_generics() const {
    return instantiated_generics;
  }
  const std::unordered_map<std::string,
                           std::unordered_map<std::string, std::int64_t>> &
