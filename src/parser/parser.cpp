    ...
    void parser::parse_function_call_expr(std::vector<std::unique_ptr<cst>> &expr_cst_ref,
        ...
        while (i_ref + 1 < expr_ref.size() && 0 < paren_depth) {
            ...
            if (current_token.tt == left_paren_tt) {
                ...
                if (paren_depth > 0) { current_args.push_back(current_token); }
            }
                ...
                if (current_args.size() == 1 && current_args[0].tt == identifier_tt) {
                    ...
                    for (auto &c: parsed_args) { arg_node->add_child(std::move(c)); }
                }
            ...
            else { current_args.push_back(current_token); }
        }
    ...
    void parser::parse_array_access_expr(std::vector<std::unique_ptr<cst>> &expr_cst_ref,
        ...
        while (i_ref + 1 < expr_ref.size() && expr_ref[i_ref + 1].tt == left_bracket_tt) {
            ...
            while (i_ref < expr_ref.size() && bracket_depth > 0) {
                ...
                else { index_tokens.push_back(tok); }
    ...
    void parser::shunting_yard(std::stack<token_t> &stack_ref, std::vector<std::unique_ptr<cst>> &expr_cst_ref,
        ...
        switch (curr_tok_ref.tt) {
            ...
            case right_paren_tt: {
                ...
                if (!stack_ref.empty() && stack_ref.top().tt == left_paren_tt) { stack_ref.pop(); }
                else {
    ...
    */
        ...
        for (std::size_t i = 0; i < expr.size(); i++) {
            ...
            /*else if (t.tt == reference_operator_tt || t.tt == dereference_operator_tt) {
              expr_cst.push_back(cst_map[t.tt](t.lexeme));
                ...
                else { expr_cst.push_back(cst_map[t.tt](t.lexeme)); }
            }
            else { shunting_yard(operator_stack, expr_cst, t); }
        }
    ...
    std::unique_ptr<cst> parser::parse_datatype() {
        if (const auto it = datatype_map.find(peek().tt); it != datatype_map.end()) {
            ...
            if (match(peek(), identifier_tt)) { node->add_child(parse_identifier()); }
    ...
    std::unique_ptr<cst_function> parser::parse_function() {
        ...
        if (match(peek(), left_paren_tt)) {
            ...
            while (!match(peek(), right_paren_tt)) {
                ...
                if (match(peek(), comma_tt)) { consume(); }
            }
    ...
    std::unique_ptr<cst_block> parser::parse_block() {
        if (match(peek(), left_curly_bracket_tt)) {
            ...
            while (!match(peek(), right_curly_bracket_tt)) { block_node->add_child(parse_keyword()); }
    ...
    std::unique_ptr<IntegerCstType> parser::parse_integer_type() {
        ...
        else { throw parsing_error("<identifier>", peek(), pos, std::source_location::current().function_name()); }
    ...
    std::unique_ptr<cst_returnstmt> parser::parse_return() {
        ...
        if (match(peek(), semicolon_tt)) { consume(); }
        else { throw parsing_error(";", peek(), pos, std::source_location::current().function_name()); }
    ...
    std::unique_ptr<cst_ifstmt> parser::parse_if() {
        ...
        while (match(peek(), elseif_keyword_tt)) { if_node->add_child(parse_elseif()); }

        if (match(peek(), else_keyword_tt)) { if_node->add_child(parse_else()); }
    ...
    std::unique_ptr<cst_forstmt> parser::parse_regular_for(std::unique_ptr<cst_forstmt> existing_for_node) {
        ...
        if (match(peek(), when_keyword_tt)) {
            ...
            for (auto &c: converted_rpn) { forcond_node->add_child(std::move(c)); }
            ...
            else { throw parsing_error("do", peek(), pos, std::source_location::current().function_name()); }
                ...
                while (!match(peek(), right_curly_bracket_tt)) {
                    if (match(peek(), left_curly_bracket_tt)) {
                        ...
                        if (!element_tokens.empty()) {
                            ...
                            for (auto &n: expr_nodes) { elem->add_child(std::move(n)); }
    ...
    std::unique_ptr<cst_struct> parser::parse_struct() {
        ...
        if (match(peek(), left_curly_bracket_tt)) {
            ...
            if (match(peek(), right_curly_bracket_tt)) { consume(); }
            else {
    ...
    std::unique_ptr<cst> parser::parse_keyword(bool nested_function) {
        ...
        if (match(peek(), int8_keyword_tt)) { return parse_integer_type<cst_int8>(); }
        if (match(peek(), int16_keyword_tt)) { return parse_integer_type<cst_int16>(); }
        if (match(peek(), int32_keyword_tt)) { return parse_integer_type<cst_int32>(); }
        if (match(peek(), int64_keyword_tt)) { return parse_integer_type<cst_int64>(); }
        if (match(peek(), uint8_keyword_tt)) { return parse_integer_type<cst_uint8>(); }
        if (match(peek(), uint16_keyword_tt)) { return parse_integer_type<cst_uint16>(); }
        if (match(peek(), uint32_keyword_tt)) { return parse_integer_type<cst_uint32>(); }
        if (match(peek(), uint64_keyword_tt)) { return parse_integer_type<cst_uint64>(); }
        if (match(peek(), float32_keyword_tt)) { return parse_integer_type<cst_float32>(); }
        if (match(peek(), float64_keyword_tt)) { return parse_integer_type<cst_float64>(); }
        ...
        if (match(peek(), if_keyword_tt)) { return parse_if(); }
        if (match(peek(), loop_keyword_tt)) { return parse_loop(); }
        if (match(peek(), break_keyword_tt)) { return parse_break(); }
        if (match(peek(), continue_keyword_tt)) { return parse_continue(); }
        if (match(peek(), while_keyword_tt)) { return parse_while(); }
        if (match(peek(), for_keyword_tt)) { return parse_for(); }
        if (match(peek(), return_keyword_tt)) { return parse_return(); }
        if (match(peek(), array_keyword_tt)) { return parse_array(); }
        if (match(peek(), struct_keyword_tt)) { return parse_struct(); }
        ...
        if (match(peek(), identifier_tt) && peek(1).tt == left_bracket_tt) { // array access
            ...
            while (match(peek(), left_bracket_tt)) {
                ...
                if (array_access_node) { new_array_access->add_child(std::move(array_access_node)); }
                else { new_array_access->add_child(std::move(lcst_id)); }
                ...
                while (bracket_depth > 0 && !match(peek(), end_of_file_tt)) {
                    if (match(peek(), left_bracket_tt)) {
                        bracket_depth++;
                        if (bracket_depth > 1) { index_tokens.push_back(peek()); }
                        consume();
                    }
                        ...
                        else { index_tokens.push_back(peek()); }
                        consume();
                    }
