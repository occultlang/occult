...
void parser::consume(const std::uintptr_t amt) {
    pos += amt;
}
...
template <typename ParentNode> void parser::parse_expression_until(ParentNode* parent, const token_type t) {
...
template <typename IntegerCstType> std::unique_ptr<IntegerCstType> parser::parse_integer_type() {
