...
namespace occult {
parser::parser(const std::vector<token_t> &stream,
               const std::string &source_file_path, const std::string &source)
...
void parser::parse_function_call_expr(
  ...
  auto start_node =
      cst_map[function_call_parser_tt]("start_call"); // start call

  start_node->add_child(
      cst_map[curr_tok_ref.tt](curr_tok_ref.lexeme)); // function name
...
void parser::parse_array_access_expr(
  ...
  while (i_ref + 1 < expr_ref.size() &&
         expr_ref[i_ref + 1].tt == left_bracket_tt) {
    i_ref++;
    i_ref++;
    ...
    if (bracket_depth != 0) {
      throw parsing_error("unmatched [ in array access", expr_ref.at(i_ref),
                          pos, std::source_location::current().function_name());
    }
...
void parser::parse_struct_member_access_expr(
    std::vector<std::unique_ptr<cst>> &expr_cst_ref,
    const std::vector<token_t> &expr_ref, const token_t &curr_tok_ref,
  ...
  while (i_ref < expr_ref.size() && expr_ref[i_ref].tt == period_tt) {
    ...
    if (i_ref >= expr_ref.size() || expr_ref[i_ref].tt != identifier_tt) {
      throw parsing_error("identifier after period",
                          i_ref < expr_ref.size() ? expr_ref[i_ref]
                                                  : expr_ref.back(),
                          pos, std::source_location::current().function_name());
    }

    member_access_node->add_child(
        cst_map[identifier_tt](expr_ref[i_ref].lexeme));
    i_ref++; // next tok
  }

  if (i_ref > 0 &&
      expr_ref[i_ref - 1].tt == identifier_tt) { // backtrack if identifier
    i_ref--;
  }
...
void parser::shunting_yard(std::stack<token_t> &stack_ref,
  ...
  case right_paren_tt: {
    while (!stack_ref.empty() && stack_ref.top().tt != left_paren_tt) {
      ...
      if (it == cst_map.end() || !it->second) {
        throw parsing_error("valid operator or operand (possibly a missing ';' "
                            "before this token)",
                            stack_ref.top(), pos,
    ...
    while (!stack_ref.empty() && // pop unary ops after paren
      ...
      expr_cst_ref.push_back(
          cst_map[stack_ref.top().tt](stack_ref.top().lexeme));
      stack_ref.pop();
    }
  ...
  default: { // binary ops
    while (!stack_ref.empty() && stack_ref.top().tt != left_paren_tt &&
           (curr_tok_ref.tt == assignment_tt
                ? precedence_map[curr_tok_ref.tt] >
                      precedence_map[stack_ref.top().tt]
                : precedence_map[curr_tok_ref.tt] >=
                      precedence_map[stack_ref.top().tt])) {
      auto it = cst_map.find(stack_ref.top().tt);
      if (it == cst_map.end() || !it->second) {
...
void parser::shunting_yard_stack_cleanup(
    std::stack<token_t> &stack_ref,
  ...
  while (!stack_ref.empty()) {
    ...
    if (stack_ref.top().tt == right_curly_bracket_tt) {
      throw parsing_error("unmatched right curly bracket", stack_ref.top(), pos,
                          std::source_location::current().function_name());
    }
...
*/
std::vector<std::unique_ptr<cst>>
parser::parse_expression(const std::vector<token_t> &raw_expr) {
  ...
  for (std::size_t i = 0; i < expr.size(); i++) {
    ...
    if (t.tt == semicolon_tt || t.tt == left_curly_bracket_tt ||
        t.tt == comma_tt) {
      break; // end of expr
    }
    ...
    if (t.tt == reference_operator_tt) {
      ...
      if (i + 1 < expr.size() && expr.at(i + 1).tt == identifier_tt &&
        ...
        while (i < expr.size() && expr.at(i).tt == period_tt) {
          ...
          member_access_node->add_child(
              cst_map[identifier_tt](expr.at(i).lexeme));
          i++;
        }
      ...
      operator_stack.push(token_t(t.line, t.column, std::to_string(count),
    ...
    { expr_cst.push_back(cst_map[t.tt](t.lexeme));
      ...
      if (i + 1 < expr.size() && expr.at(i + 1).tt == scope_resolution_tt) {
        ...
        auto try_expand_import = [&](const std::string &path) -> std::string {
          ...
          std::string first_seg = (first_sep != std::string::npos)
                                      ? path.substr(0, first_sep)
                                      : path;
          for (const auto &imp : imported_modules) {
            auto imp_last_sep = imp.rfind("::");
            std::string imp_last = (imp_last_sep != std::string::npos)
                                       ? imp.substr(imp_last_sep + 2)
                                       : imp;
        ...
        for (auto it = scoped_candidates.rbegin();
             it != scoped_candidates.rend(); ++it) {
          ...
          if (enum_it != enum_definitions.end() &&
              enum_it->second.contains(potential_member)) {
            auto value = enum_it->second.at(potential_member);
            expr_cst.push_back(
                cst::new_node<cst_numberliteral>(std::to_string(value)));
      ...
      } else if (i + 3 < expr.size() && expr.at(i + 1).tt == period_tt &&
                 expr.at(i + 2).tt == identifier_tt &&
                 expr.at(i + 3).tt == left_paren_tt) {
        ...
        while (i + 1 < expr.size() && 0 < paren_depth) {
          ...
          if (current_token.tt == left_paren_tt) {
            ...
            if (current_args.size() == 1 &&
                current_args[0].tt == identifier_tt) {
              arg_node->add_child(
                  cst_map[identifier_tt](current_args.at(0).lexeme));
            } else {
              auto parsed_args = parse_expression(current_args);
              for (auto &c : parsed_args) {
                arg_node->add_child(std::move(c));
      ...
      if (i + 1 < expr.size() && expr.at(i + 1).tt == left_paren_tt) {
        ...
        if (paren_depth != 0) {
          throw parsing_error("unmatched '(' in cast expression",
                              expr.at(type_i), pos,
                              std::source_location::current().function_name());
        }
        ...
        if (inner_expr.empty()) {
          throw parsing_error("cast expects an expression in parentheses",
                              expr.at(type_i), pos,
                              std::source_location::current().function_name());
        }

        auto cast_node =
            cst::new_node<cst_cast_to_datatype>(expr.at(type_i).lexeme);
...
std::unique_ptr<cst> parser::parse_datatype() {
  ...
  if (match(peek(), identifier_tt) && pos + 2 < stream.size() &&
      match(peek(1), scope_resolution_tt) && match(peek(2), identifier_tt)) {
    ...
    if (pos + 3 < stream.size() && match(peek(3), less_than_operator_tt) &&
        (generic_struct_templates.contains(qualified_name) ||
         generic_struct_templates.contains(type_name))) {
      std::string template_name =
          generic_struct_templates.contains(qualified_name) ? qualified_name
                                                            : type_name;
      ...
      auto type_args = parse_type_argument_tokens();
      const bool has_generic_param = type_args_have_generic_params(type_args);

      std::string struct_name = template_name;
      if (!has_generic_param) {
        struct_name = instantiate_generic_struct(template_name, type_args);
      }
  ...
  if (match(peek(), identifier_tt) &&
      generic_struct_templates.contains(peek().lexeme) &&
  ...
  if (match(peek(), identifier_tt) &&
      cst_generic_type_cache.contains(peek().lexeme)) {
  ...
  if (match(peek(), array_keyword_tt)) {
    throw parsing_error("scalar return type (array types cannot be used as a "
...
std::unique_ptr<cst_function> parser::parse_function() {
  ...
  if (match(peek(), left_paren_tt)) {
    ...
    while (!match(peek(), right_paren_tt)) {
      if (match(peek(), variadic_tt)) {
        ...
        int existing_args =
            static_cast<int>(func_args_node->get_children().size());
        int max_total_args =
            existing_args + 16; // support up to 16 variadic args
        ...
        for (int va_idx = 0; va_idx < synthetic_count; va_idx++) {
          ...
          arg->add_child(
              cst::new_node<cst_identifier>("__va" + std::to_string(va_idx)));
      ...
      if (!arg) {
        throw parsing_error("valid type in function argument", peek(), pos,
                            std::source_location::current().function_name());
      }

      if (arg->get_children().size() < 1) { // for shellcode
        arg->add_child(
            cst::new_node<cst_identifier>("arg" + std::to_string(++arg_count)));
      }
  ...
  if (uses_shellcode) {
    ...
    if (match(peek(), left_curly_bracket_tt)) {
      ...
      while (!match(peek(), right_curly_bracket_tt)) {
        ...
        consume();
        shellcode_node->add_child(
            cst::new_node<cst_numberliteral>(number_token.lexeme));
      }
  ...
  if (uses_assembly) {
    ...
    if (match(peek(), left_curly_bracket_tt)) {
      ...
      if (tok.tt !=
          shellcode_denoter_tt) { // lets just check anyways can't hurt
        throw parsing_error("assembly code", peek(), pos,
                            std::source_location::current().function_name());
      }
...
void parser::parse_expression_until(ParentNode *parent, const token_type t) {
  const auto first_pos =
      find_first_token(stream.begin() + pos, stream.end(), t);
  if (first_pos == -1) {
    return;
  }
  const std::vector<token_t> sub_stream = {
      stream.begin() + pos, stream.begin() + pos + first_pos + 1};
  pos += first_pos;

  for (auto converted_rpn = parse_expression(sub_stream);
       auto &c : converted_rpn) {
...
parser::parse_regular_for(std::unique_ptr<cst_forstmt> existing_for_node) {
  ...
  const auto when_pos =
      find_first_token(stream.begin() + pos, stream.end(), when_keyword_tt);
  stream.insert(stream.begin() + pos + when_pos,
                token_t(stream.at(pos).line, stream.at(pos).column + 1, ";",
                        semicolon_tt));
  ...
  if (match(peek(), when_keyword_tt)) {
    ...
    const auto do_pos =
        find_first_token(stream.begin() + pos, stream.end(), do_keyword_tt);
    stream.insert(stream.begin() + pos + do_pos,
                  token_t(stream.at(pos).line, stream.at(pos).column + 1, ";",
                          semicolon_tt));
    std::vector<token_t> sub_stream = {stream.begin() + pos,
                                       stream.begin() + pos + do_pos + 2};
    pos += do_pos + 1;
    ...
    if (match(peek(), do_keyword_tt)) {
      ...
      const auto left_curly_bracket_pos = find_first_token(
          stream.begin() + pos, stream.end(), left_curly_bracket_tt);
...
std::unique_ptr<cst_forstmt> parser::parse_for() { // for expr; in expr; { }
  ...
  if (find_first_token(stream.begin() + pos, stream.end(), when_keyword_tt) !=
      -1) {
    return parse_regular_for(std::move(for_node));
  }

  const auto in_pos =
      find_first_token(stream.begin() + pos, stream.end(), in_keyword_tt);
...
parser::parse_array() { // array <dimensions> <datatype> <identifier> = { ... };
  ...
  while (match(peek(), left_bracket_tt)) {
    ...
    const auto dimension_size =
        from_numerical_string<std::size_t>(peek().lexeme);
  ...
  if (!(match(peek(), int8_keyword_tt) || match(peek(), int16_keyword_tt) ||
        ...
        match(peek(), float64_keyword_tt) || match(peek(), string_keyword_tt) ||
        match(peek(), boolean_keyword_tt) || match(peek(), char_keyword_tt))) {
  ...
  auto dimensions_count =
      cst::new_node<cst_dimensions_count>(std::to_string(dimensions.size()));
...
std::unique_ptr<cst_enum> parser::parse_enum() {
  ...
  if (!match(peek(), identifier_tt)) {
    throw parsing_error("enum name", peek(), pos,
                        std::source_location::current().function_name());
  }
  ...
  if (!match(peek(), left_curly_bracket_tt)) {
    throw parsing_error("{ in enum declaration", peek(), pos,
                        std::source_location::current().function_name());
  }
  consume(); // consume '{'
  ...
  while (!match(peek(), right_curly_bracket_tt)) {
    if (!match(peek(), identifier_tt)) {
      throw parsing_error("enum member name", peek(), pos,
                          std::source_location::current().function_name());
    }
    ...
    if (match(peek(), assignment_tt)) {
      consume(); // consume '='
      if (!match(peek(), number_literal_tt)) {
        throw parsing_error("integer value for enum member", peek(), pos,
                            std::source_location::current().function_name());
      }
    ...
    auto value_node =
        cst::new_node<cst_numberliteral>(std::to_string(next_value));
...
std::unique_ptr<cst_switchstmt> parser::parse_switch() {
  ...
  if (!match(peek(), left_paren_tt)) {
    throw parsing_error("( after switch", peek(), pos,
                        std::source_location::current().function_name());
  }
  consume(); // consume '('
  ...
  if (!match(peek(), right_paren_tt)) {
    throw parsing_error(") after switch expression", peek(), pos,
                        std::source_location::current().function_name());
  }
  consume(); // consume ')'
  ...
  while (!match(peek(), right_curly_bracket_tt) &&
    ...
    if (match(peek(), case_keyword_tt)) {
      ...
      if (!match(peek(), colon_tt)) {
        throw parsing_error(": after case value", peek(), pos,
                            std::source_location::current().function_name());
      }
      consume(); // consume ':'
      ...
      if (!match(peek(), colon_tt)) {
        throw parsing_error(": after default", peek(), pos,
                            std::source_location::current().function_name());
      }
      consume(); // consume ':'
      ...
      throw parsing_error("case or default in switch body", peek(), pos,
                          std::source_location::current().function_name());
    }
  }

  if (!match(peek(), right_curly_bracket_tt)) {
    throw parsing_error("} to close switch", peek(), pos,
                        std::source_location::current().function_name());
  }
  consume(); // consume '}'
...
static void
prefix_module_references(cst *node, const std::string &prefix,
                         const std::unordered_set<std::string> &module_names) {
  if (!node)
    return;

  if (node->get_type() == cst_type::functioncall &&
      node->content == "start_call") {
    if (!node->get_children().empty()) {
      auto *name_child = node->get_children().front().get();
      if (name_child->get_type() == cst_type::identifier) {
        if (module_names.count(name_child->content) &&
            name_child->content.find("::") == std::string::npos) {
...
std::unique_ptr<cst> parser::parse_module() {
  ...
  if (!match(peek(), identifier_tt)) {
    throw parsing_error("module name", peek(), pos,
                        std::source_location::current().function_name());
  }
  ...
  if (!match(peek(), left_curly_bracket_tt)) {
    throw parsing_error("{ after module name", peek(), pos,
                        std::source_location::current().function_name());
  }
  consume(); // consume '{'
          ...
          if (first_child->get_type() == cst_type::identifier) {
            ...
            for (std::size_t ci = 1; ci < node->get_children().size(); ++ci) {
              ...
              if (fn_name->content.rfind(old_method_prefix, 0) == 0) {
                fn_name->content =
                    new_name + fn_name->content.substr(old_name.size());
              }
            }
          }
        }
  ...
  if (!match(peek(), right_curly_bracket_tt)) {
    throw parsing_error("} to close module", peek(), pos,
                        std::source_location::current().function_name());
  }
...
std::unique_ptr<cst> parser::parse_import() {
  ...
  if (!match(peek(), identifier_tt)) {
    throw parsing_error("module path after import", peek(), pos,
                        std::source_location::current().function_name());
  }
  ...
  while (match(peek(), scope_resolution_tt)) {
    consume(); // consume '::'
    if (!match(peek(), identifier_tt)) {
      throw parsing_error("identifier after ::", peek(), pos,
                          std::source_location::current().function_name());
    }
    import_path += "::" + peek().lexeme;
    path_segments.push_back(peek().lexeme);
  ...
  if (match(peek(), identifier_tt) && peek().lexeme == "from") {
    ...
    if (!match(peek(), identifier_tt)) {
      throw parsing_error("source path after 'from'", peek(), pos,
                          std::source_location::current().function_name());
    }
    ...
    while (match(peek(), division_operator_tt)) {
      consume(); // consume '/'
      if (!match(peek(), identifier_tt)) {
        throw parsing_error("identifier after '/' in from path", peek(), pos,
                            std::source_location::current().function_name());
      }
      from_source += "/";
      from_source += peek().lexeme;
  ...
  if (!match(peek(), semicolon_tt)) {
    throw parsing_error("; after import", peek(), pos,
                        std::source_location::current().function_name());
  }
  consume(); // consume ';'

  auto try_load_module_file =
      [&](const std::filesystem::path &base_dir) -> std::unique_ptr<cst_root> {
    ...
    if (!file.is_open()) {
      throw parsing_error("readable module file '" + module_file.string() + "'",
                          peek(), pos,
                          std::source_location::current().function_name());
    }
    ...
    p.import_generic_templates(generic_struct_templates, generic_func_templates,
                               instantiated_generics, custom_type_map,
                               enum_definitions);
    auto included_cst = p.parse();

    import_generic_templates(p.get_generic_struct_templates(),
                             p.get_generic_func_templates(),
                             p.get_instantiated_generics(),
                             p.get_custom_type_map(), p.get_enum_definitions());
...
std::unique_ptr<cst> parser::parse_compound_assignment_identifier(
    std::unique_ptr<cst_identifier> to_assign) {
...
std::unique_ptr<cst> parser::parse_keyword(bool nested_function) {
  ...
  if (recursion_depth > max_recursion_depth) {
    throw parsing_error("expression (recursion depth exceeded)", peek(), pos,
                        std::source_location::current().function_name());
  }
    ...
    */
    ...
    if (match(peek(), less_than_operator_tt)) {
      ...
      if (match(peek(), greater_than_operator_tt)) {
        throw parsing_error("typename in generic definition", peek(), pos,
                            std::source_location::current().function_name());
      }
      ...
      while (!match(peek(), greater_than_operator_tt)) {
        ...
        cst_generic_type_cache[generic_typename] =
            cst::new_node<cst_generic_type>(generic_typename);
        type_params.push_back(generic_typename);

        if (match(peek(), comma_tt) &&
            match(peek(1), greater_than_operator_tt)) {
          throw parsing_error("typename after comma", peek(), pos,
                              std::source_location::current().function_name());
        }
      ...
      if (!match(peek(), greater_than_operator_tt)) {
        throw parsing_error("greater than operator (for generic type)", peek(),
                            pos,
                            std::source_location::current().function_name());
      }
      ...
      if (match(peek(), struct_keyword_tt)) {
        ...
        generic_struct_templates[struct_name] = {
            type_params,
            std::vector<token_t>(stream.begin() + template_start,
                                 stream.begin() + template_end),
            struct_name};
        ...
        throw parsing_error("function or struct definition", peek(), pos,
                            std::source_location::current().function_name());
      }
    } else {
      throw parsing_error(
          "'<' with type parameters after 'generic' (e.g., generic<T>)", peek(),
          pos, std::source_location::current().function_name());
    }
  }
  ...
  if (match(peek(), include_keyword_tt)) {
    ...
    if (match(peek(), string_literal_tt)) {
      ...
      if (!file.is_open()) {
        throw parsing_error("existing include file", string_token, pos,
                            std::source_location::current().function_name());
      }
      ...
      p.import_generic_templates(generic_struct_templates,
                                 generic_func_templates, instantiated_generics,
                                 custom_type_map, enum_definitions);
      auto included_cst = p.parse();

      import_generic_templates(
          p.get_generic_struct_templates(), p.get_generic_func_templates(),
          p.get_instantiated_generics(), p.get_custom_type_map(),
          p.get_enum_definitions());
  ...
  if (match(peek(), identifier_tt) && pos + 2 < stream.size() &&
    ...
    if (pos + 3 < stream.size() && peek(3).tt == less_than_operator_tt &&
        (generic_struct_templates.contains(qualified) ||
         generic_struct_templates.contains(bare_name))) {
      std::string template_name =
          generic_struct_templates.contains(qualified) ? qualified : bare_name;
      consume(); // consume module name
      consume(); // consume '::'
      consume(); // consume type name
      consume(); // consume '<'
      ...
      if (match(peek(), assignment_tt)) {
        ...
        if (match(peek(), semicolon_tt)) {
          throw parsing_error("expression (found empty assignment)", peek(),
                              pos,
                              std::source_location::current().function_name());
        }
  ...
  if (match(peek(), identifier_tt) &&
      generic_struct_templates.contains(peek().lexeme) &&
      peek(1).tt == less_than_operator_tt) {
  ...
  // Forward-referenced generic struct in template context (inside function
  // body)
  {
    std::unique_ptr<cst> fwd_node;
    if (try_parse_forward_generic_struct_type(fwd_node)) {
      ...
      if (match(peek(), assignment_tt)) {
        ...
        parse_expression_until(fwd_node->get_children().at(1).get(),
                               semicolon_tt);
      }
  ...
  if (match(peek(), identifier_tt) &&
      cst_generic_type_cache.contains(
          peek().lexeme)) { // T* var = expr; or T var = expr;
    const auto type_name = peek().lexeme;
  ...
  cst::new_node<cst_dereference>(std::to_string(deref_count));
          ...
          if (match(peek(), assignment_tt)) {
              ...
              if (match(peek(), semicolon_tt)) {
                  throw parsing_error("expression (found empty assignment)",
  peek(), pos, std::source_location::current().function_name());
              }
  ...
  if (match(peek(), dereference_operator_tt)) {
    ...
    if (match(peek(), left_paren_tt)) {
      ...
      throw parsing_error("identifier or (", peek(), pos,
                          std::source_location::current().function_name());
    }
    ...
    if (match(peek(), period_tt)) {
      ...
      while (match(peek(), period_tt)) {
        ...
        if (match(peek(), identifier_tt)) {
          ...
          throw parsing_error("member name after '.'", peek(), pos,
                              std::source_location::current().function_name());
        }
      }
    ...
    bool handled_compound_assignment =
        parse_deref_compound_assignment(add_assign_tt) ||
        parse_deref_compound_assignment(sub_assign_tt) ||
        parse_deref_compound_assignment(mul_assign_tt) ||
        parse_deref_compound_assignment(div_assign_tt) ||
        parse_deref_compound_assignment(mod_assign_tt) ||
        parse_deref_compound_assignment(and_assign_tt) ||
        parse_deref_compound_assignment(or_assign_tt) ||
        parse_deref_compound_assignment(xor_assign_tt) ||
        parse_deref_compound_assignment(lshift_assign_tt) ||
    ...
    if (match(peek(), assignment_tt)) {
      ...
      throw parsing_error("assignment operator after dereference", peek(), pos,
                          std::source_location::current().function_name());
    }
  ...
  if (match(peek(), identifier_tt) &&
      peek(1).tt == less_than_operator_tt) { // potential generic fn call at
                                             // statement level
    ...
    while (scan < stream.size() && stream[scan].tt != semicolon_tt &&
           stream[scan].tt != end_of_file_tt) {
      if (stream[scan].tt == greater_than_operator_tt &&
          scan + 1 < stream.size() && stream[scan + 1].tt == left_paren_tt) {
    ...
    if (found_generic_call) {
      ...
      if (first_semicolon_pos == -1) {
        ...
        while (err_scan < stream.size() &&
               stream[err_scan].tt != right_paren_tt &&
               stream[err_scan].tt != end_of_file_tt) {
          ++err_scan;
        }
        const auto &err_tok = (err_scan + 1 < stream.size())
                                  ? stream[err_scan + 1]
                                  : stream[err_scan];
        throw parsing_error(";", err_tok, pos,
                            std::source_location::current().function_name());
      }
      std::vector<token_t> sub_stream = {
          stream.begin() + pos, stream.begin() + pos + first_semicolon_pos + 1};
      pos += first_semicolon_pos;
      auto converted_rpn = parse_expression(sub_stream);

      if (converted_rpn.size() == 1 &&
          converted_rpn.at(0)->content == "start_call") {
        if (match(peek(), semicolon_tt)) {
          consume();
        } else {
          throw parsing_error(";", peek(), pos,
      ...
      throw parsing_error("a valid generic function call statement", peek(),
                          pos, std::source_location::current().function_name());
    }
  }
  ...
  if (match(peek(), identifier_tt) && peek(1).tt == left_paren_tt) { // fn call
    ...
    if (first_semicolon_pos == -1) {
      ...
      std::size_t scan = pos;
      while (scan < stream.size() && stream[scan].tt != right_paren_tt &&
             stream[scan].tt != end_of_file_tt) {
        ++scan;
      }

      const auto &err_tok =
          (scan + 1 < stream.size()) ? stream[scan + 1] : stream[scan];
      throw parsing_error(";", err_tok, pos,
                          std::source_location::current().function_name());
    }
    std::vector<token_t> sub_stream = {
    ...
    throw parsing_error("a valid function call statement", peek(), pos,
                        std::source_location::current().function_name());
  }
  if (match(peek(), identifier_tt) && peek(1).tt == period_tt &&
      peek(2).tt == identifier_tt && peek(3).tt == left_paren_tt) {
    auto first_semicolon_pos =
        find_first_token(stream.begin() + pos, stream.end(), semicolon_tt);
    if (first_semicolon_pos == -1) {
      throw parsing_error(";", peek(), pos,
    ...
    throw parsing_error("a valid method call statement", peek(), pos,
                        std::source_location::current().function_name());
  }
  if (match(peek(), identifier_tt) &&
      peek(1).tt == left_bracket_tt) { // array access
  ...
  if (match(peek(), identifier_tt) && peek(1).tt == period_tt) {
    ...
    if (match(peek(), assignment_tt)) {
      ...
      const bool handled_member_compound =
          parse_member_compound_assignment(add_assign_tt) ||
          parse_member_compound_assignment(sub_assign_tt) ||
          parse_member_compound_assignment(mul_assign_tt) ||
          parse_member_compound_assignment(div_assign_tt) ||
          parse_member_compound_assignment(mod_assign_tt) ||
          parse_member_compound_assignment(and_assign_tt) ||
          parse_member_compound_assignment(or_assign_tt) ||
          parse_member_compound_assignment(xor_assign_tt) ||
          parse_member_compound_assignment(lshift_assign_tt) ||
  ...
  throw parsing_error("a valid statement or keyword", peek(), pos,
                      std::source_location::current().function_name());
}

/*
    Returns true if the current position looks like a forward-referenced generic
   struct type in a template context (e.g., child_entry<T>* where child_entry
   isn't defined yet). If true, advances pos past the type reference.
*/

bool parser::try_parse_forward_generic_struct_type(
    std::unique_ptr<cst> &out_node) {
  ...
  while (scan < stream.size()) {
    if (stream[scan].tt == greater_than_operator_tt ||
        stream[scan].tt == bitwise_rshift_tt) {
      found_close = true;
      break;
    }
    if (stream[scan].tt == identifier_tt &&
        cst_generic_type_cache.contains(stream[scan].lexeme)) {
      found_generic_param = true;
    }
    if (stream[scan].tt == semicolon_tt ||
        stream[scan].tt == left_curly_bracket_tt) {
      break;
    }
    scan++;
  }
...
void parser::synchronize(const parsing_error &e) {
  ...
  if (!source_lines.empty() && tk.line > 0 &&
      static_cast<std::size_t>(tk.line) <= source_lines.size()) {
    const std::string &src_line =
        source_lines[static_cast<std::size_t>(tk.line) - 1];
    ...
    const std::size_t tildes =
        (tk.lexeme.size() > 1 ? tk.lexeme.size() - 1 : 0);
    std::cout << YELLOW << std::string(spaces, ' ') << '^'
              << std::string(tildes, '~') << RESET << '\n';
  }
...
std::vector<std::vector<token_t>> parser::parse_type_argument_tokens() {
  std::vector<std::vector<token_t>> type_args;
  ...
  while (!match(peek(), end_of_file_tt)) {
    ...
    if (tok.tt == less_than_operator_tt) {
      depth++;
    ...
    if (tok.tt == bitwise_rshift_tt) {
      token_t gt_tok = tok;
      gt_tok.tt = greater_than_operator_tt;
      gt_tok.lexeme = ">";

      for (int close = 0; close < 2; close++) {
        depth -= 1;
        if (depth >= 1) {
          current_arg.push_back(gt_tok);
        } else {
          type_args.push_back(current_arg);
          current_arg.clear();
          consume(); // consume >> token once
          return type_args;
        }
      }
    ...
    if (tok.tt == greater_than_operator_tt) {
      depth -= 1;
      if (depth >= 1) {
        current_arg.push_back(tok);
        consume();
...
bool parser::type_args_have_generic_params(
    const std::vector<std::vector<token_t>> &type_args) const {
  for (const auto &arg_tokens : type_args) {
    for (const auto &tok : arg_tokens) {
      if (tok.tt == identifier_tt &&
          cst_generic_type_cache.contains(tok.lexeme)) {
        return true;
      }
    }
  }
...
std::string
parser::mangle_type_token_sequence(const std::vector<token_t> &tokens) const {
  std::string mangled_piece;

  for (const auto &tok : tokens) {
    if (tok.tt == comma_tt || tok.tt == less_than_operator_tt ||
        tok.tt == greater_than_operator_tt) {
      continue;
    }

    std::string piece = tok.lexeme;
    if (tok.tt == scope_resolution_tt) {
      piece = "ns";
    } else if (tok.tt == multiply_operator_tt) {
      piece = "ptr";
    } else if (tok.tt == reference_operator_tt) {
      piece = "ref";
    }
    ...
    if (!sanitized.empty()) {
      if (!mangled_piece.empty()) {
        mangled_piece.push_back('_');
      }
      mangled_piece += sanitized;
    }
  }

  return mangled_piece;
}

std::string parser::mangle_generic_struct_name(
    const std::string &template_name,
    const std::vector<std::vector<token_t>> &concrete_type_args) const {
  std::string mangled = template_name;

  for (const auto &arg_tokens : concrete_type_args) {
    auto piece = mangle_type_token_sequence(arg_tokens);
    if (!piece.empty()) {
      mangled.push_back('_');
      mangled += piece;
    }
...
std::string parser::instantiate_generic_struct(
    const std::string &template_name,
    const std::vector<std::vector<token_t>> &concrete_type_args) {
  const std::string mangled_name =
      mangle_generic_struct_name(template_name, concrete_type_args);
  std::string base_template_name = template_name;
  if (auto scope_pos = base_template_name.find_last_of(':');
      scope_pos != std::string::npos) {
    base_template_name = base_template_name.substr(scope_pos + 1);
  }
  if (custom_type_map.contains(mangled_name))
    return mangled_name;

  auto &tmpl = generic_struct_templates[template_name];

  std::unordered_map<std::string, std::vector<token_t>> type_map;
  for (std::size_t i = 0;
       i < tmpl.type_params.size() && i < concrete_type_args.size(); i++) {
    type_map[tmpl.type_params[i]] = concrete_type_args[i];
  }

  std::vector<token_t> substituted;
  for (auto tok : tmpl.tokens) {
    if (tok.tt == identifier_tt && type_map.contains(tok.lexeme)) {
      const auto &mapped = type_map[tok.lexeme];
      substituted.insert(substituted.end(), mapped.begin(), mapped.end());
    } else if (tok.tt == identifier_tt && (tok.lexeme == template_name ||
                                           tok.lexeme == base_template_name)) {
      tok.lexeme = mangled_name;
...
void parser::instantiate_generic_function(
    const std::string &template_name,
    const std::vector<std::vector<token_t>> &concrete_type_args) {
  ...
  for (const auto &tok : tmpl.tokens) {
    if (tok.tt == identifier_tt && type_map.contains(tok.lexeme)) {
      const auto &mapped = type_map[tok.lexeme];
      substituted.insert(substituted.end(), mapped.begin(), mapped.end());
    } else if (!name_mangled && tok.tt == identifier_tt &&
               tok.lexeme == template_name) {
...
std::vector<token_t>
parser::preprocess_generic_calls(const std::vector<token_t> &expr) {
  ...
  for (std::size_t j = 0; j < expr.size(); j++) {
    ...
    if (expr[j].tt == identifier_tt && j + 2 < expr.size() &&
        expr[j + 1].tt == scope_resolution_tt &&
        expr[j + 2].tt == identifier_tt && j + 3 < expr.size() &&
        expr[j + 3].tt == less_than_operator_tt) {
    ...
    bool is_generic_pattern =
        (expr[check_idx].tt == identifier_tt && check_idx + 1 < expr.size() &&
         expr[check_idx + 1].tt == less_than_operator_tt);
    ...
    if (is_generic_pattern) {
      ...
      while (scan < expr.size()) {
        if ((expr[scan].tt == greater_than_operator_tt ||
             expr[scan].tt == bitwise_rshift_tt) &&
            scan + 1 < expr.size() && expr[scan + 1].tt == left_paren_tt) {
          looks_like_generic_call = true;
          break;
        }
        if (expr[scan].tt == semicolon_tt ||
            expr[scan].tt == left_curly_bracket_tt) {
          break;
        }
        scan++;
      }
      ...
      if (looks_like_generic_call) {
        ...
        std::size_t scan = check_idx + 2; // position after '<'
        int depth = 1;
        std::vector<std::vector<token_t>> type_args;
        std::vector<token_t> current_arg;
        bool finished = false;

        while (scan < expr.size() && !finished) {
          auto tok = expr[scan];
        ...
        j = scan; // position at final closing token

        // check if any type arg is still an unresolved generic type parameter
        bool has_generic_param = type_args_have_generic_params(type_args);
          ...
          if (is_module_qualified) {
            for (auto &child : root->get_children()) {
              if (child->get_type() == cst_type::function &&
                ...
                auto *name_node = child->get_children().front().get();
                if (name_node->get_type() == cst_type::identifier &&
                    name_node->content == mangled_name) {
...
void parser::import_generic_templates(
    const std::unordered_map<std::string, generic_template> &struct_templates,
    ...
    const std::unordered_map<
        std::string, std::unordered_map<std::string, std::int64_t>> &enums) {
...
std::unique_ptr<cst_root> parser::parse() {
  while (!match(peek(), end_of_file_tt)) {
    try {
      ...
      std::cout << tk.line << ':' << tk.column << ": internal parser error — "
                << e.what() << '\n';
